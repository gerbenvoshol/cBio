...
main(int argc, char **argv)
{
	...
	if ((w = esl_stopwatch_Create()) == NULL) {
		cm_Fail("out of memory, trying to create stopwatch");
	}
...
serial_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if ((status = init_master_cfg(go, cfg, errbuf)) != eslOK) {
		cm_Fail(errbuf);
	}
	do_sample  = esl_opt_GetBoolean(go, "--sample") ? TRUE : FALSE;
	if (do_sample) {
		if ((r = esl_randomness_Create(esl_opt_GetInteger(go, "--seed"))) == NULL) {
			cm_Fail("out of memory, trying to create RNG");
		}
	}
	...
	if (status != eslEOF) {
		cm_Fail("CM file %s does not contain just one CM\n", cfg->cmfp->fname);
	}

	if (cfg->ofp != stdout) {
		output_header(stdout, go, cfg->cmfile, cfg->sqfile, cm, ncpus);
	}
	...
	for (k = 0; k < ncpus * 2; k++) {
		init_sqA[k] = NULL;
		if ((init_sqA[k] = esl_sq_CreateDigital(cfg->abc)) == NULL) {
			cm_Fail("Failed to allocate a sequence");
		}
		if ((status      = esl_workqueue_Init(queue, init_sqA[k])) != eslOK) {
			cm_Fail("Failed to add sequence to work queue");
		}
	}
#endif
	...
	if ((status = initialize_cm(go, cfg, errbuf, cm)) != eslOK) {
		cm_Fail(errbuf);
	}
	for (k = 0; k < infocnt; ++k) {
		if ((status = cm_Clone(cm, errbuf, &(info[k].cm))) != eslOK) {
			cm_Fail(errbuf);
		}
	}
	reached_eof = FALSE;
	...
	if ((map_file = esl_opt_GetString(go, "--mapali")) != NULL) {
		if ((status = map_alignment(map_file, cm, esl_opt_GetBoolean(go, "--noss"), errbuf, &map_dataA, &nmap_data, &map_sscons)) != eslOK) {
			cm_Fail(errbuf);
		}
		if (esl_opt_GetBoolean(go, "--mapstr") && map_sscons == NULL) {
			cm_Fail("Failed to read SS_cons for --mapstr from %s", map_file);
		}
	}
	...
	while (sstatus == eslOK) {
		...
		if (ncpus > 0) {
			for (k = 0; k < infocnt; ++k) {
				esl_threads_AddThread(threadObj, &info[k]);
			}
		}
#endif
		...
		if ((! reached_eof) && cfg->do_oneblock) {
			esl_fatal("Error: the sequence file is too big (has > %d seqs or %d residues) for --ileaved or output\nformat other than Pfam. Use esl-reformat to reformat alignment later.", CMALIGN_MAX_NSEQ, CM_MAX_RESIDUE_COUNT);
		}

		/* align the sequences in the block */
#ifdef HMMER_THREADS
		if (ncpus > 0) {
			status = thread_loop(info, errbuf, threadObj, queue, sq_block);
		} else {
		...
		if (nmap_cur > 0) {
			for (i = 0; i < nmap_cur; i++) {
				merged_dataA[i] = map_dataA[i];
			}
		...
		if (use_tmpfile && (! created_tmpfile)) {
			...
			if ((status = esl_tmpfile_named(tmpfile, &(cfg->tmpfp))) != eslOK) {
				cm_Fail("Failed to open temporary output file (status %d)", status);
			}
			created_tmpfile = TRUE;
		}
		if ((status   = output_alignment(go, cfg, errbuf, cm, (use_tmpfile ? cfg->tmpfp : cfg->ofp), merged_dataA, nseq_cur + nmap_cur, map_sscons)) != eslOK) {
			cm_Fail(errbuf);
		}
		/* optionally output same alignment to regress file */
		if (cfg->rfp != NULL) {
			if ((status = output_alignment(go, cfg, errbuf, cm, cfg->rfp,                              merged_dataA, nseq_cur + nmap_cur, map_sscons)) != eslOK) {
				cm_Fail(errbuf);
			}
		}
		nali++;
		...
		if (cfg->ofp != stdout) {
			if ((status =  output_scores(stdout,   cm, errbuf, merged_dataA, nseq_cur + nmap_cur, nmap_cur, cfg->be_verbose)) != eslOK) {
				cm_Fail(errbuf);
			}
		}
		...
		if (cfg->sfp != NULL) {
			...
			if ((status =  output_scores(cfg->sfp, cm, errbuf, merged_dataA, nseq_cur + nmap_cur, nmap_cur, cfg->be_verbose)) != eslOK) {
				cm_Fail(errbuf);
			}
		}
		...
		for (i = 0; i < nmap_cur; i++) { /* free the mapali seqs if nec */
			if (merged_dataA[i]->sq != NULL) {
				esl_sq_Destroy(merged_dataA[i]->sq);
			}
		}
		for (i = 0; i < nmerged; i++) {
	...
	} else if (sstatus == eslEMEM) {
		cm_Fail("Out of memory");
	} else if (sstatus != eslEOF) {
		cm_Fail("Unexpected error while reading sequence file");
	}
	...
	if (created_tmpfile) {
		...
		if ((status = create_and_output_final_msa(go, cfg, errbuf, cm, nali, tmpfile)) != eslOK) {
			cm_Fail(errbuf);
		}
		remove(tmpfile);
	}
	...
	for (k = 0; k < infocnt; ++k) {
		...
		if (info[k].w     != NULL) {
			esl_stopwatch_Destroy(info[k].w);
		}
		if (info[k].w_tot != NULL) {
			esl_stopwatch_Destroy(info[k].w_tot);
		}
	}
	free(info);
...
thread_loop(WORKER_INFO *info, char *errbuf, ESL_THREADS *obj, ESL_WORK_QUEUE *queue, ESL_SQ_BLOCK *sq_block)
{
	...
	if (status != eslOK) {
		cm_Fail("Work queue reader failed");
	}
...
pipeline_thread(void *arg)
{
	...
	if (status != eslOK) {
		cm_Fail("Work queue worker failed\n");
	}
	...
	while (sq->L != -1) {
		...
		if (info->n == nalloc) {
			...
			for (j = nalloc; j < info->n + allocsize; j++) {
				info->dataA[j] = NULL;
			}
			nalloc += allocsize;
		}
...
mpi_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if (esl_opt_GetBoolean(go, "--sample")) {
		mpi_failure("--sample does not work with in MPI mode (b/c results would not be exactly reproducible)");
	}

	/* Read one CM, and make sure there's only one. This fills cfg->abc. */
	status = cm_file_Read(cfg->cmfp, TRUE, &(cfg->abc), &cm);
	if (status != eslOK) {
		mpi_failure(cfg->cmfp->errbuf);
	}
	status = cm_file_Read(cfg->cmfp, TRUE, &(cfg->abc), NULL);
	if (status != eslEOF) {
		mpi_failure("CM file %s does not contain just one CM\n", cfg->cmfp->fname);
	}

	nworkers  = cfg->nproc - 1;
	if (cfg->ofp != stdout) {
		output_header(stdout, go, cfg->cmfile, cfg->sqfile, cm, nworkers + 1);
	}
	...
	if ((map_file = esl_opt_GetString(go, "--mapali")) != NULL) {
		...
		if (esl_opt_GetBoolean(go, "--mapstr") && map_sscons == NULL) {
			mpi_failure("Failed to read SS_cons for --mapstr from %s", map_file);
		}
	}
	...
	while (sstatus == eslOK) {
		...
		while (have_work || nworking > 0) {
			...
			if (mpistatus.MPI_TAG == INFERNAL_ALNDATA_TAG) {
				...
				if (status != eslOK) {
					mpi_failure("problem with alignment results received from worker %d", wi);
				}
			...
			if (have_work) { /* send new sequence: si's dsq, L, and seqidx to the worker */
				...
				if (status != eslOK) {
					mpi_failure("problem sending dsq to worker %d", wi);
				}
		...
		if (nworking != 0) {
			mpi_failure("%d workers still working when all should be idle", nworking);
		}
		...
		for (wi = 1; wi < cfg->nproc; wi++) {
			if ((status = cm_dsq_MPISend(NULL, -1, -1, wi, INFERNAL_DSQ_TAG, MPI_COMM_WORLD, &mpibuf, &mpibuf_size)) != eslOK) {
				mpi_failure(errbuf);
			}
		}
		...
		if (use_tmpfile && (! created_tmpfile)) {
			/* first aln for temporary output file, open the file */
			if ((status = esl_tmpfile_named(tmpfile, &(cfg->tmpfp))) != eslOK) {
				mpi_failure("Failed to open temporary output file (status %d)", status);
			}
			created_tmpfile = TRUE;
		}
		if ((status   = output_alignment(go, cfg, errbuf, cm, (use_tmpfile ? cfg->tmpfp : cfg->ofp), merged_dataA, nseq_cur + nmap_cur, map_sscons)) != eslOK) {
	...
	if (sstatus == eslEFORMAT) {
		...
		mpi_failure("Unexpected error while reading sequence file");
	}
...
mpi_worker(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if ((status = init_shared_cfg(go, cfg, errbuf)) != eslOK) {
		mpi_failure(errbuf);
	}
	if (esl_opt_GetBoolean(go, "--sample")) {
		mpi_failure("--sample does not work with in MPI mode (b/c results would not be exactly reproducible)");
	...
	while (blocks_remain_in_file) {
		...
		if (status == eslOK  && dsq    == NULL) {
			mpi_failure("problem receiving 1st dsq");
		} else if (status == eslEOD && dsq    != NULL) {
			mpi_failure("problem receiving termination signal");
		} else if (status != eslOK  && status != eslEOD) {
			mpi_failure("problem receiving 1st dsq");
		}
		...
		while (seqs_remain_in_block) {
			...
			if ((sq = esl_sq_CreateDigitalFrom(cfg->abc, "irrelevant", dsq, L, NULL, NULL, NULL)) == NULL) {
				mpi_failure("out of memory");
			}
			...
			status = cm_dsq_MPIRecv(0, INFERNAL_DSQ_TAG, MPI_COMM_WORLD, &mpibuf, &mpibuf_size, &dsq, &L, &idx);
			if (status == eslOK  && dsq    == NULL) {
				mpi_failure("problem receiving dsq");
			} else if (status == eslEOD && dsq    != NULL) {
				mpi_failure("problem receiving termination signal");
			} else if (status != eslOK  && status != eslEOD) {
				mpi_failure("problem receiving dsq");
			...
			if (dsq == NULL) {
				seqs_remain_in_block = FALSE;    /* we're done with this block */
			}
#if DEBUGMPI
			if (dsq == NULL) {
				printf("worker received NULL dsq from master, block is done");
			}
#endif
	...
	if (info.w     != NULL) {
		esl_stopwatch_Destroy(info.w);
	}
	if (info.w_tot != NULL) {
		esl_stopwatch_Destroy(info.w_tot);
	}
	if (mpibuf     != NULL) {
		free(mpibuf);
	}
...
process_commandline(int argc, char **argv, ESL_GETOPTS **ret_go, char **ret_cmfile, char **ret_sqfile, int *ret_infmt, int *ret_outfmt)
{
	...
	if ((*ret_sqfile = esl_opt_GetArg(go, 2)) == NULL)  {
...
output_header(FILE *ofp, const ESL_GETOPTS *go, char *cmfile, char *sqfile, CM_t *cm, int ncpus)
{
	...
	if (esl_opt_IsUsed(go, "-o"))          {
		fprintf(ofp, "# saving alignment to file:                    %s\n", esl_opt_GetString(go, "-o"));
	}
	if (esl_opt_IsUsed(go, "-g"))          {
		fprintf(ofp, "# model configuration:                         global\n");
	}

	if (esl_opt_IsUsed(go, "--optacc"))    {
		fprintf(ofp, "# alignment algorithm:                         optimal accuracy\n");
	}
	if (esl_opt_IsUsed(go, "--cyk"))       {
		fprintf(ofp, "# alignment algorithm:                         CYK\n");
	}
	if (esl_opt_IsUsed(go, "--sample"))    {
		fprintf(ofp, "# sampling aln from posterior distribution:    yes\n");
	}
	...
	if (esl_opt_IsUsed(go, "--notrunc"))   {
		fprintf(ofp, "# truncated sequence alignment mode:           off\n");
	}
	if (esl_opt_IsUsed(go, "--sub"))       {
		fprintf(ofp, "# alternative truncated seq alignment mode:    on\n");
	}

	if (esl_opt_IsUsed(go, "--mxsize"))    {
		fprintf(ofp, "# maximum total DP matrix size set to:         %.2f Mb\n", esl_opt_GetReal(go, "--mxsize"));
	}
	if (esl_opt_IsUsed(go, "--hbanded"))   {
		fprintf(ofp, "# using HMM bands for acceleration:            yes\n");
	}
	if (esl_opt_IsUsed(go, "--tau"))       {
		fprintf(ofp, "# tail loss probability for HMM bands set to:  %g\n", esl_opt_GetReal(go, "--tau"));
	}
	if (esl_opt_IsUsed(go, "--fixedtau"))  {
		fprintf(ofp, "# tighten HMM bands when necessary:            no\n");
	}
	if (esl_opt_IsUsed(go, "--maxtau"))    {
		fprintf(ofp, "# maximum tau allowed during band tightening:  %g\n", esl_opt_GetReal(go, "--maxtau"));
	}
	if (esl_opt_IsUsed(go, "--nonbanded")) {
		fprintf(ofp, "# using HMM bands for acceleration:            no\n");
	}
	if (esl_opt_IsUsed(go, "--small"))     {
		fprintf(ofp, "# small memory D&C alignment algorithm:        on\n");
	}

	if (esl_opt_IsUsed(go, "--sfile"))     {
		fprintf(ofp, "# saving alignment score info to file:         %s\n", esl_opt_GetString(go, "--sfile"));
	}
	if (esl_opt_IsUsed(go, "--tfile"))     {
		fprintf(ofp, "# saving parsetrees to file:                   %s\n", esl_opt_GetString(go, "--tfile"));
	}
	if (esl_opt_IsUsed(go, "--ifile"))     {
		fprintf(ofp, "# saving insert information to file:           %s\n", esl_opt_GetString(go, "--ifile"));
	}
	if (esl_opt_IsUsed(go, "--elfile"))    {
		fprintf(ofp, "# saving local end information to file:        %s\n", esl_opt_GetString(go, "--elfile"));
	}

	if (esl_opt_IsUsed(go, "--mapali"))    {
		fprintf(ofp, "# including alignment from file:               %s\n", esl_opt_GetString(go, "--mapali"));
	}
	if (esl_opt_IsUsed(go, "--mapstr"))    {
		fprintf(ofp, "# including structure from alnment from file:  %s\n", esl_opt_GetString(go, "--mapali"));
	}
	if (esl_opt_IsUsed(go, "--informat"))  {
		fprintf(ofp, "# input sequence file format specified as:     %s\n", esl_opt_GetString(go, "--informat"));
	}
	if (esl_opt_IsUsed(go, "--outformat")) {
		fprintf(ofp, "# output alignment format specified as:        %s\n", esl_opt_GetString(go, "--outformat"));
	}
	if (esl_opt_IsUsed(go, "--dnaout"))    {
		fprintf(ofp, "# output alignment alphabet:                   DNA\n");
	}
	if (esl_opt_IsUsed(go, "--noprob"))    {
		fprintf(ofp, "# posterior probability annotation:            off\n");
	}
	if (esl_opt_IsUsed(go, "--matchonly")) {
		fprintf(ofp, "# include alignment insert columns:            no\n");
	}
	if (esl_opt_IsUsed(go, "--ileaved"))   {
		fprintf(ofp, "# forcing interleaved Stockholm output aln:    yes\n");
	}
	if (esl_opt_IsUsed(go, "--regress"))   {
		fprintf(ofp, "# saving alignment without author info to:     %s\n", esl_opt_GetString(go, "--regress"));
	}
...
init_master_cfg(const ESL_GETOPTS *go, struct cfg_s *cfg, char *errbuf)
{
	...
	if (esl_opt_IsUsed(go, "--elfile")) {
		if ((cfg->efp = fopen(esl_opt_GetString(go, "--elfile"), "w")) == NULL) {
	...
	if (esl_opt_IsUsed(go, "--regress")) {
		if ((cfg->rfp = fopen(esl_opt_GetString(go, "--regress"), "w")) == NULL) {
...
init_shared_cfg(const ESL_GETOPTS *go, struct cfg_s *cfg, char *errbuf)
{
	...
	} else if (status == eslEINVAL && cfg->infmt == eslSQFILE_UNKNOWN) {
...
initialize_cm(const ESL_GETOPTS *go, struct cfg_s *cfg, char *errbuf, CM_t *cm)
{
	...
	} else if (esl_opt_GetBoolean(go, "--sample")) {
	...
	if (! esl_opt_GetBoolean(go, "--noprob")) {
	...
	if (esl_opt_GetBoolean(go, "--sub")) {
		cm->align_opts |= CM_ALIGN_SUB;    /* --sub requires --notrunc */
	}
	if (esl_opt_GetBoolean(go, "--small")) {
		cm->align_opts |= CM_ALIGN_SMALL;    /* --small requires --noprob --nonbanded --cyk */
	}
	...
	if (esl_opt_GetBoolean(go, "--sub")) {
		cm->config_opts |= CM_CONFIG_SUB;    /* --sub requires --notrunc */
	}
...
map_alignment(const char *msafile, CM_t *cm, int noss_used, char *errbuf, CM_ALNDATA ***ret_dataA, int *ret_ndata, char **ret_ss)
{
	...
	if (!(cm->flags & CMH_CHKSUM)) {
		cm_Fail("CM has no checksum. --mapali unreliable without it.");
	}
	if (!(cm->flags & CMH_MAP)) {
		cm_Fail("CM has no map. --mapali can't work without it.");
	}
	esl_msa_Checksum(msa, &chksum);
	if (cm->checksum != chksum) {
		cm_Fail("--mapali MSA %s isn't same as the one CM came from (checksum mismatch)", msafile);
	}

	/* allocate and initialize dataA */
	ESL_ALLOC(dataA, sizeof(CM_ALNDATA *) * msa->nseq);
	for (i = 0; i < msa->nseq; i++) {
		dataA[i] = cm_alndata_Create();
	}
	...
	if (msa->ss_cons != NULL) {
		...
		for (cpos = 1; cpos <= cm->clen; cpos++) {
			i_am_rf[cm->map[cpos]] = 1;
		}

		/* get CT array that describes all basepairs in the ss_cons (ct array is 1..alen, not 0..alen-1 */
		if ((status = esl_strdup(msa->ss_cons, msa->alen, &msa_ss_cons_copy)) != eslOK) {
			cm_Fail("Out of memory");
		}
		if ((status = esl_wuss2ct(msa_ss_cons_copy, msa->alen, msa_ct)) != eslOK) {
			cm_Fail("Problem including structure from --mapali, maybe out of memory");
		}
		...
		ss[cm->clen] = '\0';
		for (cpos = 1; cpos <= cm->clen; cpos++) {
			ss[cpos - 1] = msa_ss_cons_copy[cm->map[cpos] - 1];
		}
	} else {
	...
	for (cpos = 1; cpos <= cm->clen;  cpos++) {
		msa->rf[cm->map[cpos] - 1] = 'x';    /* note off by one */
	}
	...
	a2u_map[0] = -1; /* invalid */
	for (i = 0; i < msa->nseq; i++) {
		if ((status = Transmogrify(cm, errbuf, mtr, msa->ax[i], used_el, msa->alen, &(dataA[i]->tr))) != eslOK) {
			return status;
		}
		...
		for (x = 0; x < dataA[i]->tr->n; x++) {
			if (dataA[i]->tr->emitl[x] != -1) {
				dataA[i]->tr->emitl[x] = a2u_map[dataA[i]->tr->emitl[x]];
			}
			if (dataA[i]->tr->emitr[x] != -1) {
				dataA[i]->tr->emitr[x] = a2u_map[dataA[i]->tr->emitr[x]];
			}
		}
	}
	...
	/* get sequences */
	for (i = 0; i < msa->nseq; i++) {
		esl_sq_FetchFromMSA(msa, i, &(dataA[i]->sq));
	}
	...
	*ret_ss    = ss;
	...
	if (msa_ss_cons_copy != NULL) {
		free(msa_ss_cons_copy);
	}
...
ERROR:
	...
	*ret_dataA = NULL;
	if (dataA     != NULL) {
		for (i = 0; i < msa->nseq; i++) {
			cm_alndata_Destroy(dataA[i], TRUE);
		}
		dataA = NULL;
	}
	if (afp             != NULL) {
...
output_alignment(ESL_GETOPTS *go, struct cfg_s *cfg, char *errbuf, CM_t *cm, FILE *ofp, CM_ALNDATA **dataA, int ndata, char *map_sscons)
{
	...
	/* contract check */
	if (ofp == cfg->tmpfp && esl_opt_GetBoolean(go, "--ileaved")) {
		ESL_FAIL(eslEINVAL, errbuf, "--ileaved enabled, but trying to output to temporary alignment file. This shouldn't happen.");
	}
	if (ofp == cfg->tmpfp && cfg->outfmt != eslMSAFILE_PFAM && cfg->outfmt != eslMSAFILE_STOCKHOLM) {
		ESL_FAIL(eslEINVAL, errbuf, "output format not Stockholm, nor Pfam, but trying to output to temporary alignment file. This shouldn't happen.");
	}
	if (esl_opt_GetBoolean(go, "--mapstr") && map_sscons == NULL) {
		ESL_FAIL(eslEINVAL, errbuf, "--mapstr enabled, but SS_cons not read from the --mapali alignment.");
	}

	/* output the parsetrees, if nec */
	if (cfg->tfp != NULL) {
		for (j = 0; j < ndata; j++) {
			if ((status = ParsetreeScore(cm, NULL, errbuf, dataA[j]->tr, dataA[j]->sq->dsq, FALSE, &sc, &struct_sc, NULL, NULL, NULL)) != eslOK) {
				return status;
			}
			fprintf(cfg->tfp, ">%s\n", dataA[j]->sq->name);
	...
	if (first_ali && cfg->ifp != NULL) {
		fprintf(cfg->ifp, "%s %d\n", cm->name, cm->clen);
	}
	if (first_ali && cfg->efp != NULL) {
		fprintf(cfg->efp, "%s %d\n", cm->name, cm->clen);
	}

	/* create the alignment */
	ESL_ALLOC(sqpA,   sizeof(ESL_SQ *)      * ndata);
	for (j = 0; j < ndata; j++) {
		sqpA[j]   = dataA[j]->sq;
	}
	ESL_ALLOC(trA,    sizeof(Parsetree_t *) * ndata);
	for (j = 0; j < ndata; j++) {
		trA[j]    = dataA[j]->tr;
	}
	ESL_ALLOC(ppstrA, sizeof(char *)        * ndata);
	for (j = 0; j < ndata; j++) {
		ppstrA[j] = dataA[j]->ppstr;
	}
	if ((status = Parsetrees2Alignment(cm, errbuf, cfg->abc_out, sqpA, NULL, trA, ppstrA, ndata, cfg->ifp, cfg->efp, TRUE, esl_opt_GetBoolean(go, "--matchonly"), &msa)) != eslOK) {
		return status;
	}
	...
	if (status == eslEMEM) {
		ESL_FAIL(status, errbuf, "Memory error when outputting alignment\n");
	} else if (status != eslOK) {
		ESL_FAIL(status, errbuf, "Writing alignment file failed with error %d\n", status);
	}
...
output_scores(FILE *ofp, CM_t *cm, char *errbuf, CM_ALNDATA **dataA, int ndata, int first_idx, int be_verbose)
{
	...
	for (i = first_idx; i < ndata; i++) {
		namewidth = ESL_MAX(namewidth, strlen(dataA[i]->sq->name));
	}
	...
	do {
		idxwidth++;
		maxidx /= 10;
	} while (maxidx); /* poor man's (int)log_10(maxidx)+1 */
	...
	if (be_verbose) {
		fprintf(ofp, "  %7s  %7s  %7s  %8s", "tau", "thresh1", "thresh2", "failover");
	}
	fprintf(ofp, "\n");
	...
	for (i = first_idx; i < ndata; i++) {
		...
		if (do_sub) {
			if (dataA[i]->spos != 1 && dataA[i]->epos != cm->clen) {
				fprintf(ofp, "  %5s", "5'&3'");
			} else if (dataA[i]->spos == 1 && dataA[i]->epos != cm->clen) {
				fprintf(ofp, "  %5s", "3'");
			} else if (dataA[i]->spos != 1 && dataA[i]->epos == cm->clen) {
				fprintf(ofp, "  %5s", "5'");
			} else if (dataA[i]->spos == 1 && dataA[i]->epos == cm->clen) {
			...
			if (dataA[i]->tr->mode[0] == TRMODE_T) {
				fprintf(ofp, "  %5s", "5'&3'");
			} else if (dataA[i]->tr->mode[0] == TRMODE_L) {
				fprintf(ofp, "  %5s", "3'");
			} else if (dataA[i]->tr->mode[0] == TRMODE_R) {
				fprintf(ofp, "  %5s", "5'");
			} else {
				fprintf(ofp, "  %5s", "no");
			}
		...
		if (do_post) {
			fprintf(ofp, "  %6.3f", dataA[i]->pp);
		} else {
			fprintf(ofp, "  %6s",   "-");
		}
		if (! do_nonbanded) {
			fprintf(ofp, "  %9.2f", dataA[i]->secs_bands);
		} else {
			fprintf(ofp, "  %9s",   "-");
		}
		...
		if (be_verbose) {
			if (dataA[i]->tau > -0.5) {
				fprintf(ofp, "  %7.2g", dataA[i]->tau);    /* tau is -1. if aln did not use HMM bands */
			} else {
				fprintf(ofp, "  %7s", "-");
			}
			if (do_trunc) {
				fprintf(ofp, "  %7.2f  %7.2f", dataA[i]->thresh1, dataA[i]->thresh2);
			} else {
				fprintf(ofp, "  %7s  %7s", "-", "-");
			}
...
create_and_output_final_msa(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, CM_t *cm, int nali, char *tmpfile)
{
	 ...
	 ****************************************************************************/
	if ((status = esl_msafile2_Open(tmpfile, NULL, &afp)) != eslOK) {
		cm_Fail("unable to open temp file %s for reading", tmpfile);
	}
	...
	for (ai = 0; ai < nali; ai++) {
		...
		if (status == eslEFORMAT) {
			cm_Fail("Rereading alignment %d for merging, parse error:\n%s\n", ai + 1, afp->errbuf);
		...
		msaA[ai]->abc = cfg->abc;
		if (msaA[ai]->rf == NULL) {
			ESL_FAIL(eslEINCONCEIVABLE, errbuf, "When rereading alignment %d for merging, no RF annotation found.", ai + 1);
		}
		cur_clen = 0;
		for (apos = 0; apos < (int) alen_cur; apos++) {
			if ((! esl_abc_CIsGap(msaA[ai]->abc, msaA[ai]->rf[apos])) && (! esl_abc_CIsMissing(msaA[ai]->abc, msaA[ai]->rf[apos]))) {
				cur_clen++;
			}
		}
		if (cur_clen != cm->clen) {
			ESL_FAIL(eslEINCONCEIVABLE, errbuf, "When rereading alignment %d for merging, consensus length wrong (%d, when %d was expected)", ai, cur_clen, cm->clen);
		}
	...
	if (status != eslEOF) {
		ESL_FAIL(status, errbuf, "More alignments in temp file than expected.");
	}
	...
	if (gs_exists) {
		if ((status = esl_msafile2_Open(tmpfile, NULL, &afp)) != eslOK) {
			cm_Fail("unable to open temp file %s for reading on second pass", tmpfile);
		}
		for (ai = 0; ai < nali; ai++) {
			...
			status = esl_msafile2_RegurgitatePfam(afp, cfg->ofp,
			                                      ...
			                                      '.', NULL, NULL);
			if (status == eslEOF) {
				cm_Fail("Second pass, error out of temp alignments too soon, when trying to read aln %d", ai);
			}
			if (status != eslOK) {
				cm_Fail("Second pass, error reading temp alignment %d %s", ai, afp->errbuf);
			}
			fflush(cfg->ofp);
		}
		esl_msafile2_Close(afp);
	...
	if ((status = esl_msafile2_Open(tmpfile, NULL, &afp)) != eslOK) {
		cm_Fail("unable to open temp file %s for reading on second (or third) pass", tmpfile);
	}
	...
	for (ai = 0; ai < nali; ai++) {
		...
		status = esl_msafile2_RegurgitatePfam(afp, cfg->ofp,
		                                      ...
		                                      '.', NULL, NULL);
		if (status == eslEOF) {
			cm_Fail("Second pass, error out of alignments too soon, when trying to read temp aln %d", ai);
		}
		if (status != eslOK) {
			cm_Fail("Second pass, error reading temp alignment %d: %s", ai, afp->errbuf);
		}
...
update_maxins_and_maxel(ESL_MSA *msa, int clen, int64_t alen, int *maxins, int *maxel)
{
	...
	maxel[cpos]  = ESL_MAX(maxel[cpos], nel);
	if (cpos != clen) {
		cm_Fail("Unexpected error in update_maxins_and_maxel(), expected clen (%d) not equal to actual clen (%d).\n", clen, cpos);
	}
...
determine_gap_columns_to_add(ESL_MSA *msa, int *maxins, int *maxel, int clen, int **ret_ngap_insA, int **ret_ngap_elA, int **ret_ngap_eitherA, char *errbuf)
{
	...
	/* contract check */
	if (maxel[0]     != 0) {
		ESL_FAIL(eslEINVAL, errbuf, "missing characters exist prior to first cpos, this shouldn't happen.\n");
	}
	if (msa->ss_cons == NULL) {
		ESL_FAIL(eslEINVAL, errbuf, "MSA's SS_cons is null in determine_gap_columns_to_add.\n");
	}
	...
	for (apos = 0; apos < msa->alen; apos++) {
		if (esl_abc_CIsMissing(msa->abc, msa->rf[apos])) {
			nel++;
			if (nins > 0) {
				ESL_FAIL(eslEINVAL, errbuf, "after nongap RF pos %d, %d gap columns precede a missing data column (none should)", cpos, nins);
			}
			...
			/* a few sanity checks */
			if (nins > maxins[cpos]) {
				ESL_FAIL(eslEINCONCEIVABLE, errbuf, "%d inserts before cpos %d greater than max expected (%d).\n", nins, cpos, maxins[cpos]);
			}
			if (nel  > maxel[cpos]) {
				ESL_FAIL(eslEINCONCEIVABLE, errbuf, "%d EL inserts before cpos %d greater than max expected (%d).\n", nel, cpos, maxel[cpos]);
			}

			if (cpos == 0) {
				if (nel != 0) {
					ESL_FAIL(eslEINVAL, errbuf, "found missing chars prior to first cpos, shouldn't happen\n");
				}
...
inflate_gc_with_gaps_and_els(FILE *ofp, ESL_MSA *msa, int *ngap_insA, int *ngap_elA, char **ret_ss_cons2print, char **ret_rf2print)
{
	...
	if (msa->ss_cons == NULL) {
		cm_Fail("Error: trying to add inserts to SS_cons, but unexpectedly it doesn't exist.");
	}
	if (msa->rf      == NULL) {
		cm_Fail("Error: trying to add inserts to SS_cons, but unexpectedly it doesn't exist.");
	}
