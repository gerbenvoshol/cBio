...
main(int argc, char **argv)
{
	...
	if (esl_opt_IsOn(go, "--informat")) {
		...
		if (cfg.fmt == eslMSAFILE_UNKNOWN) {
			cm_Fail("%s is not a recognized input sequence file format\n", esl_opt_GetString(go, "--informat"));
		}
	...
	if (esl_sprintf(&mfile,   "%s.i1m",   cfg.cmfile) != eslOK) {
		cm_Fail("esl_sprintf() failed");
	}
	if (esl_sprintf(&ffile,   "%s.i1f",   cfg.cmfile) != eslOK) {
		cm_Fail("esl_sprintf() failed");
	}
	if (esl_sprintf(&pfile,   "%s.i1p",   cfg.cmfile) != eslOK) {
		cm_Fail("esl_sprintf() failed");
	}
	if (esl_sprintf(&ifile,   "%s.i1i",   cfg.cmfile) != eslOK) {
		cm_Fail("esl_sprintf() failed");
	}
	if (esl_sprintf(&ssifile, "%s.i1ssi", cfg.cmfile) != eslOK) {
		cm_Fail("esl_sprintf() failed");
	}
	if (esl_FileExists(mfile)) {
		cm_Fail("Binary CM file %s already exists; you must delete old cmpress %s.i1* files first", mfile, cfg.cmfile);
	}
	if (esl_FileExists(ffile)) {
		cm_Fail("Binary MSV filter file %s already exists; you must delete old cmpress %s.i1* files first", ffile, cfg.cmfile);
	}
	if (esl_FileExists(pfile)) {
		cm_Fail("Binary optimized profile file %s already exists; you must delete old cmpress %s.i1* files first", pfile, cfg.cmfile);
	}
	if (esl_FileExists(ifile)) {
		cm_Fail("Binary SSI index file %s already exists; you must delete old cmpress %s.i1* files first", ifile, cfg.cmfile);
	}
	if (esl_FileExists(ssifile)) {
		cm_Fail("Binary SSI index file %s already exists; you must delete this old cmfetch index file first", ssifile, cfg.cmfile);
	}
	free(mfile);
	free(ffile);
	free(pfile);
	...
	if (cfg.pri        != NULL) {
		Prior_Destroy(cfg.pri);
	}
	if (cfg.pri_zerobp != NULL) {
		Prior_Destroy(cfg.pri_zerobp);
	}
	if (cfg.null       != NULL) {
		free(cfg.null);
	}
	...
	if (cfg.fp7_bg     != NULL) {
		p7_bg_Destroy(cfg.fp7_bg);
	}
	if (cfg.fp7_bld    != NULL) {
		p7_builder_Destroy(cfg.fp7_bld);
	}
...
master(const ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if ((do_ctarget + do_cmindiff + do_call) > TRUE) {
	...
	while ((status = esl_msafile_Read(cfg->afp, &msa)) != eslEOF) {
		if (status != eslOK) {
			esl_msafile_ReadFailure(cfg->afp, status);
		}
		cfg->nali++;

		if (set_msa_name(go, cfg, errbuf, msa) != eslOK) {
			cm_Fail(errbuf);
		}
		if (msa->name == NULL) {
			cm_Fail("Error naming MSA");
		}
		...
		if (do_cluster) { /* divide input MSA into clusters, and build CM from each cluster */
			if ((status = MSADivide(msa, do_call, do_cmindiff, do_ctarget, mindiff, nc,
			                        esl_opt_GetBoolean(go, "--corig"), &ncm, &cmsa, errbuf)) != eslOK) {
				cm_Fail(errbuf);
			}
		...
		for (c = 0; c < ncm; c++) {
			...
			/* msa -> cm */
			if ((status = process_build_workunit(go, cfg, errbuf, msa, &cm, &mtr, &tr)) != eslOK) {
				cm_Fail(errbuf);
			}
			...
			if (esl_opt_IsOn(go, "--refine")) {
				...
				if ((status = refine_msa(go, cfg, errbuf, cm, msa, tr, &new_cm, &new_msa, &new_mtr, &new_tr, &niter)) != eslOK) {
					cm_Fail(errbuf);
				}
				if (cm != new_cm) {
					FreeCM(cm);
			...
			/* output cm */
			if ((status = output_result(go, cfg, errbuf, cfg->nali, cfg->ncm_total, msa,  cm, mtr, tr)) != eslOK) {
				cm_Fail(errbuf);
			}
...
process_commandline(int argc, char **argv, ESL_GETOPTS **ret_go, char **ret_cmfile, char **ret_alifile)
{
	...
	if ((*ret_cmfile = esl_opt_GetArg(go, 1)) == NULL)  {
		puts("Failed to get <cmfile_out> argument on command line");
		goto ERROR;
	}
	if ((*ret_alifile = esl_opt_GetArg(go, 2)) == NULL) {
		puts("Failed to get <alifile> argument on command line");
		goto ERROR;
	}

	if (strcmp(*ret_cmfile, "-") == 0) {
		puts("Can't write <cmfile_out> to stdout: don't use '-'");
		goto ERROR;
	}
	if (strcmp(*ret_alifile, "-") == 0 && ! esl_opt_IsOn(go, "--informat")) {
		puts("Must specify --informat to read <alifile> from stdin ('-')");
		goto ERROR;
	}
...
output_header(FILE *ofp, const ESL_GETOPTS *go, char *cmfile, char *alifile)
{
	...
	if (esl_opt_IsUsed(go, "-n"))            {
		fprintf(ofp, "# name (the single) CM:                               %s\n", esl_opt_GetString(go, "-n"));
	}
	if (esl_opt_IsUsed(go, "-F"))            {
		fprintf(ofp, "# overwrite CM file if necessary:                     yes\n");
	}
	if (esl_opt_IsUsed(go, "-o"))            {
		fprintf(ofp, "# output directed to file:                            %s\n", esl_opt_GetString(go, "-o"));
	}
	if (esl_opt_IsUsed(go, "-O"))            {
		fprintf(ofp, "# processed alignment resaved to:                     %s\n", esl_opt_GetString(go, "-O"));
	}
	if (esl_opt_IsUsed(go, "--symfrac"))     {
		fprintf(ofp, "# minimum symbol fraction in a consensus column:      %g\n", esl_opt_GetReal(go, "--symfrac"));
	}
	if (esl_opt_IsUsed(go, "--hand"))        {
		fprintf(ofp, "# use #=GC RF annotation to define consensus columns: yes\n");
	}
	if (esl_opt_IsUsed(go, "--null"))        {
		fprintf(ofp, "# read null model from file:                          %s\n", esl_opt_GetString(go, "--null"));
	}
	if (esl_opt_IsUsed(go, "--prior"))       {
		fprintf(ofp, "# read prior from file:                               %s\n", esl_opt_GetString(go, "--prior"));
	}
	if (esl_opt_IsUsed(go, "--noss"))        {
		fprintf(ofp, "# ignore secondary structure, if any:                 yes\n");
	}
	if (esl_opt_IsUsed(go, "--rsearch"))     {
		fprintf(ofp, "# RSEARCH parameterization mode w/RIBOSUM mx file:    %s\n", esl_opt_GetString(go, "--rsearch"));
	}
	if (esl_opt_IsUsed(go, "--betaW"))       {
		fprintf(ofp, "# tail loss probability for defining W:               %g\n", esl_opt_GetReal(go, "--betaW"));
	}
	if (esl_opt_IsUsed(go, "--beta1"))       {
		fprintf(ofp, "# tail loss probability for defining tight QDBs:      %g\n", esl_opt_GetReal(go, "--beta1"));
	}
	if (esl_opt_IsUsed(go, "--beta2"))       {
		fprintf(ofp, "# tail loss probability for defining loose QDBs:      %g\n", esl_opt_GetReal(go, "--beta2"));
	}
	if (esl_opt_IsUsed(go, "--informat"))    {
		fprintf(ofp, "# input format specified as:                          %s\n", esl_opt_GetString(go, "--informat"));
	}
	if (esl_opt_IsUsed(go, "--v1p0"))        {
		fprintf(ofp, "# v1.0 parameterization mode:                         on\n");
	}
	if (esl_opt_IsUsed(go, "--p56"))         {
		fprintf(ofp, "# use default priors from v0.56 through v1.0.2:       yes\n");
	}
	if (esl_opt_IsUsed(go, "--iins"))        {
		fprintf(ofp, "# allowing informative insert emission probabilities: yes\n");
	}
	if (esl_opt_IsUsed(go, "--iflank"))      {
		fprintf(ofp, "# allowing informative ROOT_IL/IR transition probs:   yes\n");
	}
	if (esl_opt_IsUsed(go, "--nobalance"))   {
		fprintf(ofp, "# do default rebalancing of CM:                       no\n");
	}
	if (esl_opt_IsUsed(go, "--nodetach"))    {
		fprintf(ofp, "# do default detachment of flawed ambiguous inserts:  no\n");
	}
	if (esl_opt_IsUsed(go, "--elself"))      {
		fprintf(ofp, "# local end (EL) self loop probability:               %g\n", esl_opt_GetReal(go, "--elself"));
	}
	if (esl_opt_IsUsed(go, "--n2omega"))     {
		fprintf(ofp, "# prior probability of null2 model (if used):         %g\n", esl_opt_GetReal(go, "--n2omega"));
	}
	if (esl_opt_IsUsed(go, "--n3omega"))     {
		fprintf(ofp, "# prior probability of null3 model (if used):         %g\n", esl_opt_GetReal(go, "--n3omega"));
	}
	if (esl_opt_IsUsed(go, "--wpb"))         {
		fprintf(ofp, "# relative weighting scheme:                          Henikoff PB\n");
	}
	if (esl_opt_IsUsed(go, "--wgsc"))        {
		fprintf(ofp, "# relative weighting scheme:                          G/S/C\n");
	}
	if (esl_opt_IsUsed(go, "--wnone"))       {
		fprintf(ofp, "# relative weighting scheme:                          none\n");
	}
	if (esl_opt_IsUsed(go, "--wgiven"))      {
		fprintf(ofp, "# relative weighting scheme:                          wts from MSA file\n");
	}
	if (esl_opt_IsUsed(go, "--wblosum"))     {
		fprintf(ofp, "# relative weighting scheme:                          BLOSUM filter\n");
	}
	if (esl_opt_IsUsed(go, "--wid"))         {
		fprintf(ofp, "# frac id cutoff for BLOSUM wgts:                     %f\n", esl_opt_GetReal(go, "--wid"));
	}

	if (esl_opt_IsUsed(go, "--eent"))        {
		fprintf(ofp, "# effective seq number scheme:                        entropy weighting\n");
	}
	if (esl_opt_IsUsed(go, "--enone"))       {
		fprintf(ofp, "# effective seq number scheme:                        none\n");
	}
	if (esl_opt_IsUsed(go, "--ere"))         {
		fprintf(ofp, "# minimum rel entropy target:                         %f bits\n",   esl_opt_GetReal(go, "--ere"));
	}
	if (esl_opt_IsUsed(go, "--eset"))        {
		fprintf(ofp, "# effective seq number:                               set to %f\n", esl_opt_GetReal(go, "--eset"));
	}
	if (esl_opt_IsUsed(go, "--eminseq"))     {
		fprintf(ofp, "# minimum effective sequence number allowed:          %g\n", esl_opt_GetReal(go, "--eminseq"));
	}
	if (esl_opt_IsUsed(go, "--ehmmre"))      {
		fprintf(ofp, "# minimum ML CP9 HMM rel entropy target:              %f bits\n",   esl_opt_GetReal(go, "--ehmmre"));
	}
	if (esl_opt_IsUsed(go, "--esigma"))      {
		fprintf(ofp, "# entropy target sigma parameter:                     %f bits\n",   esl_opt_GetReal(go, "--esigma"));
	}

	if (esl_opt_IsUsed(go, "--refine"))      {
		fprintf(ofp, "# input alignment refinement prior to model building: on\n");
	}
	if (esl_opt_IsUsed(go, "-l"))            {
		fprintf(ofp, "# model configuration for aln refinement:             local\n");
	}
	if (esl_opt_IsUsed(go, "--gibbs"))       {
		fprintf(ofp, "# Gibbs sampling (instead of EM) for aln refinement:  on\n");
	}
	...
	if (esl_opt_IsUsed(go, "--notrunc"))     {
		fprintf(ofp, "# use truncated aln algorithms for aln refinement:    no\n");
	}
	if (esl_opt_IsUsed(go, "--cyk"))         {
		fprintf(ofp, "# use the CYK algorithm instead of optimal accuracy:  yes\n");
	}
	if (esl_opt_IsUsed(go, "--sub"))         {
		fprintf(ofp, "# alternative truncated seq alignment 'sub' mode:     on\n");
	}
	if (esl_opt_IsUsed(go, "--nonbanded"))   {
		fprintf(ofp, "# use HMM bands for accelerating aln refinement:      no\n");
	}
	if (esl_opt_IsUsed(go, "--indi"))        {
		fprintf(ofp, "# print individual seq scores during aln refinement:  yes\n");
	}
	if (esl_opt_IsUsed(go, "--fins"))        {
		fprintf(ofp, "# flush inserts left/rigth during aln refinement:     yes\n");
	}
	if (esl_opt_IsUsed(go, "--tau"))         {
		fprintf(ofp, "# tail loss probability for HMM bands set to:         %g\n", esl_opt_GetReal(go, "--tau"));
	}
	if (esl_opt_IsUsed(go, "--mxsize"))      {
		fprintf(ofp, "# maximum DP matrix size set to:                      %.2f Mb\n", esl_opt_GetReal(go, "--mxsize"));
	}
	if (esl_opt_IsUsed(go, "--rdump"))       {
		fprintf(ofp, "# printing intermediate alns during aln refnment to:  %s\n", esl_opt_GetString(go, "--rdump"));
	}

	if (esl_opt_IsUsed(go, "--p7ml"))        {
		fprintf(ofp, "# filter HMM is ML HMM created from CM:               yes\n");
	}
	if (esl_opt_IsUsed(go, "--p7ere"))       {
		fprintf(ofp, "# filter HMM minimum rel entropy target:              %g bits\n", esl_opt_GetReal(go, "--p7ere"));
	}
	if (esl_opt_IsUsed(go, "--p7prior"))     {
		fprintf(ofp, "# read filter p7 HMM prior from:                      %s\n", esl_opt_GetString(go, "--p7prior"));
	}
	if (esl_opt_IsUsed(go, "--p7hprior"))    {
		fprintf(ofp, "# using HMMER3's default prior for filter p7 HMM:     yes\n");
	}
	if (esl_opt_IsUsed(go, "--p7hemit"))     {
		fprintf(ofp, "# using HMMER3's emission priors for filter p7 HMM:   yes\n");
	}

	if (esl_opt_IsUsed(go, "--EmN"))         {
		fprintf(ofp, "# seq number for filter HMM MSV Gumbel mu fit:        %d\n", esl_opt_GetInteger(go, "--EmN"));
	}
	if (esl_opt_IsUsed(go, "--EvN"))         {
		fprintf(ofp, "# seq number for filter HMM Vit Gumbel mu fit:        %d\n", esl_opt_GetInteger(go, "--EvN"));
	}
	if (esl_opt_IsUsed(go, "--ElfN"))        {
		fprintf(ofp, "# seq number for filter HMM local Fwd Gumbel mu fit:  %d\n", esl_opt_GetInteger(go, "--ElfN"));
	}
	if (esl_opt_IsUsed(go, "--EgfN"))        {
		fprintf(ofp, "# seq number for filter HMM glocal Fwd Gumbel mu fit: %d\n", esl_opt_GetInteger(go, "--EgfN"));
	}
	if (esl_opt_IsUsed(go, "--Elftp"))       {
		fprintf(ofp, "# tail fit frac for filter HMM local Fwd Gumbel fit:  %g\n", esl_opt_GetReal(go, "--Elftp"));
	}
	if (esl_opt_IsUsed(go, "--Egftp"))       {
		fprintf(ofp, "# tail fit frac for filter HMM glocal Fwd Gumbel fit: %g\n", esl_opt_GetReal(go, "--Egftp"));
	}
	if (esl_opt_IsUsed(go, "--Ereal"))       {
		fprintf(ofp, "# sample realistic, not iid seqs for HMM calibration: yes\n");
	}
	if (esl_opt_IsUsed(go, "--Enull3"))      {
		fprintf(ofp, "# use null3 correction during HMM calibration:        yes\n");
	}
	if (esl_opt_IsUsed(go, "--Ebias"))       {
		fprintf(ofp, "# use bias correction during HMM calibration:         yes\n");
	}
	if (esl_opt_IsUsed(go, "--Efitlam"))     {
		fprintf(ofp, "# fit lambda for HMM calibration:                     yes\n");
	}
	if (esl_opt_IsUsed(go, "--Elcmult"))     {
		fprintf(ofp, "# cm->clen multiplier for HMM local calib seq len:    %f\n", esl_opt_GetReal(go, "--Elcmult"));
	}
	if (esl_opt_IsUsed(go, "--Egcmult"))     {
		fprintf(ofp, "# cm->clen multiplier for HMM glocal calib seq len:   %f\n", esl_opt_GetReal(go, "--Egcmult"));
	}
	if (esl_opt_IsUsed(go, "--ElL"))         {
		fprintf(ofp, "# seq length for local filter HMM calibration:        %d\n", esl_opt_GetInteger(go, "--ElL"));
	}
	if (esl_opt_IsUsed(go, "--EgL"))         {
		fprintf(ofp, "# seq length for glocal filter HMM calibration:       %d\n", esl_opt_GetInteger(go, "--EgL"));
	}

	if (esl_opt_IsUsed(go, "--verbose"))     {
		fprintf(ofp, "# verbose mode:                                       on\n");
	}
	if (esl_opt_IsUsed(go, "--cfile"))       {
		fprintf(ofp, "# saving count vectors to file:                       %s\n", esl_opt_GetString(go, "--cfile"));
	}
	if (esl_opt_IsUsed(go, "--efile"))       {
		fprintf(ofp, "# saving emission score information to file:          %s\n", esl_opt_GetString(go, "--efile"));
	}
	if (esl_opt_IsUsed(go, "--tfile"))       {
		fprintf(ofp, "# saving individual sequence parsetrees to file:      %s\n", esl_opt_GetString(go, "--tfile"));
	}
	if (esl_opt_IsUsed(go, "--cmtbl"))       {
		fprintf(ofp, "# saving tabular description of CM topology to file:  %s\n", esl_opt_GetString(go, "--cmtbl"));
	}
	if (esl_opt_IsUsed(go, "--emap"))        {
		fprintf(ofp, "# saving consensus emit map to file:                  %s\n", esl_opt_GetString(go, "--emap"));
	}
	if (esl_opt_IsUsed(go, "--gtree"))       {
		fprintf(ofp, "# saving tree description of master tree to file:     %s\n", esl_opt_GetString(go, "--gtree"));
	}
	if (esl_opt_IsUsed(go, "--gtbl"))        {
		fprintf(ofp, "# saving tabular description of master tree to file:  %s\n", esl_opt_GetString(go, "--gtbl"));
	}

	if (esl_opt_IsUsed(go, "--ctarget"))     {
		fprintf(ofp, "# building >1 CMs from each MSA; target num CMs:      %d\n", esl_opt_GetInteger(go, "--ctarget"));
	}
	if (esl_opt_IsUsed(go, "--cmaxid"))      {
		fprintf(ofp, "# building >1 CMs from each MSA; max id b/t clusters: %g\n", esl_opt_GetReal(go, "--cmaxid"));
	}
	if (esl_opt_IsUsed(go, "--call"))        {
		fprintf(ofp, "# building a CM from each sequence in each MSA:       yes\n");
	}
	if (esl_opt_IsUsed(go, "--corig"))       {
		fprintf(ofp, "# appending original CM to cluster CMs:               yes\n");
	}
	if (esl_opt_IsUsed(go, "--cdump"))       {
		fprintf(ofp, "# writing training alns for each cluster CM to file:  %s\n", esl_opt_GetString(go, "--cdump"));
	}

	if (esl_opt_IsUsed(go, "--pbegin"))      {
		fprintf(ofp, "# aggregate probability of local begin:               %g\n", esl_opt_GetReal(go, "--pbegin"));
	}
	if (esl_opt_IsUsed(go, "--pend"))        {
		fprintf(ofp, "# aggregate probability of local end:                 %g\n", esl_opt_GetReal(go, "--pend"));
	}
	if (esl_opt_IsUsed(go, "--pebegin"))     {
		fprintf(ofp, "# set all local begins as equiprobable:               yes\n");
	}
	if (esl_opt_IsUsed(go, "--pfend"))       {
		fprintf(ofp, "# set all local end probs to:                         %g\n", esl_opt_GetReal(go, "--pfend"));
	}
...
init_cfg(const ESL_GETOPTS *go, struct cfg_s *cfg, char *errbuf)
{
	...
	if ((cfg->abc = esl_alphabet_Create(eslRNA)) == NULL) {
		ESL_FAIL(eslEMEM, errbuf, "Failed to create alphabet for sequence file");
	}
	...
	if ((cfg->cmoutfp = fopen(cfg->cmfile, "w")) == NULL) {
		ESL_FAIL(eslFAIL, errbuf, "Failed to open CM file %s for writing", cfg->cmfile);
	}
	...
	if (cfg->postmsafile) {
		...
		if (cfg->postmsafp == NULL) {
			ESL_FAIL(eslFAIL, errbuf, "Failed to MSA resave file %s for writing", cfg->postmsafile);
		}
	} else {
		cfg->postmsafp = NULL;
	}
	...
	if (esl_opt_GetString(go, "--null") != NULL) { /* read freqs from a file and overwrite bg->f */
		...
		if (status != eslOK) {
			cm_Fail("Failure creating the null model, code: %d", status);
		}
	}
	...
	if (esl_opt_GetBoolean(go, "--gibbs")) {
		...
		if (cfg->r == NULL) {
			ESL_FAIL(eslEINVAL, errbuf, "Failed to create random number generator: probably out of memory");
		}
	}
	...
	if (esl_opt_IsUsed(go, "--p7prior")) {
		FILE *pfp;
		if (cfg->fp7_bld->prior != NULL) {
			p7_prior_Destroy(cfg->fp7_bld->prior);
		}
		if ((pfp = fopen(esl_opt_GetString(go, "--p7prior"), "r")) == NULL) {
			cm_Fail("Failed to open p7 prior file %s\n", esl_opt_GetString(go, "--p7prior"));
		}
	...
	if (cfg->pri     == NULL) {
		ESL_FAIL(eslEINVAL, errbuf, "alphabet initialization failed");
	}
	if (cfg->null    == NULL) {
		ESL_FAIL(eslEINVAL, errbuf, "null model initialization failed");
	}
...
process_build_workunit(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, ESL_MSA *msa, CM_t **ret_cm, Parsetree_t **ret_mtr, Parsetree_t ***ret_msa_tr)
{
	...
	if ((status =  check_and_clean_msa(go, cfg, errbuf, msa))                                 != eslOK) {
		goto ERROR;
	}
	if ((status =  esl_msa_Checksum(msa, &checksum))                                       != eslOK) {
		ESL_FAIL(status, errbuf, "Failed to calculate checksum");
	}
	if ((status =  set_relative_weights(go, cfg, errbuf, msa))                                 != eslOK) {
		goto ERROR;
	}
	if ((status =  build_model(go, cfg, errbuf, TRUE, msa, &cm, ret_mtr, ret_msa_tr)) != eslOK) {
		goto ERROR;
	}
	...
	if ((status =  annotate(go, cfg, errbuf, msa, cm))                             != eslOK) {
		goto ERROR;
	}
	if ((status =  set_model_cutoffs(go, cfg, errbuf, msa, cm))                             != eslOK) {
		goto ERROR;
	}
	if ((status =  set_effective_seqnumber(go, cfg, errbuf, msa, cm, pri2use))                    != eslOK) {
		goto ERROR;
	}
	if ((status =  parameterize(go, cfg, errbuf, TRUE, cm, pri2use, msa->nseq))        != eslOK) {
		goto ERROR;
	}
	if ((status =  configure_model(go, cfg, errbuf, cm, 1))                               != eslOK) {
		goto ERROR;
	}
	if ((status =  set_consensus(go, cfg, errbuf, cm))                                  != eslOK) {
		goto ERROR;
	}
	/* if <pretend_cm_is_hmm> we'll set the CM's filter p7 HMM as its maximum likelihood HMM */
	if ((status =  build_and_calibrate_p7_filter(go, cfg, errbuf, msa, cm, pretend_cm_is_hmm))          != eslOK) {
		goto ERROR;
	}
           ...
           int *ret_niter)
{
	...
	/* check contract */
	if (input_msa       == NULL) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "refine_msa(), input_msa passed in as NULL");
	}
	if (input_msa->name == NULL) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "refine_msa(), input_msa must have a name");
	}
	if (init_cm         == NULL) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "refine_msa(), init_cm passed in as NULL");
	}
	if (ret_cm          == NULL) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "refine_msa(), ret_cm is NULL");
	}
	if (ret_mtr         == NULL) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "refine_msa(), ret_mtr is NULL");
	}
	if (ret_trA         == NULL) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "refine_msa(), ret_trA is NULL");
	}

	/* copy input MSA's name, we'll copy it to the MSA we create at each iteration */
	if ((status = esl_strdup(input_msa->name, -1, &(msa_name))) != eslOK) {
	...
	for (i = 0; i < nseq; i++) {
		...
		if (input_msa->ss && input_msa->ss[i]) {
			ESL_ALLOC(tmp_sqp->ss, sizeof(char) * tmp_sqp->salloc);
		}
	...
	for (i = 0; i < nseq; i++) {
		tmp_sqp = sq_block->list + i;
		if ((status = ParsetreeScore(init_cm, NULL, errbuf, input_msa_tr[i], tmp_sqp->dsq, FALSE, &(sc[i]), NULL, NULL, NULL, NULL)) != eslOK) {
			return status;
		}
	}
	...
	while (iter <= max_niter) {
		...
		/* 1. cm -> parsetrees */
		if ((status = DispatchSqBlockAlignment(cm, errbuf, sq_block, esl_opt_GetReal(go, "--mxsize"), NULL, NULL, cfg->r, &dataA)) != eslOK) {
			return status;
		}
		...
		if (iter > 1 || (! do_trunc)) {
			fprintf(cfg->ofp, "  %5d %13.2f %10.3f\n", iter, totscore, delta);
		} else                         {
			fprintf(cfg->ofp, "  %5d %13.2f %10s\n",   iter, totscore, "-");
		}
		if (delta <= threshold && delta > (-1. * eslSMALLX1)) {
			for (i = 0; i < nseq; i++) {
				cm_alndata_Destroy(dataA[i], FALSE);    /* FALSE: don't free sequences, ESL_SQ_BLOCK still points at them */
			}
			free(dataA);
		...
		for (i = 0; i < nseq; i++) {
			tmp_sqpA[i] = dataA[i]->sq;
			tmp_trpA[i] = dataA[i]->tr;
		}
		...
		if ((status = esl_strdup(msa_name, -1, &(msa->name))) != eslOK) {
			ESL_FAIL(status, errbuf, "refine_msa(), esl_strdup() call failed.");
		}
		...
		free(dataA);
		if ((status = process_build_workunit(go, cfg, errbuf, msa, &cm, &mtr, &trA))  != eslOK) {
			cm_Fail(errbuf);
		}
	}
	...
	if (cm->flags & CMH_LOCAL_BEGIN) { /* local begins on */
		if (cm->root_trans == NULL) {
			ESL_FAIL(eslFAIL, errbuf, "trying to globalize model but cm->root_trans is NULL...");
		}
		for (v = 0; v < cm->M; v++) {
			cm->begin[v] = 0.;
		}
		for (v = 0; v < cm->cnum[0]; v++) {
			cm->t[0][v] = cm->root_trans[v];
		}
...
output_result(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, int msaidx, int cmidx, ESL_MSA *msa, CM_t *cm, Parsetree_t *mtr, Parsetree_t **tr)
{
	...
	if (msaidx == 1 && cmidx == 1) {
		if ((status = print_column_headings(go, cfg, errbuf)) != eslOK) {
			return status;
		}
	}

	if ((status = cm_Validate(cm, 0.0001, errbuf)) != eslOK) {
		return status;
	}

	if ((status = cm_file_WriteASCII(cfg->cmoutfp, -1, cm)) != eslOK) {
		ESL_FAIL(status, errbuf, "CM save failed");
	}
	...
	if (cfg->tblfp != NULL) {
		PrintCM(cfg->tblfp, cm);    /* tabular description of CM topology */
	}
	/* emit map */
	if (cfg->efp != NULL) {
		CMEmitMap_t *emap;
	...
	if (cfg->gtblfp != NULL) {
		PrintParsetree(cfg->gtblfp, mtr);
	}
	/* save tree description of guide tree topology, if nec */
	if (cfg->gfp    != NULL) {
		MasterTraceDisplay(cfg->gfp, mtr, cm);
	}
	/* save base pair info, if nec */
	if (cfg->escfp   != NULL)
		if ((status = dump_emission_info(cfg->escfp, cm, errbuf)) != eslOK) {
			return status;
		}
	...
	if (cfg->tfp != NULL) {
		for (i = 0; i < msa->nseq; i++) {
			...
			if ((status = ParsetreeScore(cm, NULL, errbuf, tr[i], msa->ax[i], FALSE, &sc, &struct_sc, NULL, NULL, NULL)) != eslOK) {
				return status;
			}
	...
	if (cfg->postmsafp != NULL && msa != NULL) {
		...
		for (i = 0; i < msa->nseq; i++) {
			if ((status = esl_sq_FetchFromMSA(msa, i, &(sq[i]))) != eslOK) {
				ESL_FAIL(status, errbuf, "problem creating -O output alignment, probably out of memory");
			}
		}
		...
		for (i = 0; i < msa->nseq; i++) {
			...
			*/
			...
			for (x = 0; x < tr[i]->n; x++) {
				if (tr[i]->emitl[x] != -1) {
					tr[i]->emitl[x] = a2ua_map[tr[i]->emitl[x]];
				}
				if (tr[i]->emitr[x] != -1) {
					tr[i]->emitr[x] = a2ua_map[tr[i]->emitr[x]];
				}
			}
		}
		if ((status = Parsetrees2Alignment(cm, errbuf, cm->abc, sq, msa->wgt, tr, NULL, msa->nseq, NULL, NULL, TRUE, FALSE, &omsa)) != eslOK) {
			return status;
		}
...
check_and_clean_msa(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, ESL_MSA *msa)
{
	...
	if (esl_opt_GetBoolean(go, "--hand") && msa->rf == NULL) {
		ESL_FAIL(eslFAIL, errbuf, "--hand used, but alignment #%d has no reference coord annotation", cfg->nali);
	}
	...
	if (msa->ss_cons == NULL) {
		ESL_FAIL(eslFAIL, errbuf, "Alignment #%d has no consensus structure annotation, and --noss not used.", cfg->nali);
	}
	if (! clean_cs(msa->ss_cons, msa->alen, (! cfg->be_verbose))) {
		ESL_FAIL(eslFAIL, errbuf, "Failed to parse consensus structure annotation in alignment #%d", cfg->nali);
	}

	if (esl_opt_IsOn(go, "--rsearch")) {
		if (msa->nseq != 1) {
			ESL_FAIL(eslEINCOMPAT, errbuf, "with --rsearch option, all of the input alignments must have exactly 1 sequence");
		}
	...
	if (msa->name == NULL) {
		ESL_FAIL(eslEINCONCEIVABLE, errbuf, "MSA is nameless, (we thought we named it...) shouldn't happen");
	}
...
set_relative_weights(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, ESL_MSA *msa)
{
	...
	else if (esl_opt_GetBoolean(go, "--wpb")) {
...
build_model(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, int do_print, ESL_MSA *msa, CM_t **ret_cm, Parsetree_t **ret_mtr, Parsetree_t ***ret_msa_tr)
{
	...
	if ((status = HandModelmaker(msa, errbuf, use_rf,
	                             ...
	                             use_wts, (1. - esl_opt_GetReal(go, "--symfrac")), &cm, &mtr)) != eslOK) {
		return status;
	}
	...
	if (esl_opt_GetString(go, "--rsearch") != NULL) {
		cm->flags |= CM_RSEARCHEMIT;
	}

	/* rebalance CM */
	if (! esl_opt_GetBoolean(go, "--nobalance")) {
		CM_t *new = NULL;
		if ((status = CMRebalance(cm, errbuf, &new)) != eslOK) {
			return status;
		}
		FreeCM(cm);
		cm = new;
	...
	for (idx = 0; idx < msa->nseq; idx++) {
		if ((status = Transmogrify(cm, errbuf, mtr, msa->ax[idx], used_el, msa->alen, &(tr[idx]))) != eslOK) {
			return status;
		}
		if (pretend_cm_is_hmm) {
			if ((status = cm_parsetree_Doctor(cm, errbuf, tr[idx], NULL, NULL)) != eslOK) {
				return status;
			}
		}
	...
	if ((! esl_opt_GetBoolean(go, "--v1p0")) && (! esl_opt_GetBoolean(go, "--iflank"))) {
		if ((status = cm_zero_flanking_insert_counts(cm, errbuf)) != eslOK) {
			return status;
		}
	}
	 ...
	 */
	if (cfg->cfp != NULL) {
		if ((status = print_countvectors(cfg, errbuf, cm)) != eslOK) {
			goto ERROR;
		}
	}
...
annotate(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, ESL_MSA *msa, CM_t *cm)
{
	...
	if ((status = cm_SetName(cm, msa->name))                    != eslOK) {
		ESL_XFAIL(status, errbuf, "Unable to set name for CM");
	}
	if ((status = cm_SetAccession(cm, msa->acc))                     != eslOK) {
		ESL_XFAIL(status, errbuf, "Failed to record MSA accession");
	}
	if ((status = cm_SetDescription(cm, msa->desc))                    != eslOK) {
		ESL_XFAIL(status, errbuf, "Failed to record MSA description");
	}
	if ((status = cm_AppendComlog(cm, go->argc, go->argv, FALSE, 0)) != eslOK) {
		ESL_XFAIL(status, errbuf, "Failed to record command log");
	}
	if ((status = cm_SetCtime(cm))                               != eslOK) {
		ESL_XFAIL(status, errbuf, "Failed to record timestamp");
	}
		...
		*/
		if (esl_opt_IsOn(go, "--ehmmre")) {
			...
			if (hmm_re < hmm_etarget) {
				...
				if (status == eslEMEM) {
					ESL_FAIL(status, errbuf, "memory allocation failed");
				} else if (status != eslOK) {
					ESL_FAIL(status, errbuf, "internal failure in entropy weighting algorithm");
				}
...
parameterize(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, int do_print, CM_t *cm, const Prior_t *prior, float msa_nseq)
{
	...
	if (! esl_opt_GetBoolean(go, "--iins")) {
		...
		if ((status = flatten_insert_emissions(cm)) != eslOK) {
			ESL_FAIL(status, errbuf, "flatten_insert_emissions() failed");
		}
...
configure_model(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, CM_t *cm, int iter)
{
	...
	if (esl_opt_IsUsed(go, "--refine")) {
		...
		if (esl_opt_GetBoolean(go, "--gibbs"))     {
			cm->align_opts |= CM_ALIGN_SAMPLE;
		} else if (esl_opt_GetBoolean(go, "--cyk"))       {
		...
		if (esl_opt_GetBoolean(go, "--fins")) {
			cm->align_opts  |= CM_ALIGN_FLUSHINSERTS;
		}
		...
		if (esl_opt_GetBoolean(go, "--nonbanded")) {
			cm->config_opts |= CM_CONFIG_NONBANDEDMX;
		}
...
set_consensus(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, CM_t *cm)
{
	...
	if (!(cm->flags & CMH_BITS)) {
		ESL_XFAIL(eslEINVAL, errbuf, "Trying to set cm->consensus before bit scores are valid");
	}

	if ((status = cm_SetConsensus(cm, cm->cmcons, NULL)) != eslOK) {
		ESL_XFAIL(status, errbuf, "Failed to calculate consensus sequence");
	}
...
build_and_calibrate_p7_filter(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, ESL_MSA *msa, CM_t *cm, int use_mlp7_as_filter)
{
	...
	if (use_mlp7_as_filter) {
		...
		if (!(cm->flags & CMH_MAP)) {
			cm_Fail("Unable to create additional p7 HMM, CM has no map, this shouldn't happen");
		}
		/* init to all inserts, then set match states based on cm->map */
		for (apos = 0; apos <  amsa->alen; apos++) {
			amsa->rf[apos] = '.';
		}
		for (cpos = 1; cpos <= cm->clen;   cpos++) {
			amsa->rf[cm->map[cpos] - 1] = 'x';    /* note off by one */
		}
		...
		if ((status = p7_Builder(cfg->fp7_bld, amsa, cfg->fp7_bg, &fhmm,
			...
			strcpy(errbuf, cfg->fp7_bld->errbuf);
			return status;
		}
		...
		if (!(fhmm->flags & p7H_CS)) {
			cm_Fail("additional p7 HMM unexpectedly does not have consensus structure annotation");
		}
		fhmm->cs[0] = ' ';
		...
		if (! esl_opt_GetBoolean(go, "--p7hemit")) {
			 ...
			 * fhmm we just built.
			 */
			if ((status =  build_model(go, cfg, errbuf, FALSE, msa, &acm, NULL, NULL)) != eslOK) {
				return status;
			}
			...
			if ((status = parameterize(go, cfg, errbuf, FALSE, acm, cfg->pri, msa->nseq)) != eslOK) {
				return status;
			}
			 ...
			 */
			if ((status = configure_model(go, cfg, errbuf, acm, 2)) != eslOK) {
				return status;
			}
			...
			for (k = 1; k <= fhmm->M; k++) {
				esl_vec_FCopy(acm->mlp7->mat[k], fhmm->abc->K, fhmm->mat[k]);
			}
			for (k = 1; k <= fhmm->M; k++) {
				esl_vec_FNorm(fhmm->mat[k], fhmm->abc->K);
			}
			/* special case */
			...
			for (k = 0; k <= fhmm->M; k++) {
				esl_vec_FCopy(acm->mlp7->ins[k], fhmm->abc->K, fhmm->ins[k]);
			}
			for (k = 0; k <= fhmm->M; k++) {
				esl_vec_FNorm(fhmm->ins[k], fhmm->abc->K);
			}
			/* reset HMM composition */
			if ((status = p7_hmm_SetComposition(fhmm)) != eslOK) {
				goto ERROR;
			}
	...
	if ((status = cm_p7_Calibrate(fhmm, errbuf,
	                              ...
	                              &agfmu, &agflambda))
	        != eslOK) {
		ESL_FAIL(status, errbuf, "Error calibrating additional p7 HMM");
	}

	if ((status = cm_p7_hmm_SetConsensus(fhmm)) != eslOK) {
		ESL_FAIL(status, errbuf, "Unable to set the HMM filter consensus annotation");
	}
	if ((status = cm_SetFilterHMM(cm, fhmm, agfmu, agflambda))       != eslOK) {
		ESL_FAIL(status, errbuf, "Unable to set the HMM filter for the CM");
	}
	if ((status = p7_hmm_AppendComlog(cm->fp7, go->argc, go->argv)) != eslOK) {
		ESL_FAIL(status, errbuf, "Failed to record command log for filter HMM");
	}
...
set_target_relent(const ESL_GETOPTS *go, const ESL_ALPHABET *abc, int clen, int nbps)
{
	...
	if (esl_opt_IsOn(go, "--ere")) {
		...
		if (abc->type != eslRNA) {
			cm_Fail("ERROR, alphabet not RNA, user needs to specify target entropy with --ere");
		}
...
version_1p0_default_target_relent(const ESL_ALPHABET *abc, int clen, double eX)
{
	...
	case eslRNA:
		...
		break;
	default:
		cm_Fail("ERROR in default_target_relent(), alphabet not RNA!\n");
	}
	return etarget;
}
...
set_msa_name(const ESL_GETOPTS *go, struct cfg_s *cfg, char *errbuf, ESL_MSA *msa)
{
	...
	if (cfg->nali == 1) { /* first (only?) MSA in file: */
		if (esl_opt_IsUsed(go, "-n")) {
			if ((status = esl_msa_SetName(msa, esl_opt_GetString(go, "-n"), -1)) != eslOK) {
				return status;
			}
		} else if (msa->name != NULL) {
			cfg->nnamed++;
		} else if (cfg->afp->bf->filename) {
			if ((status = esl_FileTail(cfg->afp->bf->filename, TRUE, &name)) != eslOK) {
				return status;    /* TRUE=nosuffix */
			}
			if ((status = esl_msa_SetName(msa, name, -1))                    != eslOK) {
				return status;
			}
			free(name);
		} else {
			ESL_FAIL(eslEINVAL, errbuf, "Failed to set model name: msa has no name, no msa filename, and no -n");
		}
	} else {
		if (esl_opt_IsUsed(go, "-n")) {
			ESL_FAIL(eslEINVAL, errbuf, "Oops. Wait. You can't use -n with an alignment database.");
		} else if (msa->name != NULL) {
			cfg->nnamed++;
		} else {
			ESL_FAIL(eslEINVAL, errbuf, "Oops. Wait. I need name annotation on each alignment in a multi MSA file; failed on #%d", cfg->nali);
		}
		...
		if (cfg->nnamed != cfg->nali) {
			ESL_FAIL(eslEINVAL, errbuf, "Oops. Wait. I need name annotation on each alignment in a multi MSA file; first MSA didn't have one");
		}
	}
	return eslOK;
}
...
print_countvectors(const struct cfg_s *cfg, char *errbuf, CM_t *cm)
{
	...
	if (cfg->cfp == NULL) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "save_countvectors(), but cfg->cfp is NULL, shouldn't happen.");
	}
...
model_trace_info_dump(FILE *ofp, CM_t *cm, Parsetree_t *tr, char *aseq)
{
	...
	for (i = 0; i < strlen(aseq); i++)
		if (! esl_abc_CIsGap(cm->abc, aseq[i])) {
			map[i] = a++;
		} else {
			map[i] = -1;
		}
...
convert_parsetrees_to_unaln_coords(Parsetree_t **tr, ESL_MSA *msa)
{
	...
	if (!(msa->flags & eslMSA_DIGITAL)) {
		cm_Fail("get_unaln_seqs_from_msa() msa is not digitized.\n");
	}
          ...
          int do_orig, int *ret_num_msa, ESL_MSA ***ret_cmsa, char *errbuf)
{
	...
	/* Contract check */
	if ((do_all + do_nc + do_mindiff) != 1) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "MSADivide() exactly 1 of do_all, do_nc, do_mindiff must be TRUE.");
	}
	if (do_nc && target_nc == 0) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "MSADivide() target_nc is 0 but do_nc is TRUE!");
	}
	if (do_mindiff && mindiff <= 0.) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "MSADivide() mindiff is <= 0. but do_mindiff is TRUE!");
	}
	if (do_mindiff && target_nc != 0) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "MSADivide() do_mindiff is TRUE, but target_nc != 0");
	}
	/* mmsa must be digital */
	if (!(mmsa->flags & eslMSA_DIGITAL)) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "MSADivide() MSA is not digital.");
	}
	...
	if (do_all) {
		...
		if ((status = esl_dst_XDiffMx(mmsa->abc, mmsa->ax, mmsa->nseq, &D)) != eslOK) {
			ESL_FAIL(status, errbuf, "esl_dst_XDiffMx() error, status: %d", status);
		}
		if ((status = esl_tree_SingleLinkage(D, &T)) != eslOK) {
			ESL_FAIL(status, errbuf, "esl_tree_SingleLinkage() error, status: %d", status);
		}
		if ((status = esl_tree_SetTaxaParents(T)) != eslOK) {
			ESL_FAIL(status, errbuf, "esl_tree_SetTaxaParentse() error, status: %d", status);
		}
		/*esl_tree_WriteNewick(cfg->ofp, T);*/
		if ((status = esl_tree_Validate(T, errbuf) != eslOK)) {
			return status;
		}
		...
		if (do_mindiff) { /* Mode 2 */
			 ...
			 * from each other. */
			if ((status = select_node(T, diff, mindiff, &clust, &nc, &best, errbuf)) != eslOK) {
				return status;
			}
			 ...
			 * to nearest 0.001. */
			if (target_nc > (T->N)) {
				target_nc = T->N;    /* max num clusters is num seqs */
			}
			if ((status = find_mindiff(T, diff, target_nc, &clust, &nc, &mindiff, errbuf)) != eslOK) {
				return status;
			}
			printf("# Alignment split into %d clusters; each will be used to train a CM.\n", nc);
			printf("# Maximum identity b/t any 2 seqs in different clusters: %.2f\n", (1. - mindiff));
	...
	for (m = 0; m <= nc; m++) {
		...
		if (m < nc) {
			esl_vec_ISet(useme[m], mmsa->nseq, FALSE);
		} else {
			esl_vec_ISet(useme[m], mmsa->nseq, TRUE);    /* keep all seqs in cmsa[nc]*/
		}
	}
	...
	for (m = 0; m < nc; m++) {
		if ((status = esl_msa_SequenceSubset(mmsa, useme[m], &(cmsa[m]))) != eslOK) {
			ESL_FAIL(status, errbuf, "MSADivide(), esl_msa_SequenceSubset error, status: %d.", status);
		}
		/* rename the MSA it by adding ".<m+1>" */
		if (cmsa[m]->name == NULL) {
			ESL_FAIL(eslEINCONCEIVABLE, errbuf, "MSADivide(), an msa's name is NULL, shouldn't happen.");
		}
		ndigits  = strlen(cmsa[m]->name);
		ndigits += sprintf(buffer, ".%d", (m + 1));
		ESL_RALLOC(cmsa[m]->name, tmp, sizeof(char) * (ndigits + 1));
		if ((status = esl_strcat(&cmsa[m]->name, -1, buffer, (ndigits + 1))) != eslOK) {
			goto ERROR;
		}
		if ((status = esl_strchop(cmsa[m]->name, ndigits)) != eslOK) {
			goto ERROR;
		}
		free(useme[m]);
	}
	if (do_orig) {
		if ((status = esl_msa_SequenceSubset(mmsa, useme[nc], &(cmsa[nc]))) != eslOK) {
			ESL_FAIL(status, errbuf, "MSADivide(), esl_msa_SequenceSubset error, status: %d.", status);
		}
	}
	free(useme[nc]);
...
select_node(ESL_TREE *T, double *diff, double mindiff, int **ret_clust, int *ret_nc, int *ret_best, char *errbuf)
{
	...
	if (T->cladesize == NULL)
		if ((status = esl_tree_SetCladesizes(T)) != eslOK) {
			ESL_FAIL(status, errbuf, "select_node(), esl_tree_SetCladeSizes error, status: %d.", status);
		}

	ESL_ALLOC(clust, (sizeof(int) * T->N));
	esl_vec_ISet(clust, T->N, 0);

	if ((ns1 = esl_stack_ICreate()) == NULL) {
		ESL_FAIL(status, errbuf, "select_node(), failed to create a stack, probably out of memory, status: %d.", status);
	}
	if ((ns2 = esl_stack_ICreate()) == NULL) {
		ESL_FAIL(status, errbuf, "select_node(), failed to create a stack, probably out of memory, status: %d.", status);
	}

	/* push root on stack to start */
	if ((status = esl_stack_IPush(ns1, 0)) != eslOK) {
		ESL_FAIL(status, errbuf, "select_node(), failed to push onto a stack, probably out of memory, status: %d.", status);
	}
	maxsize  = 0;
	best     = 0;
	c        = 0;
		...
		if ((n == 0 || diff[T->parent[n]] > mindiff) &&
			...
			while (esl_stack_IPop(ns2, &np) != eslEOD) {
				...
				if (T->left[np]  <= 0) {
					...
					if ((status = esl_stack_IPush(ns2, T->left[np])) != eslOK) {
						ESL_FAIL(status, errbuf, "select_node(), failed to push onto a stack, probably out of memory, status: %d.", status);
					}
				}
				if (T->right[np] <= 0) {
					clust[(-1 * T->right[np])]  = c;
				} else {
					if ((status = esl_stack_IPush(ns2, T->right[np])) != eslOK) {
						ESL_FAIL(status, errbuf, "select_node(), failed to push onto a stack, probably out of memory, status: %d.", status);
					}
				}
			}
			...
			if (T->left[n]  <= 0) {
				clust[(-1 * T->left[n])]  = c++;    /* single seq with its own cluster */
			} else {
				if ((status = esl_stack_IPush(ns1, T->left[n])) != eslOK) {
					ESL_FAIL(status, errbuf, "select_node(), failed to push onto a stack, probably out of memory, status: %d.", status);
				}
			}
			if (T->right[n] <= 0) {
				clust[(-1 * T->right[n])] = c++;    /* single seq with its own cluster */
			} else {
				if ((status = esl_stack_IPush(ns1, T->right[n])) != eslOK) {
					ESL_FAIL(status, errbuf, "select_node(), failed to push onto a stack, probably out of memory, status: %d.", status);
				}
			}
		}
...
find_mindiff(ESL_TREE *T, double *diff, int target_nc, int **ret_clust, int *ret_nc, float *ret_mindiff, char *errbuf)
{
	...
	/* Contract check */
	if (target_nc > T->N) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "find_mindiff(), desired number of clusters is greater than number of seqs in the tree");
	}
	...
	while (keep_going) {
		...
		if ((status = select_node(T, diff, mindiff, &clust, &curr_nc, &curr_best, errbuf)) != eslOK) {
			return status;
		}
		if (curr_nc < target_nc) {
			...
			if ((fabs(high - 0.) < thresh) && (fabs(low - 0.) < thresh)) {
				keep_going = FALSE;
			}
			...
			if (fabs(high - low) < thresh) {
				keep_going = FALSE;    /* stop, high and low have converged */
			}
	...
	if (curr_nc != target_nc) {
		...
		if (high_nc < target_nc) {
			...
			while (high_nc > target_nc) {
				high += thresh;
				if (high > 1.0) {
					ESL_FAIL(eslEINCONCEIVABLE, errbuf, "find_mindiff(), mindiff has risen above 1.0");
				}
				mindiff = high;
				if ((status = select_node(T, diff, mindiff, &clust, &curr_nc, &curr_best, errbuf)) != eslOK) {
					return status;
...
flatten_insert_emissions(CM_t *cm)
{
	...
	/* Contract check */
	if (cm->abc  == NULL) {
		cm_Fail("flatten_insert_emissions(), cm->abc is NULL.\n");
	}
	if (cm->null == NULL) {
		cm_Fail("flatten_insert_emissions(), cm->null is NULL.\n");
	}
...
p7_prior_Read(FILE *fp)
{
	...
	if (esl_mixdchlet_Read(efp, &(pri->tm)) != eslOK) {
		goto ERROR;
	}
	if (esl_mixdchlet_Read(efp, &(pri->ti)) != eslOK) {
		goto ERROR;
	}
	if (esl_mixdchlet_Read(efp, &(pri->td)) != eslOK) {
		goto ERROR;
	}

	/* Emission section: match emissions, then insert emissions */
	if (esl_mixdchlet_Read(efp, &(pri->em)) != eslOK) {
		goto ERROR;
	}
	if (esl_mixdchlet_Read(efp, &(pri->ei)) != eslOK) {
		goto ERROR;
	}
...
P7_PRIOR *cm_p7_prior_CreateNucleic(void)
{
	...
	if (pri->tm == NULL || pri->ti == NULL || pri->td == NULL || pri->em == NULL || pri->ei == NULL) {
		goto ERROR;
	}
