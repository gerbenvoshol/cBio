...
main(int argc, char **argv)
{
	...
	if (cfg.my_rank == 0 && (! esl_opt_IsUsed(go, "--forecast")) && (! esl_opt_IsUsed(go, "--memreq"))) { /* master, serial or mpi */
		...
		if (esl_FileExists(cfg.tmpfile)) {
			cm_Fail("Ouch. Temporary file %s appeared during the run.", cfg.tmpfile);
		}
		if ((outfp = fopen(cfg.tmpfile, cfg.mode)) == NULL) {
			cm_Fail("Ouch. Temporary file %s couldn't be opened for writing.", cfg.tmpfile);
		}

		for (cmi = 0; cmi < cfg.ncm; cmi++) {
			if ((status = cm_file_Read(cfg.cmfp, TRUE, &(cfg.abc), &cm)) != eslOK) {
				cm_Fail("Ran out of CMs too early in pass 2");
			}
			if (cm == NULL) {
				cm_Fail("CM file %s was corrupted? Parse failed in pass 2", cfg.cmfile);
			}
			...
			if (cfg.cmfp->is_binary) {
				if ((status = cm_file_WriteBinary(outfp, -1, cm, NULL)) != eslOK) {
					ESL_FAIL(status, errbuf, "binary CM save failed");
				}
			} else {
				if ((status = cm_file_WriteASCII(outfp, -1, cm)) != eslOK) {
		...
		if (fclose(outfp)   != 0) {
			cm_Fail("system error during rewrite of CM file");
		}
		if (sigemptyset(&blocksigs) != 0) {
			cm_Fail("system error during rewrite of CM file.");
		}
		if (sigaddset(&blocksigs, SIGINT) != 0) {
			cm_Fail("system error during rewrite of CM file.");
		}
		if (sigprocmask(SIG_BLOCK, &blocksigs, NULL) != 0) {
			cm_Fail("system error during rewrite of CM file.");
		}
		if (remove(cfg.cmfile) != 0) {
			cm_Fail("system error during rewrite of CM file.");
		}
		if (rename(cfg.tmpfile, cfg.cmfile) != 0) {
			cm_Fail("system error during rewrite of CM file.");
		}
		if (sigprocmask(SIG_UNBLOCK, &blocksigs, NULL) != 0) {
			cm_Fail("system error during rewrite of CM file.");
		}
		free(cfg.tmpfile);
		cfg.tmpfile = NULL;
		...
		if (cfg.hfp || cfg.sfp || cfg.qfp || cfg.ffp || cfg.xfp) {
			printf("#\n");
		}
	...
	if (cfg.r_est   != NULL) {
		esl_randomness_Destroy(cfg.r_est);
	}
	if (cfg.tmpfile != NULL) {
		free(cfg.tmpfile);
	}
...
serial_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if (esl_opt_IsUsed(go, "--forecast")) {
		ncpus = 0;
	} else if (esl_opt_IsUsed(go, "--memreq")) {
		ncpus = 0;
	} else if (esl_opt_IsOn(go, "--cpu")) {
	...
	if (qhstatus == eslOK) {
		...
		if ((status = CreateGenomicHMM(cfg->abc, errbuf, &(cfg->ghmm_sA), &(cfg->ghmm_tAA), &(cfg->ghmm_eAA), &cfg->ghmm_nstates)) != eslOK) {
			cm_Fail("unable to create generative HMM\n%s", errbuf);
		}
		if ((status = set_dnull(cfg, cm, errbuf)) != eslOK) {
			cm_Fail("unable to create set_dnull\n%s\n", errbuf);
		}
		...
		for (i = 0; i < ncpus * 2; i++) {
			init_sqA[i] = NULL;
			if ((init_sqA[i] = esl_sq_CreateDigital(cfg->abc)) == NULL) {
				cm_Fail("Failed to allocate a sequence");
			}
			if ((status = esl_workqueue_Init(queue, init_sqA[i]))  != eslOK) {
	...
	while (qhstatus == eslOK) {
		cfg->ncm++;
		cmi = cfg->ncm - 1;
		if (cmi == 0 && (! esl_opt_IsUsed(go, "--memreq"))) {
			print_calibration_column_headings(go, cfg, errbuf, cm, available_ncpus);
		}
		if ((status = expand_exp_and_name_arrays(cfg))                              != eslOK) {
			cm_Fail("out of memory");
		}
		if ((status = esl_strdup(cm->name, -1, &(cfg->namesA[cmi])))                != eslOK) {
			cm_Fail("unable to duplicate CM name");
		}

		/* clone the non-configured CM we just read, we'll come back to it when we switch from global to local */
		if ((status = cm_Clone(cm, errbuf, &nc_cm)) != eslOK) {
			cm_Fail("unable to clone CM");
		}
		if ((status = initialize_cm(go, cfg, errbuf, cm, FALSE))                    != eslOK) {
			cm_Fail(errbuf);
		}
		...
		if (esl_opt_IsUsed(go, "--memreq")) { /* special case: if --memreq, print required memory and skip to reading next CM */
			...
			if ((status = initialize_stats(go, cfg, errbuf)) != eslOK) {
				cm_Fail(errbuf);
			}
			if (! esl_opt_GetBoolean(go, "--noforecast")) {
				if ((status = forecast_time(go, cfg, errbuf, cm, ncpus, available_ncpus, &psec, &ins_v_cyk)) != eslOK) {
					cm_Fail(errbuf);
				}
			} else {
				psec = 0.;
			...
			if ((status = generate_sequences(go, cfg, errbuf, cm, &sq_block))           != eslOK) {
				cm_Fail(errbuf);
			}
			...
			if (! esl_opt_IsUsed(go, "--forecast")) {
				...
				for (exp_mode = 0; exp_mode < EXP_NMODES; exp_mode++) {
					...
					if (exp_mode == 0) {
						for (i = 0; i < infocnt; i++) {
							if ((status = cm_Clone(cm, errbuf, &(info[i].cm))) != eslOK) {
								cm_Fail(errbuf);
							}
						}
					}
					...
					if (exp_mode > 0 && (! ExpModeIsLocal(exp_mode - 1)) && ExpModeIsLocal(exp_mode)) {
						...
						FreeCM(cm);
						cm = nc_cm;
						if ((status = initialize_cm(go, cfg, errbuf, cm, TRUE)) != eslOK) {
							cm_Fail(errbuf);
						}
						for (i = 0; i < infocnt; i++) {
							if (info[i].cm  != NULL) {
					...
					if (ExpModeIsInside(exp_mode)) {
						...
						for (i = 0; i < infocnt; i++) {
							info[i].cm->search_opts |= CM_SEARCH_INSIDE;
						}
					...
					for (i = 0; i < infocnt; ++i) {
						if (info[i].nhits > 0) {
							...
							for (h = 0; h < info[i].nhits; h++) {
								merged_scA[(merged_nhits + h)] = info[i].scA[h];
							}
					...
					if ((status = fit_histogram(go, cfg, errbuf, merged_scA, merged_nhits, exp_mode, &tmp_mu, &tmp_lambda, &tmp_nrandhits, &tmp_tailp)) != eslOK) {
						cm_Fail(errbuf);
					}
	...
	if (esl_opt_IsUsed(go, "--memreq")) {
		print_required_memory_tail(available_ncpus);
	} else if (cfg->ncm > 1) {
		print_total_time(go, total_asec, total_psec);
	}
...
serial_loop(WORKER_INFO *info, char *errbuf, ESL_SQ_BLOCK *sq_block)
{
	...
	for (i = 0; i < sq_block->count; i++) {
		if ((status = process_search_workunit(info->cm, errbuf, sq_block->list[i].dsq, sq_block->list[i].L, info->cutoff, &th)) != eslOK) {
			cm_Fail(errbuf);
		}
		...
		if (th->N > 0) {
			...
			for (h = 0; h < th->N; h++) {
				info->scA[(info->nhits + h)] = th->unsrt[h].score;
			}
			info->nhits += th->N;
		}
...
pipeline_thread(void *arg)
{
	...
	sq = (ESL_SQ *) new_sq;
	while (sq->L != -1) {
		if ((status = process_search_workunit(info->cm, errbuf, sq->dsq, sq->L, info->cutoff, &th)) != eslOK) {
			cm_Fail(errbuf);
		}
		/* append copy of hit scores to scA */
		if (th->N > 0) {
...
mpi_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if (qhstatus == eslOK) {
		...
		if ((status = set_dnull(cfg, cm, errbuf)) != eslOK) {
			mpi_failure("unable to create set_dnull\n%s\n", errbuf);
		}
	}
	...
	while (qhstatus == eslOK) {
		/* clone the non-configured CM we just read, we'll come back to it when we switch from global to local */
		if ((status = cm_Clone(cm, errbuf, &nc_cm)) != eslOK) {
			mpi_failure("unable to clone CM");
		}
		...
		if ((status = esl_strdup(cm->name, -1, &(cfg->namesA[cmi])))                       != eslOK) {
			mpi_failure("unable to duplicate CM name");
		}
		if ((status = initialize_cm(go, cfg, errbuf, cm, FALSE))                           != eslOK) {
			mpi_failure(errbuf);
		}
		...
		if (cmi == 0) {
			print_calibration_column_headings(go, cfg, errbuf, cm, 1);    /* 1 is irrelevant here, since --forecast can't have been enabled */
		}
		if (! esl_opt_GetBoolean(go, "--noforecast")) {
			if ((status = forecast_time(go, cfg, errbuf, cm, cfg->nproc - 1, 1, &psec, &ins_v_cyk)) != eslOK) {
				mpi_failure(errbuf);
			}
		} else {
			psec = 0.;
		...
		for (dest = 1; dest < cfg->nproc; dest++) {
			if (MPI_Send(&status, 1, MPI_INT, dest, INFERNAL_READY_TAG, MPI_COMM_WORLD) != 0) {
				mpi_failure("mpi send failed");
			}
#if DEBUG_MPI
		...
		for (exp_mode = 0; exp_mode < EXP_NMODES; exp_mode++) {
			...
			for (i = 0; i < cfg->N; i++) {
				...
				if (MPI_Send(&i, 1, MPI_INT, dest, INFERNAL_SEQIDX_TAG, MPI_COMM_WORLD) != 0) {
					mpi_failure("mpi send failed");
				}
#if DEBUGMPI
			...
			for (dest = 1; dest < cfg->nproc; ++dest) {
				...
				if (MPI_Probe(dest, MPI_ANY_TAG, MPI_COMM_WORLD, &mpistatus) != 0) {
					mpi_failure("MPI error %d receiving message from %d\n", mpistatus.MPI_SOURCE);
				}
				MPI_Get_count(&mpistatus, MPI_PACKED, &buf_size);
				if (mpibuf == NULL || buf_size > mpibuf_size) {
				...
				if (mpistatus.MPI_TAG != INFERNAL_SCORES_TAG) {
					mpi_failure("Unexpected tag %d from %d\n", mpistatus.MPI_TAG, dest);
				}
				pos = 0;
				if (MPI_Unpack(mpibuf, buf_size, &pos, &nhits, 1, MPI_LONG_LONG_INT, MPI_COMM_WORLD) != 0) {
					ESL_XEXCEPTION(eslESYS, "unpack failed");
				}
				if (nhits > 0) {
					if (merged_nhits == 0) {
						ESL_ALLOC(merged_scA, sizeof(float) * (merged_nhits + nhits));
					} else {
						ESL_REALLOC(merged_scA, sizeof(float) * (merged_nhits + nhits));
					}
					for (h = 0; h < nhits; h++) {
						if (MPI_Unpack(mpibuf, buf_size, &pos, &(merged_scA[merged_nhits + h]), 1, MPI_FLOAT, MPI_COMM_WORLD) != 0) {
							ESL_XEXCEPTION(eslESYS, "unpack failed");
						}
					}
					merged_nhits += nhits;
...
mpi_worker(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if (qhstatus == eslOK) {
		/* One-time initializations after alphabet <abc> becomes known */
		if ((status = CreateGenomicHMM(cfg->abc, errbuf, &(cfg->ghmm_sA), &(cfg->ghmm_tAA), &(cfg->ghmm_eAA), &cfg->ghmm_nstates)) != eslOK) {
			mpi_failure("unable to create generative HMM\n%s", errbuf);
		}
		if ((status = set_dnull(cfg, cm, errbuf)) != eslOK) {
			mpi_failure("unable to create set_dnull\n%s\n", errbuf);
		}
	...
	while (qhstatus == eslOK) {
...
#endif
		if ((status = MPI_Recv(&status, 1, MPI_INT, 0, INFERNAL_READY_TAG, MPI_COMM_WORLD, &mpistatus)) != 0) {
			mpi_failure("failed to receive ready signal");
		}
		...
		for (exp_mode = 0; exp_mode < EXP_NMODES; exp_mode++) {
			...
			if ((status = MPI_Recv(&i, 1, MPI_INT, 0, INFERNAL_SEQIDX_TAG, MPI_COMM_WORLD, &mpistatus)) != 0) {
				mpi_failure("failed to receive sequence index to search");
			}
			...
			buf_size = 0;
			status = MPI_Pack_size(1, MPI_LONG_LONG_INT, MPI_COMM_WORLD, &n);
			buf_size += n;
			if (status != 0) {
				mpi_failure("pack size failed");
			}
			for (h = 0; h < info.nhits; h++) {
				status = MPI_Pack_size(1, MPI_FLOAT, MPI_COMM_WORLD, &n);
				buf_size += n;
				if (status != 0) {
					mpi_failure("pack size failed");
				}
			...
			pos = 0;
			if (MPI_Pack(&(info.nhits),    1, MPI_LONG_LONG_INT, mpibuf, buf_size, &pos, MPI_COMM_WORLD) != 0) {
				mpi_failure("mpi pack failed");
			}
			for (h = 0; h < info.nhits; h++) {
				if (MPI_Pack(&(info.scA[h]), 1, MPI_FLOAT,         mpibuf, buf_size, &pos, MPI_COMM_WORLD) != 0) {
					mpi_failure("mpi pack failed");
				}
			}
#if DEBUGMPI
			...
			if (MPI_Send(mpibuf, buf_size, MPI_PACKED, 0, INFERNAL_SCORES_TAG, MPI_COMM_WORLD)    != 0) {
				mpi_failure("mpi send failed");
			}
...
output_header(FILE *ofp, const ESL_GETOPTS *go, char *cmfile, int available_ncpus, int ncpus)
{
	...
	if (esl_opt_IsUsed(go, "-L"))          {
		fprintf(ofp, "# total sequence length to search per mode:    %g Mb\n", esl_opt_GetReal(go, "-L"));
	}

	if (esl_opt_IsUsed(go, "--forecast"))  {
		fprintf(ofp, "# forecast mode (no calibration):              on\n");
	}
	if (esl_opt_IsUsed(go, "--memreq"))    {
		fprintf(ofp, "# memory-requirement mode (no calibration):    on\n");
	}

	if (esl_opt_IsUsed(go, "--gtailn"))    {
		fprintf(ofp, "# number of hits/Mb to fit (glocal):           %d\n", esl_opt_GetInteger(go, "--gtailn"));
	}
	if (esl_opt_IsUsed(go, "--ltailn"))    {
		fprintf(ofp, "# number of hits/Mb to fit (local):            %d\n", esl_opt_GetInteger(go, "--ltailn"));
	}
	if (esl_opt_IsUsed(go, "--tailp"))     {
		fprintf(ofp, "# fraction of histogram tail to fit:           %g\n", esl_opt_GetReal(go, "--tailp"));
	}

	if (esl_opt_IsUsed(go, "--hfile"))     {
		fprintf(ofp, "# saving fitted score histograms to file:      %s\n", esl_opt_GetString(go, "--hfile"));
	}
	if (esl_opt_IsUsed(go, "--sfile"))     {
		fprintf(ofp, "# saving survival plot to file:                %s\n", esl_opt_GetString(go, "--sfile"));
	}
	if (esl_opt_IsUsed(go, "--qqfile"))    {
		fprintf(ofp, "# saving Q-Q plot for histograms to file:      %s\n", esl_opt_GetString(go, "--qqfile"));
	}
	if (esl_opt_IsUsed(go, "--ffile"))     {
		fprintf(ofp, "# saving lambdas for tail fit probs to file:   %s\n", esl_opt_GetString(go, "--ffile"));
	}
	if (esl_opt_IsUsed(go, "--xfile"))     {
		fprintf(ofp, "# saving scores from fit tails to file:        %s\n", esl_opt_GetString(go, "--xfile"));
	}
	...
	if (esl_opt_IsUsed(go, "--beta"))      {
		fprintf(ofp, "# tail loss probability for QDBs:              %g\n", esl_opt_GetReal(go, "--beta"));
	}
	if (esl_opt_IsUsed(go, "--nonbanded")) {
		fprintf(ofp, "# query dependent bands (QDBs):                off\n");
	}
	if (esl_opt_IsUsed(go, "--nonull3"))   {
		fprintf(ofp, "# null3 bias corrections:                      off\n");
	}
	if (esl_opt_IsUsed(go, "--random"))    {
		fprintf(ofp, "# generating seqs with cm->null (usually iid): yes\n");
	}
	if (esl_opt_IsUsed(go, "--gc"))        {
		fprintf(ofp, "# nucleotide distribution from file:           %s\n", esl_opt_GetString(go, "--gc"));
	}
...
init_master_cfg(const ESL_GETOPTS *go, struct cfg_s *cfg, char *errbuf)
{
	...
	if (cfg->w == NULL) {
		ESL_FAIL(eslEMEM, errbuf, "Failed to create stopwatch.");
	}
...
init_shared_cfg(const ESL_GETOPTS *go, struct cfg_s *cfg, char *errbuf)
{
	...
	if (esl_opt_GetString(go, "--gc") != NULL) {
		...
		if (status == eslENOTFOUND) {
			ESL_FAIL(status, errbuf, "No such file: %s.", esl_opt_GetString(go, "--gc"));
		} else if (status == eslEFORMAT) {
			ESL_FAIL(status, errbuf, "file: %s format unrecognized.", esl_opt_GetString(go, "--gc"));
		} else if (status != eslOK) {
			ESL_FAIL(status, errbuf, "Failed to open sequence database file %s, code %d.", esl_opt_GetString(go, "--gc"), status);
		}
		if ((status = GetDBInfo(tmp_abc, dbfp, errbuf, NULL, NULL, &(cfg->gc_freq))) != eslOK) {
			return status;
		}
	...
	if (cfg->r     == NULL) {
		ESL_FAIL(eslEMEM, errbuf, "Failed to create RNG.");
	}
	if (cfg->r_est == NULL) {
		ESL_FAIL(eslEMEM, errbuf, "Failed to create RNG.");
	}
...
fit_histogram(const ESL_GETOPTS *go, struct cfg_s *cfg, char *errbuf, float *scores, int nscores, int exp_mode, double *ret_mu, double *ret_lambda, int *ret_nrandhits, float *ret_tailp)
{
	...
	for (i = 0; i < nscores; i++) {
		if ((status = esl_histogram_Add(h, scores[i])) != eslOK) {
			ESL_FAIL(status, errbuf, "fit_histogram(), esl_histogram_Add() call returned non-OK status: %d\n", status);
		}
		/* printf("%4d %.3f\n", i, scores[i]); */
	}
	...
	if (esl_opt_IsOn(go, "--tailp")) {
		...
		if (ExpModeIsLocal(exp_mode)) { /* local mode */
			...
			if (tailp > 1.) {
				ESL_FAIL(eslERANGE, errbuf, "--ltailn <n>=%d cannot be used, there's only %.3f hits per Mb in the histogram! Lower <n> or use --tailp.", esl_opt_GetInteger(go, "--ltailn"), (h->n / ((float) cfg->N * ((float) cfg->L) / 1000000.)));
			}
		} else { /* glocal mode */
			nhits_to_fit = (float) esl_opt_GetInteger(go, "--gtailn") * ((cfg->N * cfg->L) / 1000000.);
			tailp = nhits_to_fit / (float) h->n;
			if (tailp > 1.) {
				ESL_FAIL(eslERANGE, errbuf, "--gtailn <n>=%d cannot be used, there's only %.3f hits per Mb in the histogram! Lower <n> or use --tailp.", esl_opt_GetInteger(go, "--gtailn"), (h->n / ((float) cfg->N * ((float) cfg->L) / 1000000.)));
			}
		}
	}
	...
	if (n <= 1) {
		ESL_FAIL(eslERANGE, errbuf, "fit_histogram(), too few points in right tailfit: %f fraction of histogram. Increase <x> with -L <x>.", tailp);
	}
	if (cfg->xfp != NULL) {
		for (i = 0; i < n; i++) {
			fprintf(cfg->xfp, "%.5f\n", xv[i]);
	...
	lambda = params[1];
	if (isnan(lambda)) {
		ESL_FAIL(eslERANGE, errbuf, "fit_histogram(), exp tail fit lambda is NaN, too few hits in histogram. Increase <x> with -L <x>.");
	}
	if (isinf(lambda)) {
		ESL_FAIL(eslERANGE, errbuf, "fit_histogram(), exp tail fit lambda is inf, too few hits in histogram. Increase <x> with -L <x>.");
	}
	nrandhits = h->n; /* total number of hits in the histogram */

	/* print to output files if nec */
	if (cfg->hfp != NULL) {
		esl_histogram_Plot(cfg->hfp, h);
	}
	if (cfg->qfp != NULL) {
		esl_histogram_PlotQQ(cfg->qfp, h, &esl_exp_generic_invcdf, params);
	}
	if (cfg->sfp != NULL) {
...
get_random_dsq(const struct cfg_s *cfg, char *errbuf, CM_t *cm, int L, ESL_RANDOMNESS *r, ESL_DSQ **ret_dsq)
{
	...
	if ((status = esl_rsq_xIID(r, distro, cm->abc->K, L, dsq) != eslOK)) {
		return status;
	}
...
generate_sequences(const ESL_GETOPTS *go, const struct cfg_s *cfg, char *errbuf, CM_t *cm, ESL_SQ_BLOCK **ret_sq_block)
{
	...
	for (i = 0; i < cfg->N; i++) {
		...
		if (esl_opt_GetBoolean(go, "--random") || esl_opt_IsUsed(go, "--gc")) {
			if ((status = get_random_dsq(cfg, errbuf, cm, cfg->L, cfg->r, &dsq)) != eslOK) {
				goto ERROR;
			}
		} else {
			if ((status = SampleGenomicSequenceFromHMM(cfg->r, cfg->abc, errbuf, cfg->ghmm_sA, cfg->ghmm_tAA, cfg->ghmm_eAA, cfg->ghmm_nstates, cfg->L, &dsq)) != eslOK) {
				goto ERROR;
			}
		}
		...
		if ((status = esl_abc_dsqdup(dsq, cfg->L, &(sq->dsq))) != eslOK) {
			goto ERROR;
		}
...
int forecast_time(const ESL_GETOPTS *go, struct cfg_s *cfg, char *errbuf, CM_t *cm, int ncpus, int available_ncpus, double *ret_psec, int *ret_ins_v_cyk)
{
	...
	if (cm->flags & CMH_LOCAL_BEGIN) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "forecast_running_time() CM has local begins on");
	}
	if (cm->flags & CMH_LOCAL_END) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "forecast_running_time() CM has local ends on");
	}
	if (cm->search_opts & CM_SEARCH_INSIDE) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "forecast_running_time() CM wants to search with Inside");
	}
	if (cm->smx == NULL) {
		ESL_FAIL(eslEINCOMPAT, errbuf, "forecast_running_time(), cm->smx is NULL");
	}
	...
	if (esl_opt_GetBoolean(go, "--random") || esl_opt_IsUsed(go, "--gc")) {
		if ((status = get_random_dsq(cfg, errbuf, cm, L, cfg->r_est, &dsq)) != eslOK) {
			return status;
		}
	} else {
		if ((status = SampleGenomicSequenceFromHMM(cfg->r_est, cm->abc, errbuf, cfg->ghmm_sA, cfg->ghmm_tAA, cfg->ghmm_eAA, cfg->ghmm_nstates, L, &dsq)) != eslOK) {
			cm_Fail(errbuf);
		}
	}
	 ...
	 */
	if (use_qdb) {
		if ((status = cm_CountSearchDPCalcs(cm, errbuf, L, NULL,               NULL,               cm->W, FALSE,  NULL, &Mc_per_res)) != eslOK) {
			return status;
		}
	} else        {
		if ((status = cm_CountSearchDPCalcs(cm, errbuf, L, cm->qdbinfo->dmin2, cm->qdbinfo->dmax2, cm->W, FALSE,  NULL, &Mc_per_res)) != eslOK) {
			return status;
		}
	}
	 ...
	 */
	if (use_qdb) {
		if ((status = cm_CountSearchDPCalcs(cm, errbuf, 10 * cm->W, NULL,               NULL,               cm->W, TRUE,  NULL, &Mc_per_res_corr)) != eslOK) {
			return status;
		}
	} else        {
		if ((status = cm_CountSearchDPCalcs(cm, errbuf, 10 * cm->W, cm->qdbinfo->dmin2, cm->qdbinfo->dmax2, cm->W, TRUE,  NULL, &Mc_per_res_corr)) != eslOK) {
			return status;
		}
	}
	...
	esl_stopwatch_Start(cfg->w);
	if ((status = process_search_workunit(cm, errbuf, dsq, L, cfg->sc_cutoff, NULL)) != eslOK) {
		cm_Fail(errbuf);
	}
	esl_stopwatch_Stop(cfg->w);
	...
	*/
	...
	if (ret_ins_v_cyk != NULL) {
		*ret_ins_v_cyk = ins_v_cyk;
	}
