...
main(int argc, char **argv)
{
	...
	if (esl_opt_GetBoolean(go, "--index")) {
		create_ssi_index(go, cmfp);
	} else if (esl_opt_GetBoolean(go, "-f")) {
		multifetch(go, ofp, keyfile, cmfp);
	} else {
...
create_ssi_index(ESL_GETOPTS *go, CM_FILE *cmfp)
{
	...
	if (esl_sprintf(&ssifile,  "%s.ssi", cmfp->fname) != eslOK) {
	...
	if (status == eslENOTFOUND) {
		...
		cm_Fail("SSI index %s already exists; delete or rename it", ssifile);
	} else if (status != eslOK) {
...
multifetch(ESL_GETOPTS *go, FILE *ofp, char *keyfile, CM_FILE *cmfp)
{
	...
	if (esl_fileparser_Open(keyfile, NULL, &efp) != eslOK) {
		cm_Fail("Failed to open key file %s\n", keyfile);
	}
	...
	while (esl_fileparser_NextLine(efp) == eslOK) {
		...
		if (status == eslEDUP) {
			cm_Fail("CM key %s occurs more than once in file %s\n", key, keyfile);
		}

		if (cmfp->ssi != NULL) {
			onefetch(go, ofp, key, cmfp);
			ncm++;
		}
	}
	...
	if (cmfp->ssi == NULL) {
		while ((status = cm_file_Read(cmfp, TRUE, &abc, &cm)) == eslOK) {
			if (esl_keyhash_Lookup(keys, cm->name, -1, &keyidx) == eslOK ||
				...
				if ((status = cm_file_WriteASCII(ofp, -1, cm)) != eslOK) {
					cm_Fail("CM save failed");
				}
				ncm++;
			}
			FreeCM(cm);
...
onefetch(ESL_GETOPTS *go, FILE *ofp, char *key, CM_FILE *cmfp)
{
	...
	if (cmfp->ssi != NULL) {
		...
		if (status == eslENOTFOUND) {
			cm_Fail("CM %s not found in SSI index for file %s\n", key, cmfp->fname);
		} else if (status == eslEFORMAT) {
			cm_Fail("Failed to parse SSI index for %s\n", cmfp->fname);
		} else if (status != eslOK) {
			cm_Fail("Failed to look up location of CM %s in SSI index of file %s\n", key, cmfp->fname);
		}
	}
	...
	while ((status = cm_file_Read(cmfp, TRUE, &abc, &cm)) != eslEOF) {
		...
		if (strcmp(key, cm->name) == 0 || (cm->acc && strcmp(key, cm->acc) == 0)) {
			break;
		}
		FreeCM(cm);
		cm = NULL;
