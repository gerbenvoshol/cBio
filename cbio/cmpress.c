...
main(int argc, char **argv)
{
	...
	if (strcmp(cmfile, "-") == 0) {
		cm_Fail("Can't use - for <cmfile> argument: can't index standard input\n");
	}
	...
	if (cmfp->do_stdin || cmfp->do_gzip) {
		cm_Fail("CM file %s must be a normal file, not gzipped or a stdin pipe", cmfile);
	}
	...
	while ((status = cm_file_Read(cmfp, TRUE, &abc, &cm)) == eslOK) {
		if (cm->name == NULL) {
			cm_Fail("Every CM must have a name to be indexed. Failed to find name of CM #%d\n", ncm + 1);
		}
		if (!(cm->flags & CMH_FP7)) {
			cm_Fail("Failed to read a p7 HMM filter for CM #%d\n", ncm + 1);
		}
		...
		if (!(cm->flags & CMH_EXPTAIL_STATS)) {
			cm_Fail("CMs must have E-value statistics to be press'd. Failed to find stats for CM #%d\n", ncm + 1);
		}
		...
		if (cm->fp7 == NULL || (!(cm->flags & CMH_FP7))) {
			cm_Fail("CM #%d does not have a filter HMM", ncm + 1);
		}
		...
		if ((cm_offset            = ftello(mfp)) == -1) {
			cm_Fail("Failed to ftello() current disk position of CM db file");
		}
		if (esl_newssi_AddKey(nssi, cm->name, fh, cm_offset, 0, 0) != eslOK) {
			cm_Fail("Failed to add key %s to SSI index", cm->name);
		}
		if (cm->acc) {
			if (esl_newssi_AddAlias(nssi, cm->acc, cm->name)         != eslOK) {
				cm_Fail("Failed to add secondary key %s to SSI index", cm->acc);
			}
		}
		if ((!(cm->flags & CMH_FP7)) || (cm->fp7 == NULL)) {
			cm_Fail("CM %s does not have a p7 filter HMM", cm->name);
		}
		cm_file_WriteBinary(mfp, -1, cm, &fp7_offset);

		/* write the oprofile after the CM, because we need to know fp7_offset first */
		if ((om->offs[p7_FOFFSET] = ftello(ffp)) == -1) {
			cm_Fail("Failed to ftello() current disk position of MSV db file");
		}
		if ((om->offs[p7_POFFSET] = ftello(pfp)) == -1) {
			cm_Fail("Failed to ftello() current disk position of profile db file");
		}
	...
	if (status == eslEFORMAT) {
		cm_Fail("bad file format in CM file %s",             cmfile);
	} else if (status == eslEINCOMPAT) {
		cm_Fail("CM file %s contains different alphabets",   cmfile);
	} else if (status != eslEOF) {
		cm_Fail("Unexpected error in reading CMs from %s",   cmfile);
	}

	if (esl_newssi_Write(nssi) != eslOK) {
		cm_Fail("Failed to write keys to ssi file\n");
	}
...
open_db_files(ESL_GETOPTS *go, char *basename, FILE **ret_mfp, FILE **ret_ffp,  FILE **ret_pfp, ESL_NEWSSI **ret_nssi)
{
	...
	if (esl_sprintf(&ssifile, "%s.i1i", basename) != eslOK) {
		cm_Die("esl_sprintf() failed");
	}
	status = esl_newssi_Open(ssifile, allow_overwrite, &nssi);
	if (status == eslENOTFOUND) {
		cm_Fail("failed to open SSI index %s", ssifile);
	} else if (status == eslEOVERWRITE) {
		cm_Fail("Looks like %s is already pressed (.i1i file present, anyway): delete old cmpress indices first, or use -F", basename);
	} else if (status != eslOK) {
		cm_Fail("failed to create a new SSI index");
	}

	if (esl_sprintf(&mfile, "%s.i1m", basename) != eslOK) {
		cm_Die("esl_sprintf() failed");
	}
	if (! allow_overwrite && esl_FileExists(mfile)) {
		cm_Fail("Binary CM file %s already exists; delete old cmpress indices first, or use -F", mfile);
	}
	if ((mfp = fopen(mfile, "wb"))              == NULL) {
		cm_Fail("Failed to open binary CM file %s for writing", mfile);
	}

	if (esl_sprintf(&ffile, "%s.i1f", basename) != eslOK) {
		cm_Die("esl_sprintf() failed");
	}
	if (! allow_overwrite && esl_FileExists(ffile)) {
		cm_Fail("Binary MSV filter file %s already exists; delete old cmpress indices first, or use -F", ffile);
	}
	if ((ffp = fopen(ffile, "wb"))              == NULL) {
		cm_Fail("Failed to open binary MSV filter file %s for writing", ffile);
	}

	if (esl_sprintf(&pfile, "%s.i1p", basename) != eslOK) {
		cm_Die("esl_sprintf() failed");
	}
	if (! allow_overwrite && esl_FileExists(pfile)) {
		cm_Fail("Binary optimized profile file %s already exists; delete old cmpress indices first, or use -F", pfile);
	}
	if ((pfp = fopen(pfile, "wb"))              == NULL) {
		cm_Fail("Failed to open binary optimized profile file %s for writing", pfile);
	}
