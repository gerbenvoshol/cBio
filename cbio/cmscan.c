...
main(int argc, char **argv)
{
	...
	if (esl_opt_GetBoolean(go, "--mpi")) {
		...
		if (cfg.nproc == 1) {
			cm_Fail("MPI mode, but only 1 processor running... (did you execute mpirun?)");
		}

		if (cfg.my_rank > 0) {
			status = mpi_worker(go, &cfg);
		} else {
			status = mpi_master(go, &cfg);
		}
...
serial_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if (esl_opt_GetBoolean(go, "--notextw")) {
		textw = 0;
	} else {
		textw = esl_opt_GetInteger(go, "--textw");
	}
	...
	if (status == eslENOTFOUND) {
		cm_Fail("File existence/permissions problem in trying to open CM file %s.\n%s\n", cfg->cmfile, errbuf);
	} else if (status == eslEFORMAT) {
		cm_Fail("File format problem in trying to open CM file %s.\n%s\n",                cfg->cmfile, errbuf);
	} else if (status != eslOK) {
		cm_Fail("Unexpected error %d in opening CM file %s.\n%s\n",               status, cfg->cmfile, errbuf);
	}
	if (cmfp->do_gzip) {
		cm_Fail("Reading gzipped CM files is not supported");
	}
	if (cmfp->do_stdin) {
		cm_Fail("Reading CM files from stdin is not supported");
	}
	if (! cmfp->is_pressed) {
		cm_Fail("Failed to open binary auxfiles for %s: use cmpress first\n",             cmfp->fname);
	}
	nmodels = (int64_t) cmfp->ssi->nprimary;

	hstatus = cm_file_Read(cmfp, FALSE, &abc, NULL);
	if (hstatus == eslEFORMAT) {
		cm_Fail("bad file format in CM file %s\n%s",           cfg->cmfile, cmfp->errbuf);
	} else if (hstatus != eslOK) {
		cm_Fail("Unexpected error in reading CMs from %s\n%s", cfg->cmfile, cmfp->errbuf);
	}
	...
	if (esl_opt_IsUsed(go, "-Z")) {
		...
		if (cmfp->ssi == NULL) {
			cm_Fail("Failed to open SSI index for CM file: %s\n", cmfp->fname);
		}
	...
	if (esl_opt_IsOn(go, "--glist")) {
		if ((status = read_glocal_list_file(esl_opt_GetString(go, "--glist"), errbuf, cmfp, &glocal_kh)) != eslOK) {
			cm_Fail(errbuf);
		}
	}
	/* If nec, open and read the clan info file, while CM file is open */
	if (esl_opt_IsOn(go, "--clanin")) {
		if ((status = read_clan_info_file(esl_opt_GetString(go, "--clanin"), errbuf, cmfp, &clan_name_kh, &clan_fam_kh, &clan_mapA)) != eslOK) {
			cm_Fail(errbuf);
		}
	}
	...
	if (status == eslENOTFOUND) {
		cm_Fail("Failed to open sequence file %s for reading\n",      cfg->seqfile);
	} else if (status == eslEFORMAT) {
		cm_Fail("Sequence file %s is empty or misformatted\n",        cfg->seqfile);
	} else if (status == eslEINVAL) {
		cm_Fail("Can't autodetect format of a stdin or .gz seqfile");
	} else if (status != eslOK) {
		cm_Fail("Unexpected error %d opening sequence file %s\n", status, cfg->seqfile);
	}
	...
	if (esl_opt_IsOn(go, "--tblout"))    {
		if ((tblfp    = fopen(esl_opt_GetString(go, "--tblout"),    "w")) == NULL) {
	...
	for (i = 0; i < tinfocnt; ++i) {
		...
		if (glocal_kh != NULL) {
			if ((tinfo[i].glocal_kh = esl_keyhash_Clone(glocal_kh)) == NULL) {
				esl_fatal("Failed to clone keyhash, out of memory");
			}
		} else {
	...
	while ((sstatus = esl_sqio_Read(sqfp, qsq)) == eslOK) {
		...
		for (in_rc = 0; in_rc <= 1; in_rc++) {
			if (in_rc == 0 && (! tinfo->pli->do_top)) {
				continue;    /* skip top strand */
			}
			if (in_rc == 1 && (! tinfo->pli->do_bot)) {
				continue;    /* skip bottom strand */
			}
			if (in_rc == 1) {
				if (qsq->abc->complement == NULL) {
					if (! tinfo->pli->do_top) {
						cm_Fail("Trying to only search bottom strand but sequence cannot be complemented");
					} else {
						continue;    /* skip bottom strand b/c we can't complement the sequence */
					}
				}
			...
			if ((status = cm_file_Open(cfg->cmfile, CMDBENV, FALSE, &cmfp, NULL)) != eslOK) {
				cm_Fail("Unexpected error %d in opening cm file %s.\n%s", status, cfg->cmfile, cmfp->errbuf);
			}
#ifdef HMMER_THREADS
			if (ncpus > 0) { /* if we are threaded, create locks to prevent multiple readers */
				if ((status = cm_file_CreateLock(cmfp)) != eslOK) {
					cm_Fail("Unexpected error %d creating lock\n", status);
				}
			}
#endif
			for (i = 0; i < tinfocnt; ++i) {
...
#ifdef HMMER_THREADS
				if (ncpus > 0) {
					esl_threads_AddThread(threadObj, &tinfo[i]);
				}
#endif
			}
#ifdef HMMER_THREADS
			if (ncpus > 0) {
				hstatus = thread_loop(threadObj, queue, rinfo, cmfp);
			} else {
			...
			case eslEMEM:
				...
				break;
			case eslEOF:
				cm_Fail("Unexpected, premature end of file in %s",   cfg->cmfile);
				break;
			case eslOK:        /* normal: do nothing */
				break;
			default:
		...
		if (tinfo[0].pli->do_top && tinfo[0].pli->do_bot) {
			...
			if (tinfo[0].pli->nmodels_hmmonly > 0) {
				tinfo[0].pli->nmodels_hmmonly /= 2;
			}
			if (tinfo[0].pli->nnodes_hmmonly  > 0) {
				tinfo[0].pli->nnodes_hmmonly /= 2;
			}
		}
		...
		if ((status = cm_tophits_RemoveOrMarkOverlaps(tinfo[0].th, FALSE, errbuf)) != eslOK) {
			cm_Fail(errbuf);
		}
		...
		if ((status = cm_tophits_RemoveOrMarkOverlaps(tinfo[0].th, esl_opt_GetBoolean(go, "--oclan"), errbuf)) != eslOK) {
			cm_Fail(errbuf);
		}

		/* Resort: by score (usually) or by position (if in special 'terminate after F3' mode) */
		if (tinfo[0].pli->do_trm_F3) {
			cm_tophits_SortByPosition(tinfo[0].th);
		} else {
			cm_tophits_SortByEvalue(tinfo[0].th);
		}
		...
		if (tinfo[0].pli->show_alignments) {
			if ((status = cm_tophits_HitAlignments(ofp, tinfo[0].th, tinfo[0].pli, textw)) != eslOK) {
		...
		if (tblfp != NULL) {
			if ((! esl_opt_IsUsed(go, "--fmt")) || (esl_opt_GetInteger(go, "--fmt") == 1)) { /* fmt defaults to 1 */
				if (tinfo[0].pli->do_trm_F3) {
					cm_tophits_F3TabularTargets1(tblfp, tinfo[0].th, tinfo[0].pli, (seq_idx == 1));
				} else {
					cm_tophits_TabularTargets1(tblfp, qsq->name, qsq->acc, tinfo[0].th, tinfo[0].pli, (seq_idx == 1));
				}
	...
	if (esl_opt_GetBoolean(go, "--verbose")) {
		esl_stopwatch_Display(ofp, mw, "# Total CPU time:");
	}

	/* Terminate outputs - any last words?
	 */
	if (tblfp) {
		cm_tophits_TabularTail(tblfp,    "cmscan", CM_SCAN_MODELS, cfg->seqfile, cfg->cmfile, go);
	}
	if (ofp) {
		fprintf(ofp, "[ok]\n");
	}
	...
	for (i = 0; i < tinfocnt; ++i) {
		...
		if (tinfo[i].glocal_kh    != NULL) {
			esl_keyhash_Destroy(tinfo[i].glocal_kh);
		}
	}
	...
	if (glocal_kh    != NULL) {
		esl_keyhash_Destroy(glocal_kh);
	}
	if (clan_name_kh != NULL) {
		esl_keyhash_Destroy(clan_name_kh);
	}
	if (clan_fam_kh  != NULL) {
		esl_keyhash_Destroy(clan_fam_kh);
	}
	if (clan_mapA    != NULL) {
		free(clan_mapA);
	}
...
serial_loop(THREAD_INFO *tinfo, READER_INFO *rinfo, CM_FILE *cmfp)
{
	...
	while (cm_idx < rinfo->nmodels && status == eslOK) {
		if (rinfo->omA[cm_idx] == NULL) {
			...
			if (have_clans) {
				rinfo->clan_idxA[cm_idx] = determine_clan_index(rinfo->omA[cm_idx]->name, rinfo->clan_fam_kh, rinfo->clan_mapA);
			} else           {
		...
		if (status == eslOK) {
			...
			cm     = NULL; /* ditto */
			if (tinfo->pli->do_wcx) {
				cm_W = (int) cm_clen * tinfo->pli->wcx;    /* do_wcx == TRUE means --wcx was used */
			}
			if ((status = cm_pli_NewModel(tinfo->pli, CM_NEWMODEL_MSV,
			                              ...
			                              tinfo->bg, tinfo->p7_evparam, om->max_length, cm_idx, clan_idx, tinfo->glocal_kh)) != eslOK) {
				cm_Fail(tinfo->pli->errbuf);
			}
			...
			while (prv_posn != tinfo->qsq->L) {
				...
				if (tinfo->qsq->C > 0) {
					cm_pli_AdjustNresForOverlaps(tinfo->pli, tinfo->qsq->C, tinfo->in_rc);
				}
				/* adjust hit positions so they're w.r.t full source sequence */
				if (tinfo->th->N != prv_ntophits) {
					cm_tophits_UpdateHitPositions(tinfo->th, prv_ntophits, tinfo->qsq->start, tinfo->in_rc);
				}

				if (tinfo->th->N != prv_ntophits && (! tinfo->pli->do_trm_F3)) {
					if (tinfo->pli->do_hmmonly_cur) {
						eZ = tinfo->pli->Z / (float) om->max_length;
					} else {
						eZ = cm->expA[tinfo->pli->final_cm_exp_mode]->cur_eff_dbsize;
					}
...
thread_loop(ESL_THREADS *obj, ESL_WORK_QUEUE *queue, READER_INFO *rinfo, CM_FILE *cmfp)
{
	...
	if (status != eslOK) {
		esl_fatal("Work queue reader failed");
	}
	...
	while (sstatus == eslOK) {
		...
		if (idx0 < rinfo->nmodels && rinfo->omA[idx0] == NULL) {
			...
			i = 0;
			while (i < block->count) {
				if (cm_idx >= rinfo->nmodels) {
					return eslFAIL;    /* this should never happen */
				}
				...
				if (have_clans) {
					block->clan_idxA[i] = determine_clan_index(rinfo->omA[cm_idx]->name, rinfo->clan_fam_kh, rinfo->clan_mapA);
				} else           {
					block->clan_idxA[i] = -1;
				}
		...
		if (sstatus == eslEOF) {
			if (eofCount < esl_threads_GetWorkerCount(obj)) {
				sstatus = eslOK;
			}
			++eofCount;
		}
...
pipeline_thread(void *arg)
{
	...
	if (status != eslOK) {
		esl_fatal("Work queue worker failed");
	}
...
mpi_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if (esl_opt_IsOn(go, "--qformat")) {
		seqfmt = esl_sqio_EncodeFormat(esl_opt_GetString(go, "--qformat"));
		if (seqfmt == eslSQFILE_UNKNOWN) {
			mpi_failure("%s is not a recognized input sequence file format\n", esl_opt_GetString(go, "--qformat"));
		}
	}
	...
	status = cm_file_Open(cfg->cmfile, CMDBENV, FALSE, &cmfp, errbuf);
	if (status == eslENOTFOUND) {
		mpi_failure("File existence/permissions problem in trying to open CM file %s.\n%s\n", cfg->cmfile, errbuf);
	} else if (status == eslEFORMAT) {
		mpi_failure("File format problem in trying to open CM file %s.\n%s\n",                cfg->cmfile, errbuf);
	} else if (status != eslOK) {
		mpi_failure("Unexpected error %d in opening CM file %s.\n%s\n",               status, cfg->cmfile, errbuf);
	}
	if (cmfp->do_gzip) {
		mpi_failure("Reading gzipped CM files is not supported");
	}
	if (cmfp->do_stdin) {
		mpi_failure("Reading CM files from stdin is not supported");
	}
	if (! cmfp->is_pressed) {
		mpi_failure("Failed to open binary auxfiles for %s: use cmpress first\n",             cmfp->fname);
	}
	nmodels = (int64_t) cmfp->ssi->nprimary;

	hstatus = cm_file_Read(cmfp, FALSE, &abc, NULL);
	if (hstatus == eslEFORMAT) {
		mpi_failure("bad file format in CM file %s\n%s",           cfg->cmfile, cmfp->errbuf);
	} else if (hstatus != eslOK) {
		mpi_failure("Unexpected error in reading CMs from %s\n%s", cfg->cmfile, cmfp->errbuf);
	}
	...
	if (esl_opt_IsUsed(go, "-Z")) {
		...
		if (cmfp->ssi == NULL) {
			mpi_failure("Failed to open SSI index for CM file: %s\n", cmfp->fname);
		}
		cfg->Z = (int64_t) cmfp->ssi->nprimary;
		cfg->Z_setby = CM_ZSETBY_SSI_AND_QLENGTH; /* we will multiply Z by each query sequence length */
	}
	...
	status = esl_sqfile_OpenDigital(abc, cfg->seqfile, seqfmt, NULL, &sqfp);
	if (status == eslENOTFOUND) {
		mpi_failure("Failed to open sequence file %s for reading\n",      cfg->seqfile);
	} else if (status == eslEFORMAT) {
		mpi_failure("Sequence file %s is empty or misformatted\n",        cfg->seqfile);
	} else if (status == eslEINVAL) {
		mpi_failure("Can't autodetect format of a stdin or .gz seqfile");
	} else if (status != eslOK) {
		mpi_failure("Unexpected error %d opening sequence file %s\n", status, cfg->seqfile);
	}
	...
	while ((sstatus = esl_sqio_Read(sqfp, qsq)) == eslOK) {
		...
		/* scan all target CMs twice, once with the top strand of the query and once with the bottom strand */
		for (in_rc = 0; in_rc <= 1; in_rc++) {
			if (in_rc == 0 && (! pli->do_top)) {
				continue;    /* skip top strand */
			}
			if (in_rc == 1 && (! pli->do_bot)) {
				continue;    /* skip bottom strand */
			}
			if (in_rc == 1) {
				if (qsq->abc->complement == NULL) {
					if (! pli->do_top) {
						mpi_failure("Trying to only search bottom strand but sequence cannot be complemented");
					} else {
						continue;    /* skip bottom strand b/c we can't complement the sequence */
					}
				}
			...
			switch (hstatus) {
			case eslEFORMAT:
				mpi_failure("bad file format in CM file %s",           cfg->cmfile);
				break;
			case eslEINCOMPAT:
				mpi_failure("CM file %s contains different alphabets", cfg->cmfile);
				break;
			case eslEOF:       /* do nothing */
				break;
			default:
				mpi_failure("Unexpected error %d in reading CMs from %s", hstatus, cfg->cmfile);
				break;
			}
		...
		if (pli->do_top && pli->do_bot) {
			...
			if (pli->nmodels_hmmonly > 0) {
				pli->nmodels_hmmonly /= 2;
			}
			if (pli->nnodes_hmmonly  > 0) {
				pli->nnodes_hmmonly /= 2;
			}
		}

		/* Sort by sequence index/position and remove duplicates found because we searched overlapping chunks */
		cm_tophits_SortForOverlapRemoval(th);
		if ((status = cm_tophits_RemoveOrMarkOverlaps(th, FALSE, errbuf)) != eslOK) {
			mpi_failure(errbuf);
		}

		/* Resort in order to markup overlapping hits from different models (only within clans if --oclan) */
		cm_tophits_SortForOverlapMarkup(th, esl_opt_GetBoolean(go, "--oclan"));
		if ((status = cm_tophits_RemoveOrMarkOverlaps(th, esl_opt_GetBoolean(go, "--oclan"), errbuf)) != eslOK) {
			cm_Fail(errbuf);
		}
		...
		if (pli->show_alignments) {
			if ((status = cm_tophits_HitAlignments(ofp, th, pli, textw)) != eslOK) {
		...
		if (tblfp != NULL) {
			if ((! esl_opt_IsUsed(go, "--fmt")) || (esl_opt_GetInteger(go, "--fmt") == 1)) { /* fmt defaults to 1 */
				if (pli->do_trm_F3) {
					cm_tophits_F3TabularTargets1(tblfp, th, pli, (seq_idx == 1));
				} else {
					cm_tophits_TabularTargets1(tblfp, qsq->name, qsq->acc, th, pli, (seq_idx == 1));
				}
			} else if (esl_opt_GetInteger(go, "--fmt") == 2) {
	...
	esl_stopwatch_Stop(mw);
	if (esl_opt_GetBoolean(go, "--verbose")) {
		esl_stopwatch_Display(stdout, mw, "# Total CPU time:");
	}
		...
		for (in_rc = 0; in_rc <= 1; in_rc++) {
			...
			while (block.count > 0) {
				...
				if (do_read) {
					...
					if (hstatus != eslOK) {
						mpi_failure("Cannot position optimized model to %ld\n", block.offset);
					}
				}
				...
				while (count > 0 && hstatus == eslOK) {
					if (do_read) {
						if (rinfo->omA[cm_idx] != NULL) {
							mpi_failure("Worker was supposed to read MSVs but already had one...\n");
						}
					...
					if (hstatus == eslOK) {
						...
						cm     = NULL; /* ditto */
						if (pli->do_wcx) {
							cm_W = (int) cm_clen * pli->wcx;    /* do_wcx == TRUE means --wcx was used */
						}
						if ((status = cm_pli_NewModel(pli, CM_NEWMODEL_MSV,
						                              cm,                        /* this is NULL b/c we don't have one yet */
						                              cm_clen, cm_W, cm_nbp, om, /* we read these in cm_p7_oprofile_ReadMSV() */
						                              bg, p7_evparam, om->max_length, cm_idx, clan_idx, glocal_kh)) != eslOK) {
							mpi_failure(pli->errbuf);
						}
						...
						while (prv_posn != qsq->L) {
							...
							/* subtract overlapping residues from previous chunk */
							if (qsq->C > 0) {
								cm_pli_AdjustNresForOverlaps(pli, qsq->C, in_rc);
							}
							/* adjust hit positions so they're w.r.t full source sequence */
							if (th->N != prv_ntophits) {
								cm_tophits_UpdateHitPositions(th, prv_ntophits, qsq->start, in_rc);
							}

							if (th->N != prv_ntophits) {
								if (pli->do_hmmonly_cur) {
									eZ = pli->Z / (float) om->max_length;
								} else {
									eZ = cm->expA[pli->final_cm_exp_mode]->cur_eff_dbsize;
								}
				...
				if (count > 0) { /* this means hstatus is not eslOK */
					switch (hstatus) {
					case eslEFORMAT:
						mpi_failure("bad file format in HMM file %s\n%s",          cfg->cmfile, cmfp->errbuf);
						break;
					case eslEINCOMPAT:
						mpi_failure("HMM file %s contains different alphabets",    cfg->cmfile);
						break;
					case eslOK:
					case eslEOF:
						...
						break;
					default:
						mpi_failure("Unexpected error %d in reading HMMs from %s\n%s", hstatus, cfg->cmfile, cmfp->errbuf);
						break;
					}
				}
...
process_commandline(int argc, char **argv, ESL_GETOPTS **ret_go, char **ret_cmfile, char **ret_seqfile)
{
	...
	if ((*ret_cmfile = esl_opt_GetArg(go, 1))  == NULL)  {
		puts("Failed to get <cmdb> argument on command line");
		goto ERROR;
	}
	if ((*ret_seqfile = esl_opt_GetArg(go, 2)) == NULL)  {
	...
	if (esl_opt_IsUsed(go, "--max")) {
		if (esl_opt_IsUsed(go, "--nohmm"))      {
			puts("Failed to parse command line: Option --max is incompatible with option --nohmm");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--mid"))        {
			puts("Failed to parse command line: Option --max is incompatible with option --mid");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rfam"))       {
			puts("Failed to parse command line: Option --max is incompatible with option --rfam");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--FZ"))         {
			puts("Failed to parse command line: Option --max is incompatible with option --FZ");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF1"))       {
			puts("Failed to parse command line: Option --max is incompatible with option --noF1");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF2"))       {
			puts("Failed to parse command line: Option --max is incompatible with option --noF2");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF3"))       {
			puts("Failed to parse command line: Option --max is incompatible with option --noF3");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF4"))       {
			puts("Failed to parse command line: Option --max is incompatible with option --noF4");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF6"))       {
			puts("Failed to parse command line: Option --max is incompatible with option --noF6");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--doF1b"))      {
			puts("Failed to parse command line: Option --max is incompatible with option --doF1b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF2b"))      {
			puts("Failed to parse command line: Option --max is incompatible with option --noF2b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF3b"))      {
			puts("Failed to parse command line: Option --max is incompatible with option --noF3b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF4b"))      {
			puts("Failed to parse command line: Option --max is incompatible with option --noF4b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--doF5b"))      {
			puts("Failed to parse command line: Option --max is incompatible with option --doF5b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F1"))         {
			puts("Failed to parse command line: Option --max is incompatible with option --F1");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F1b"))        {
			puts("Failed to parse command line: Option --max is incompatible with option --F1b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F2"))         {
			puts("Failed to parse command line: Option --max is incompatible with option --F2");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F2b"))        {
			puts("Failed to parse command line: Option --max is incompatible with option --F2b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F3"))         {
			puts("Failed to parse command line: Option --max is incompatible with option --F3");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F3b"))        {
			puts("Failed to parse command line: Option --max is incompatible with option --F3b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F4"))         {
			puts("Failed to parse command line: Option --max is incompatible with option --F4");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F4b"))        {
			puts("Failed to parse command line: Option --max is incompatible with option --F4b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F5"))         {
			puts("Failed to parse command line: Option --max is incompatible with option --F5");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F6"))         {
			puts("Failed to parse command line: Option --max is incompatible with option --F6");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--ftau"))       {
			puts("Failed to parse command line: Option --max is incompatible with option --ftau");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--fsums"))      {
			puts("Failed to parse command line: Option --max is incompatible with option --fsums");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--fqdb"))       {
			puts("Failed to parse command line: Option --max is incompatible with option --fqdb");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--fbeta"))      {
			puts("Failed to parse command line: Option --max is incompatible with option --fbeta");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--fnonbanded")) {
			puts("Failed to parse command line: Option --max is incompatible with option --fnonbanded");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nocykenv"))   {
			puts("Failed to parse command line: Option --max is incompatible with option --nocykenv");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--cykenvx"))    {
			puts("Failed to parse command line: Option --max is incompatible with option --cykenvx");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--tau"))        {
			puts("Failed to parse command line: Option --max is incompatible with option --tau");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--sums"))       {
			puts("Failed to parse command line: Option --max is incompatible with option --sums");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nonbanded"))  {
			puts("Failed to parse command line: Option --max is incompatible with option --nonbanded");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rt1"))        {
			puts("Failed to parse command line: Option --max is incompatible with option --rt1");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rt2"))        {
			puts("Failed to parse command line: Option --max is incompatible with option --rt2");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rt3"))        {
			puts("Failed to parse command line: Option --max is incompatible with option --rt3");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--ns"))         {
			puts("Failed to parse command line: Option --max is incompatible with option --ns");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--maxtau"))     {
			puts("Failed to parse command line: Option --max is incompatible with option --maxtau");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--anytrunc"))   {
			puts("Failed to parse command line: Option --max is incompatible with option --anytrunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--onlytrunc"))  {
			puts("Failed to parse command line: Option --max is incompatible with option --onlytrunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--5trunc"))     {
			puts("Failed to parse command line: Option --max is incompatible with option --5trunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--3trunc"))     {
			puts("Failed to parse command line: Option --max is incompatible with option --3trunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--onepass"))    {
			puts("Failed to parse command line: Option --max is incompatible with option --onepass");
			goto ERROR;
		}
	}
	if (esl_opt_IsUsed(go, "--nohmm")) {
		if (esl_opt_IsUsed(go, "--max"))        {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --max");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--mid"))        {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --mid");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rfam"))       {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --rfam");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--FZ"))         {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --FZ");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF1"))       {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --noF1");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF2"))       {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --noF2");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF3"))       {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --noF3");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF4"))       {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --noF4");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--doF1b"))      {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --doF1b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF2b"))      {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --noF2b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF3b"))      {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --noF3b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF4b"))      {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --noF4b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--doF5b"))      {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --doF5b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F1"))         {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --F1");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F1b"))        {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --F1b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F2"))         {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --F2");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F2b"))        {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --F2b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F3"))         {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --F3");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F3b"))        {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --F3b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F4"))         {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --F4");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F4b"))        {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --F4b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F5"))         {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --F5");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--ftau"))       {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --ftau");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--fsums"))      {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --fsums");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--tau"))        {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --tau");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--sums"))       {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --sums");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rt1"))        {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --rt1");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rt2"))        {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --rt2");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rt3"))        {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --rt3");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--ns"))         {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --ns");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--maxtau"))     {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --maxtau");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--anytrunc"))   {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --anytrunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--onlytrunc"))  {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --onlytrunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--5trunc"))     {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --5trunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--3trunc"))     {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --3trunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--onepass"))    {
			puts("Failed to parse command line: Option --nohmm is incompatible with option --onepass");
			goto ERROR;
		}
	}
	if (esl_opt_IsUsed(go, "--mid")) {
		if (esl_opt_IsUsed(go, "--max"))      {
			puts("Failed to parse command line: Option --mid is incompatible with option --max");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nohmm"))    {
			puts("Failed to parse command line: Option --mid is incompatible with option --nohmm");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rfam"))     {
			puts("Failed to parse command line: Option --mid is incompatible with option --rfam");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--FZ"))       {
			puts("Failed to parse command line: Option --mid is incompatible with option --FZ");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF1"))     {
			puts("Failed to parse command line: Option --mid is incompatible with option --noF1");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF2"))     {
			puts("Failed to parse command line: Option --mid is incompatible with option --noF2");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF3"))     {
			puts("Failed to parse command line: Option --mid is incompatible with option --noF3");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--doF1b"))    {
			puts("Failed to parse command line: Option --mid is incompatible with option --doF1b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF2b"))    {
			puts("Failed to parse command line: Option --mid is incompatible with option --noF2b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F1"))       {
			puts("Failed to parse command line: Option --mid is incompatible with option --F1");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F1b"))      {
			puts("Failed to parse command line: Option --mid is incompatible with option --F1b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F2"))       {
			puts("Failed to parse command line: Option --mid is incompatible with option --F2");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F2b"))      {
			puts("Failed to parse command line: Option --mid is incompatible with option --F2b");
			goto ERROR;
		}
	}
	if (esl_opt_IsUsed(go, "--default")) {
		if (esl_opt_IsUsed(go, "--max"))   {
			puts("Failed to parse command line: Option --default is incompatible with option --max");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nohmm")) {
			puts("Failed to parse command line: Option --default is incompatible with option --nohmm");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rfam"))  {
			puts("Failed to parse command line: Option --default is incompatible with option --rfam");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--FZ"))    {
			puts("Failed to parse command line: Option --default is incompatible with option --FZ");
			goto ERROR;
		}
	}
	if (esl_opt_IsUsed(go, "--rfam")) {
		if (esl_opt_IsUsed(go, "--max"))     {
			puts("Failed to parse command line: Option --rfam is incompatible with option --max");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nohmm"))   {
			puts("Failed to parse command line: Option --rfam is incompatible with option --nohmm");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--default")) {
			puts("Failed to parse command line: Option --rfam is incompatible with option --default");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--FZ"))      {
			puts("Failed to parse command line: Option --rfam is incompatible with option --FZ");
			goto ERROR;
		}
	}
	if (esl_opt_IsUsed(go, "--FZ")) {
		if (esl_opt_IsUsed(go, "--max"))     {
			puts("Failed to parse command line: Option --FZ is incompatible with option --max");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nohmm"))   {
			puts("Failed to parse command line: Option --FZ is incompatible with option --nohmm");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--default")) {
			puts("Failed to parse command line: Option --FZ is incompatible with option --default");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rfam"))    {
			puts("Failed to parse command line: Option --FZ is incompatible with option --rfam");
			goto ERROR;
		}
	}
	if (esl_opt_IsUsed(go, "--hmmonly")) {
		if (esl_opt_IsUsed(go, "--max"))        {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --max");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nohmm"))      {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --nohmm");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--mid"))        {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --mid");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--rfam"))       {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --rfam");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--FZ"))         {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --FZ");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF1"))       {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --noF1");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF2"))       {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --noF2");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF3"))       {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --noF3");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF4"))       {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --noF4");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF6"))       {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --noF6");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--doF1b"))      {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --doF1b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF2b"))      {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --noF2b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF3b"))      {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --noF3b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--noF4b"))      {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --noF4b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--doF5b"))      {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --doF5b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F1"))         {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --F1");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F1b"))        {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --F1b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F2"))         {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --F2");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F2b"))        {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --F2b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F3"))         {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --F3");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F3b"))        {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --F3b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F4"))         {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --F4");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F4b"))        {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --F4b");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F5"))         {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --F5");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--F6"))         {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --F6");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--ftau"))       {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --ftau");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--fsums"))      {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --fsums");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--fqdb"))       {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --fqdb");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--fbeta"))      {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --fbeta");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--fnonbanded")) {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --fnonbanded");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nocykenv"))   {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --nocykenv");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--cykenvx"))    {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --cykenvx");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--tau"))        {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --tau");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--sums"))       {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --sums");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--qdb"))        {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --qdb");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--beta"))       {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --beta");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nonbanded"))  {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --nonbanded");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--maxtau"))     {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --maxtau");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--anytrunc"))   {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --anytrunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--onlytrunc"))  {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --onlytrunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--5trunc"))     {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --5trunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--3trunc"))     {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --3trunc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--onepass"))    {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --onepass");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--mxsize"))     {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --mxsize");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--smxsize"))    {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --smxsize");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nonull3"))    {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --nonull3");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nohmmonly"))  {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --nohmmonly");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--timeF4"))     {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --timeF4");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--timeF5"))     {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --timeF5");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--timeF6"))     {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --timeF6");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--nogreedy"))   {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --nogreedy");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--cp9noel"))    {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --cp9noel");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--cp9gloc"))    {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --cp9gloc");
			goto ERROR;
		}
		if (esl_opt_IsUsed(go, "--null2"))      {
			puts("Failed to parse command line: Option --hmmonly is incompatible with option --null2");
			goto ERROR;
		}
	}
...
output_header(FILE *ofp, const ESL_GETOPTS *go, char *cmfile, char *seqfile, int ncpus)
{
	...
	if (esl_opt_IsUsed(go, "-g")) {
		fprintf(ofp, "# CM configuration:                      glocal\n");
	}
	if (esl_opt_IsUsed(go, "-Z")) {
		fprintf(ofp, "# database size is set to:               %.1f Mb\n",    esl_opt_GetReal(go, "-Z"));
	}
	if (esl_opt_IsUsed(go, "-o")) {
		fprintf(ofp, "# output directed to file:               %s\n",      esl_opt_GetString(go, "-o"));
	}
	if (esl_opt_IsUsed(go, "--tblout")) {
		fprintf(ofp, "# tabular output of hits:                %s\n",      esl_opt_GetString(go, "--tblout"));
	}
	if (esl_opt_IsUsed(go, "--fmt")) {
		fprintf(ofp, "# tabular output format:                 %d\n",      esl_opt_GetInteger(go, "--fmt"));
	}
	if (esl_opt_IsUsed(go, "--acc")) {
		fprintf(ofp, "# prefer accessions over names:          yes\n");
	}
	if (esl_opt_IsUsed(go, "--noali")) {
		fprintf(ofp, "# show alignments in output:             no\n");
	}
	if (esl_opt_IsUsed(go, "--notextw")) {
		fprintf(ofp, "# max ASCII text line length:            unlimited\n");
	}
	if (esl_opt_IsUsed(go, "--textw")) {
		fprintf(ofp, "# max ASCII text line length:            %d\n",             esl_opt_GetInteger(go, "--textw"));
	}
	if (esl_opt_IsUsed(go, "--verbose")) {
		fprintf(ofp, "# verbose output mode:                   on\n");
	}
	if (esl_opt_IsUsed(go, "-E")) {
		fprintf(ofp, "# sequence reporting threshold:          E-value <= %g\n",  esl_opt_GetReal(go, "-E"));
	}
	if (esl_opt_IsUsed(go, "-T")) {
		fprintf(ofp, "# sequence reporting threshold:          score >= %g\n",    esl_opt_GetReal(go, "-T"));
	}
	if (esl_opt_IsUsed(go, "--incE")) {
		fprintf(ofp, "# sequence inclusion threshold:          E-value <= %g\n",  esl_opt_GetReal(go, "--incE"));
	}
	if (esl_opt_IsUsed(go, "--incT")) {
		fprintf(ofp, "# sequence inclusion threshold:          score >= %g\n",    esl_opt_GetReal(go, "--incT"));
	}
	if (esl_opt_IsUsed(go, "--cut_ga")) {
		fprintf(ofp, "# model-specific thresholding:           GA cutoffs\n");
	}
	if (esl_opt_IsUsed(go, "--cut_nc")) {
		fprintf(ofp, "# model-specific thresholding:           NC cutoffs\n");
	}
	if (esl_opt_IsUsed(go, "--cut_tc")) {
		fprintf(ofp, "# model-specific thresholding:           TC cutoffs\n");
	}
	if (esl_opt_IsUsed(go, "--max")) {
		fprintf(ofp, "# Max sensitivity mode:                  on [all heuristic filters off]\n");
	}
	if (esl_opt_IsUsed(go, "--nohmm")) {
		fprintf(ofp, "# CM-only mode:                          on [HMM filters off]\n");
	}
	if (esl_opt_IsUsed(go, "--mid")) {
		fprintf(ofp, "# HMM MSV and Viterbi filters:           off\n");
	}
	if (esl_opt_IsUsed(go, "--rfam")) {
		fprintf(ofp, "# Rfam pipeline mode:                    on [strict filtering]\n");
	}
	if (esl_opt_IsUsed(go, "--FZ")) {
		fprintf(ofp, "# Filters set as if DB size in Mb is:    %f\n", esl_opt_GetReal(go, "--FZ"));
	}
	if (esl_opt_IsUsed(go, "--Fmid")) {
		fprintf(ofp, "# HMM Forward filter thresholds set to:  %g\n", esl_opt_GetReal(go, "--Fmid"));
	}
	if (esl_opt_IsUsed(go, "--hmmonly")) {
		fprintf(ofp, "# HMM-only mode (for all models):        on [CM will not be used]\n");
	}
	if (esl_opt_IsUsed(go, "--notrunc")) {
		fprintf(ofp, "# truncated sequence detection:          off\n");
	}
	if (esl_opt_IsUsed(go, "--anytrunc")) {
		fprintf(ofp, "# allowing truncated sequences anywhere: on\n");
	}
	if (esl_opt_IsUsed(go, "--onlytrunc")) {
		fprintf(ofp, "# only allowing truncated seqs anywhere: on\n");
	}
	if (esl_opt_IsUsed(go, "--nonull3")) {
		fprintf(ofp, "# null3 bias corrections:                off\n");
	}
	if (esl_opt_IsUsed(go, "--mxsize")) {
		fprintf(ofp, "# maximum DP alignment matrix size:      %.1f Mb\n", esl_opt_GetReal(go, "--mxsize"));
	}
	if (esl_opt_IsUsed(go, "--smxsize")) {
		fprintf(ofp, "# maximum DP search matrix size:         %.1f Mb\n", esl_opt_GetReal(go, "--smxsize"));
	}
	if (esl_opt_IsUsed(go, "--cyk")) {
		fprintf(ofp, "# use CYK for final search stage         on\n");
	}
	if (esl_opt_IsUsed(go, "--acyk")) {
		fprintf(ofp, "# use CYK to align hits:                 on\n");
	}
	if (esl_opt_IsUsed(go, "--wcx")) {
		fprintf(ofp, "# W set as <x> * cm->clen:               <x>=%g\n", esl_opt_GetReal(go, "--wcx"));
	}
	if (esl_opt_IsUsed(go, "--onepass")) {
		fprintf(ofp, "# using CM for best HMM pass only:       on\n");
	}
	if (esl_opt_IsUsed(go, "--toponly")) {
		fprintf(ofp, "# search top-strand only:                on\n");
	}
	if (esl_opt_IsUsed(go, "--bottomonly")) {
		fprintf(ofp, "# search bottom-strand only:             on\n");
	}
	if (esl_opt_IsUsed(go, "--qformat")) {
		fprintf(ofp, "# query <seqfile> format asserted:       %s\n", esl_opt_GetString(go,  "--qformat"));
	}
	if (esl_opt_IsUsed(go, "--glist")) {
		fprintf(ofp, "# models for glocal mode scan read from: %s\n", esl_opt_GetString(go,  "--glist"));
	}
	if (esl_opt_IsUsed(go, "--block")) {
		fprintf(ofp, "# block size (# models) set to:          %d\n", esl_opt_GetInteger(go, "--block"));
	}
	if (esl_opt_IsUsed(go, "--clanin")) {
		fprintf(ofp, "# clan information read from file:       %s\n", esl_opt_GetString(go,  "--clanin"));
	}
	if (esl_opt_IsUsed(go, "--oclan")) {
		fprintf(ofp, "# only mark overlaps within clans:       yes\n");
	}
	if (esl_opt_IsUsed(go, "--oskip")) {
		fprintf(ofp, "# skipping overlaps in tbl output:       yes\n");
	}
#ifdef HAVE_MPI
	if (esl_opt_IsUsed(go, "--stall")) {
		fprintf(ofp, "# MPI stall mode:                        on\n");
	}
#endif
	/* Developer options, only shown to user if --devhelp used */
	if (esl_opt_IsUsed(go, "--noF1")) {
		fprintf(ofp, "# HMM MSV filter:                        off\n");
	}
	if (esl_opt_IsUsed(go, "--noF2")) {
		fprintf(ofp, "# HMM Vit filter:                        off\n");
	}
	if (esl_opt_IsUsed(go, "--noF3")) {
		fprintf(ofp, "# HMM Fwd filter:                        off\n");
	}
	if (esl_opt_IsUsed(go, "--noF4")) {
		fprintf(ofp, "# HMM glocal Fwd filter:                 off\n");
	}
	if (esl_opt_IsUsed(go, "--noF6")) {
		fprintf(ofp, "# CM CYK filter:                         off\n");
	}
	if (esl_opt_IsUsed(go, "--doF1b")) {
		fprintf(ofp, "# HMM MSV biased comp filter:            on\n");
	}
	if (esl_opt_IsUsed(go, "--noF2b")) {
		fprintf(ofp, "# HMM Vit biased comp filter:            off\n");
	}
	if (esl_opt_IsUsed(go, "--noF3b")) {
		fprintf(ofp, "# HMM Fwd biased comp filter:            off\n");
	}
	if (esl_opt_IsUsed(go, "--noF4b")) {
		fprintf(ofp, "# HMM gFwd biased comp filter:           off\n");
	}
	if (esl_opt_IsUsed(go, "--doF5b")) {
		fprintf(ofp, "# HMM per-envelope biased comp filter:   on\n");
	}
	if (esl_opt_IsUsed(go, "--F1")) {
		fprintf(ofp, "# HMM MSV filter P threshold:            <= %g\n", esl_opt_GetReal(go, "--F1"));
	}
	if (esl_opt_IsUsed(go, "--F1b")) {
		fprintf(ofp, "# HMM MSV bias P threshold:              <= %g\n", esl_opt_GetReal(go, "--F1b"));
	}
	if (esl_opt_IsUsed(go, "--F2")) {
		fprintf(ofp, "# HMM Vit filter P threshold:            <= %g\n", esl_opt_GetReal(go, "--F2"));
	}
	if (esl_opt_IsUsed(go, "--F2b")) {
		fprintf(ofp, "# HMM Vit bias P threshold:              <= %g\n", esl_opt_GetReal(go, "--F2b"));
	}
	if (esl_opt_IsUsed(go, "--F3")) {
		fprintf(ofp, "# HMM Fwd filter P threshold:            <= %g\n", esl_opt_GetReal(go, "--F3"));
	}
	if (esl_opt_IsUsed(go, "--F3b")) {
		fprintf(ofp, "# HMM Fwd bias P threshold:              <= %g\n", esl_opt_GetReal(go, "--F3b"));
	}
	if (esl_opt_IsUsed(go, "--F4")) {
		fprintf(ofp, "# HMM glocal Fwd filter P threshold:     <= %g\n", esl_opt_GetReal(go, "--F4"));
	}
	if (esl_opt_IsUsed(go, "--F4b")) {
		fprintf(ofp, "# HMM glocal Fwd bias P threshold:       <= %g\n", esl_opt_GetReal(go, "--F4b"));
	}
	if (esl_opt_IsUsed(go, "--F5")) {
		fprintf(ofp, "# HMM env defn filter P threshold:       <= %g\n", esl_opt_GetReal(go, "--F5"));
	}
	if (esl_opt_IsUsed(go, "--F5b")) {
		fprintf(ofp, "# HMM env defn bias   P threshold:       <= %g\n", esl_opt_GetReal(go, "--F5b"));
	}
	if (esl_opt_IsUsed(go, "--F6")) {
		fprintf(ofp, "# CM CYK filter P threshold:             <= %g\n", esl_opt_GetReal(go, "--F6"));
	}

	if (esl_opt_IsUsed(go, "--hmmmax")) {
		fprintf(ofp, "# max sensitivity mode   (HMM-only):     on [all heuristic filters off]\n");
	}
	if (esl_opt_IsUsed(go, "--hmmF1")) {
		fprintf(ofp, "# HMM MSV filter P threshold (HMM-only)  <= %g\n", esl_opt_GetReal(go, "--hmmF1"));
	}
	if (esl_opt_IsUsed(go, "--hmmF2")) {
		fprintf(ofp, "# HMM Vit filter P threshold (HMM-only)  <= %g\n", esl_opt_GetReal(go, "--hmmF2"));
	}
	if (esl_opt_IsUsed(go, "--hmmF3")) {
		fprintf(ofp, "# HMM Fwd filter P threshold (HMM-only)  <= %g\n", esl_opt_GetReal(go, "--hmmF3"));
	}
	if (esl_opt_IsUsed(go, "--hmmnobias")) {
		fprintf(ofp, "# HMM MSV biased comp filter (HMM-only)  off\n");
	}
	if (esl_opt_IsUsed(go, "--hmmnonull2")) {
		fprintf(ofp, "# null2 bias corrections (HMM-only):     off\n");
	}
	if (esl_opt_IsUsed(go, "--nohmmonly")) {
		fprintf(ofp, "# HMM-only mode for 0 basepair models:   no\n");
	}

	if (esl_opt_IsUsed(go, "--rt1")) {
		fprintf(ofp, "# domain definition rt1 parameter        %g\n", esl_opt_GetReal(go, "--rt1"));
	}
	if (esl_opt_IsUsed(go, "--rt2")) {
		fprintf(ofp, "# domain definition rt2 parameter        %g\n", esl_opt_GetReal(go, "--rt2"));
	}
	if (esl_opt_IsUsed(go, "--rt3")) {
		fprintf(ofp, "# domain definition rt3 parameter        %g\n", esl_opt_GetReal(go, "--rt3"));
	}
	if (esl_opt_IsUsed(go, "--ns")) {
		fprintf(ofp, "# number of envelope tracebacks sampled  %d\n", esl_opt_GetInteger(go, "--ns"));
	}

	if (esl_opt_IsUsed(go, "--ftau")) {
		fprintf(ofp, "# tau parameter for CYK filter stage:    %g\n", esl_opt_GetReal(go, "--ftau"));
	}
	if (esl_opt_IsUsed(go, "--fsums")) {
		fprintf(ofp, "# posterior sums (CYK filter stage):     on\n");
	}
	if (esl_opt_IsUsed(go, "--fqdb")) {
		fprintf(ofp, "# QDBs (CYK filter stage)                on\n");
	}
	if (esl_opt_IsUsed(go, "--fbeta")) {
		fprintf(ofp, "# beta parameter for CYK filter stage:   %g\n", esl_opt_GetReal(go, "--fbeta"));
	}
	if (esl_opt_IsUsed(go, "--fnonbanded")) {
		fprintf(ofp, "# no bands (CYK filter stage)            on\n");
	}
	if (esl_opt_IsUsed(go, "--nocykenv")) {
		fprintf(ofp, "# CYK envelope redefinition:             off\n");
	}
	if (esl_opt_IsUsed(go, "--cykenvx")) {
		fprintf(ofp, "# CYK envelope redefn P-val multiplier:  %d\n", esl_opt_GetInteger(go, "--cykenvx"));
	}

	if (esl_opt_IsUsed(go, "--tau")) {
		fprintf(ofp, "# tau parameter for final stage:         %g\n", esl_opt_GetReal(go, "--tau"));
	}
	if (esl_opt_IsUsed(go, "--sums")) {
		fprintf(ofp, "# posterior sums (final stage):          on\n");
	}
	if (esl_opt_IsUsed(go, "--qdb")) {
		fprintf(ofp, "# QDBs (final stage)                     on\n");
	}
	if (esl_opt_IsUsed(go, "--beta")) {
		fprintf(ofp, "# beta parameter for final stage:        %g\n", esl_opt_GetReal(go, "--beta"));
	}
	if (esl_opt_IsUsed(go, "--nonbanded")) {
		fprintf(ofp, "# no bands (final stage)                 on\n");
	}

	if (esl_opt_IsUsed(go, "--timeF1")) {
		fprintf(ofp, "# abort after Stage 1 MSV (for timing)   on\n");
	}
	if (esl_opt_IsUsed(go, "--timeF2")) {
		fprintf(ofp, "# abort after Stage 2 Vit (for timing)   on\n");
	}
	if (esl_opt_IsUsed(go, "--timeF3")) {
		fprintf(ofp, "# abort after Stage 3 Fwd (for timing)   on\n");
	}
	if (esl_opt_IsUsed(go, "--timeF4")) {
		fprintf(ofp, "# abort after Stage 4 gFwd (for timing)  on\n");
	}
	if (esl_opt_IsUsed(go, "--timeF5")) {
		fprintf(ofp, "# abort after Stage 5 env defn (for timing) on\n");
	}
	if (esl_opt_IsUsed(go, "--timeF6")) {
		fprintf(ofp, "# abort after Stage 6 CYK (for timing)   on\n");
	}

	if (esl_opt_IsUsed(go, "--trmF3")) {
		fprintf(ofp, "# terminate after Stage 3 Fwd:           on\n");
	}

	if (esl_opt_IsUsed(go, "--nogreedy")) {
		fprintf(ofp, "# greedy CM hit resolution:              off\n");
	}
	if (esl_opt_IsUsed(go, "--cp9noel")) {
		fprintf(ofp, "# CP9 HMM local ends:                    off\n");
	}
	if (esl_opt_IsUsed(go, "--cp9gloc")) {
		fprintf(ofp, "# CP9 HMM configuration:                 glocal\n");
	}
	if (esl_opt_IsUsed(go, "--null2")) {
		fprintf(ofp, "# null2 bias corrections:                on\n");
	}
	if (esl_opt_IsUsed(go, "--maxtau")) {
		fprintf(ofp, "# max tau during band tightening:        %g\n", esl_opt_GetReal(go, "--maxtau"));
	}
	if (esl_opt_IsUsed(go, "--seed"))  {
		if (esl_opt_GetInteger(go, "--seed") == 0) {
			fprintf(ofp, "# random number seed:                    one-time arbitrary\n");
		} else {
			fprintf(ofp, "# random number seed set to:             %d\n", esl_opt_GetInteger(go, "--seed"));
		}
	}
	...
	if (!(esl_opt_IsUsed(go, "--notrunc"))) {
		if (esl_opt_IsUsed(go, "--max"))        {
			fprintf(ofp, "# truncated hit detection:               off [due to --max]\n");
		}
		if (esl_opt_IsUsed(go, "--nohmm"))      {
			fprintf(ofp, "# truncated hit detection:               off [due to --nohmm]\n");
		}
	}
	/* output number of processors being used, always (this differs from H3 which only does this if --cpu) */
	int output_ncpu = FALSE;
#ifdef HAVE_MPI
	if (esl_opt_IsUsed(go, "--mpi"))         {
		fprintf(ofp, "# MPI:                                   on [%d processors]\n", ncpus);
		output_ncpu = TRUE;
	}
#endif
#ifdef HMMER_THREADS
	if (! output_ncpu)                       {
		fprintf(ofp, "# number of worker threads:              %d%s\n", ncpus, (esl_opt_IsUsed(go, "--cpu") ? " [--cpu]" : ""));
		output_ncpu = TRUE;
	}
#endif
	if (! output_ncpu)                       {
		fprintf(ofp, "# number of worker threads:              0 [serial mode; threading unavailable]\n");
	}
	fprintf(ofp, "# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n\n");
	return eslOK;
}
...
read_glocal_list_file(char *filename, char *errbuf, CM_FILE *cmfp, ESL_KEYHASH **ret_glocal_kh)
{
	...
	if (cmfp->ssi == NULL) {
		ESL_XFAIL(eslEINVAL, errbuf, "Failed to open SSI index for CM file: %s (required for --gfile)\n", cmfp->fname);
	}
	...
	if (esl_fileparser_Open(filename, NULL,  &efp) != eslOK) {
		ESL_XFAIL(eslEINVAL, errbuf, "failed to open %s for reading glocal CM names\n", filename);
	}
	if ((glocal_kh = esl_keyhash_Create()) == NULL) {
		ESL_XFAIL(eslEMEM, errbuf, "out of memory");
	}
	...
	while ((status = esl_fileparser_GetToken(efp, &tok, &toklen)) != eslEOF) {
		...
		if (status == eslENOTFOUND) {
			ESL_XFAIL(status, errbuf, "unable to find model %s listed in %s in SSI index file", tok, filename);
		}
		if (status == eslEFORMAT) {
			ESL_XFAIL(status, errbuf, "SSI index for CM file is in incorrect format, try rerunning cmpress");
		}
		if (status != eslOK) {
			ESL_XFAIL(status, errbuf, "unexpected error processing --glist file %s", filename);
		}

		/* add to keyhash */
		status = esl_keyhash_Store(glocal_kh, tok, toklen, NULL);
		if (status == eslEDUP) {
			ESL_XFAIL(status, errbuf, "model %s listed twice in %s", tok, filename);
		} else if (status != eslOK) {
			ESL_XFAIL(status, errbuf, "unexpected error processing --glist file %s", filename);
		}
	}
...
read_clan_info_file(char *filename, char *errbuf, CM_FILE *cmfp, ESL_KEYHASH **ret_clan_name_kh, ESL_KEYHASH **ret_clan_fam_kh, int **ret_clan_mapA)
{
	...
	if (esl_fileparser_Open(filename, NULL,  &efp) != eslOK) {
		ESL_XFAIL(eslEINVAL, errbuf, "failed to open %s for reading clan information\n", filename);
	}

	if ((clan_name_kh = esl_keyhash_Create()) == NULL) {
		ESL_XFAIL(eslEMEM, errbuf, "out of memory");
	}
	if ((clan_fam_kh  = esl_keyhash_Create()) == NULL) {
		ESL_XFAIL(eslEMEM, errbuf, "out of memory");
	}
	...
	while ((status = esl_fileparser_GetToken(efp, &tok, &toklen)) != eslEOF) {
		...
		while ((status = esl_fileparser_GetTokenOnLine(efp, &tok, &toklen)) == eslOK) {
			...
			if (status != eslOK) {
				ESL_XFAIL(status, errbuf, "unexpected error processing --clanin file %s", filename);
			}
			...
			if (nfam == tot_nalloc) {
				...
				for (i = tot_nalloc; i < tot_nalloc + nalloc; i++) {
					clan_mapA[i] = -1;
				}
				tot_nalloc += nalloc;
			}
	...
	if (nclan == 0) {
		ESL_FAIL(status, errbuf, "Error reading %s, no clans present in file\n", filename);
	}
...
create_reader_info(int64_t nmodels, ESL_KEYHASH *clan_fam_kh, int *clan_mapA)
{
	...
	if (clan_fam_kh != NULL && clan_mapA == NULL) {
		esl_fatal("internal error, creating reader info but some but not all clan info present");
	}
	if (clan_fam_kh == NULL && clan_mapA != NULL) {
		esl_fatal("internal error, creating reader info but some but not all clan info present");
	}
	...
	if (have_clans) {
		if ((rinfo->clan_fam_kh = esl_keyhash_Clone(clan_fam_kh)) == NULL) {
			esl_fatal("Failed to clone keyhash, out of memory");
		}
	...
	if (rinfo != NULL) {
		...
		if (rinfo->omA != NULL) {
			for (cm_idx = 0; cm_idx < rinfo->nmodels; cm_idx++) {
				if (rinfo->omA[cm_idx] != NULL) {
					p7_oprofile_Destroy(rinfo->omA[cm_idx]);
					rinfo->omA[cm_idx] = NULL;
				}
			}
			free(rinfo->omA);
		...
		if (rinfo->msvdataA != NULL) {
			for (cm_idx = 0; cm_idx < rinfo->nmodels; cm_idx++) {
				if (rinfo->msvdataA[cm_idx] != NULL) {
					p7_hmm_ScoreDataDestroy(rinfo->msvdataA[cm_idx]);
					rinfo->msvdataA[cm_idx] = NULL;
				}
			}
			free(rinfo->msvdataA);
		...
		if (rinfo->cm_offsetA != NULL) {
			free(rinfo->cm_offsetA);
			rinfo->cm_offsetA = NULL;
		}
		if (rinfo->cm_clenA   != NULL) {
			free(rinfo->cm_clenA);
			rinfo->cm_clenA   = NULL;
		}
		if (rinfo->cm_WA      != NULL) {
		...
		if (rinfo->gflambdaA  != NULL) {
			free(rinfo->gflambdaA);
			rinfo->gflambdaA  = NULL;
		}
		if (rinfo->abc        != NULL) {
			esl_alphabet_Destroy(rinfo->abc);
			rinfo->abc = NULL;
		}

		/* clan info variables */
		if (rinfo->clan_fam_kh  != NULL) {
			esl_keyhash_Destroy(rinfo->clan_fam_kh);
			rinfo->clan_fam_kh  = NULL;
		}
		if (rinfo->clan_mapA    != NULL) {
			free(rinfo->clan_mapA);
			rinfo->clan_mapA    = NULL;
		}
		if (rinfo->clan_idxA    != NULL) {
			free(rinfo->clan_idxA);
			rinfo->clan_idxA    = NULL;
		}
...
int mpi_next_block(CM_FILE *cmfp, BLOCK_LIST *list, int64_t bsize, uint64_t idx0, MSV_BLOCK *block)
{
	...
	block->count  = 0;
	block->idx0   = idx0;
	if ((prv_offset = ftello(cmfp->ffp)) < 0) {
		return eslESYS;
	}
	...
	if (status == eslEOF && block->count > 0) {
		status = eslOK;
	}
	if (status == eslEOF) {
		list->complete = 1;
