...
serial_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if ((status = open_dbfile(go, cfg, errbuf, &dbfp)) != eslOK) {
		esl_fatal(errbuf);
	}

	/* Open the query CM file */
	if ((status = cm_file_Open(cfg->cmfile, NULL, FALSE, &(cmfp), errbuf)) != eslOK) {
		cm_Fail(errbuf);
	}
	...
	if (esl_opt_IsOn(go, "-o"))           {
		if ((ofp       = fopen(esl_opt_GetString(go, "-o"),          "w")) == NULL) {
			cm_Fail("Failed to open output file %s for writing\n",         esl_opt_GetString(go, "-o"));
		}
	}
	if (esl_opt_IsOn(go, "-A"))           {
		if ((afp       = fopen(esl_opt_GetString(go, "-A"),          "w")) == NULL) {
			cm_Fail("Failed to open alignment file %s for writing\n", esl_opt_GetString(go, "-A"));
		}
	}
	if (esl_opt_IsOn(go, "--tblout"))     {
		if ((tblfp     = fopen(esl_opt_GetString(go, "--tblout"),    "w")) == NULL) {
			cm_Fail("Failed to open tabular output file %s for writing\n", esl_opt_GetString(go, "--tblout"));
		}
	}
	...
	while (qhstatus == eslOK) {
		...
		if (!(tinfo->cm->flags & CMH_FP7)) {
			cm_Fail("no filter HMM was read for CM: %s\n", tinfo->cm->name);
		}
		...
		if ((esl_opt_GetBoolean(go, "--nohmmonly"))  ||
			...
			if (!(tinfo->cm->flags & CMH_EXPTAIL_STATS)) {
				cm_Fail("no E-value parameters were read for CM: %s\n", tinfo->cm->name);
			}
		}
		...
		if ((status = configure_cm(tinfo))         != eslOK) {
			cm_Fail(tinfo->pli->errbuf);
		}
		if ((status = setup_hmm_filter(go, tinfo)) != eslOK) {
			cm_Fail(tinfo->pli->errbuf);
		}
		/* Clone all data structures in tinfo into the WORKER_INFO's in info */
		if ((status = clone_info(go, tinfo, info, infocnt, errbuf)) != eslOK) {
			cm_Fail(errbuf);
		}
...
#ifdef HMMER_THREADS
		if (ncpus > 0) {
			sstatus = thread_loop(info, threadObj, queue, dbfp, srcL);
		} else {
		...
		for (i = 0; i < infocnt; ++i) {
			if (info[i].pli->do_hmmonly_cur) {
				eZ = info[i].pli->Z / (float) info[i].om->max_length;
			} else {
				eZ = info[i].cm->expA[info[i].pli->final_cm_exp_mode]->cur_eff_dbsize;
			}
		...
		if ((status = cm_tophits_RemoveOrMarkOverlaps(info[0].th, FALSE, errbuf)) != eslOK) {
			cm_Fail(errbuf);
		}
		...
		if (info[0].pli->show_alignments) {
			if ((status = cm_tophits_HitAlignments(ofp, info[0].th, info[0].pli, textw)) != eslOK) {
	...
	case eslEOF:       /* do nothing. EOF is what we want. */
		break;
	default:
		cm_Fail("Unexpected error (%d) in reading CMs from %s\n%s", qhstatus, cfg->cmfile, cmfp->errbuf);
	}
	 ...
	 */
	if (tblfp) {
		cm_tophits_TabularTail(tblfp,    "cmsearch", CM_SEARCH_SEQS, cfg->cmfile, cfg->dbfile, go);
	}
	fprintf(ofp, "[ok]\n");
...
serial_loop(WORKER_INFO *info, ESL_SQFILE *dbfp, int64_t *srcL)
{
	...
	while (wstatus == eslOK) {
		...
		if (info->pli->do_top) {
			...
			if (dbsq->C > 0) {
				cm_pli_AdjustNresForOverlaps(info->pli, dbsq->C, FALSE);    /* 'FALSE': we're not on bottom strand */
			}
		...
		if (info->pli->do_bot && dbsq->abc->complement != NULL) {
			...
			/* subtract overlapping residues from previous window */
			if (dbsq->C > 0) {
				cm_pli_AdjustNresForOverlaps(info->pli, dbsq->C, TRUE);    /* 'TRUE': we are on bottom strand */
			}
	...
	while (block->count > 0) {
		...
		for (i = 0; i < block->count; ++i) {
			...
			if (info->pli->do_bot && dbsq->abc->complement != NULL) {
				...
				/* subtract overlapping residues from previous window */
				if (dbsq->C > 0) {
					cm_pli_AdjustNresForOverlaps(info->pli, dbsq->C, TRUE);    /* 'TRUE': we're not on bottom strand */
				}
...
mpi_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if (hstatus == eslOK) {
		 ...
		 * unless -Z is set.
		 */
		if ((status = mpi_open_dbfile_ssi(go, cfg, dbfp, errbuf)) != eslOK) {
			mpi_failure(errbuf);
		}
		if (esl_opt_IsUsed(go, "-Z")) {
			...
			esl_stopwatch_Start(w);
			if ((status = mpi_dbsize_using_ssi(go, cfg, dbfp, errbuf)) != eslOK) {
				mpi_failure(errbuf);
			}
			esl_stopwatch_Stop(w);
			if (esl_opt_GetBoolean(go, "--verbose")) {
	...
	while (hstatus == eslOK) {
		...
		/* Sanity check: we better have a filter HMM */
		if (!(info->cm->flags & CMH_FP7)) {
			mpi_failure("no filter HMM was read for CM: %s\n", info->cm->name);
		}
		...
		if ((esl_opt_GetBoolean(go, "--nohmmonly"))  ||
		        ...
		        ((! esl_opt_GetBoolean(go, "--hmmonly"))    && (nbps > 0))) {
			/* we're NOT running HMM-only pipeline variant, we need CM E-value stats */
			if (!(info->cm->flags & CMH_EXPTAIL_STATS)) {
				mpi_failure("no E-value parameters were read for CM: %s\n", info->cm->name);
			}
		}
		...
		if ((status = configure_cm(info))         != eslOK) {
			mpi_failure(info->pli->errbuf);
		}
		if ((status = setup_hmm_filter(go, info)) != eslOK) {
			mpi_failure(info->pli->errbuf);
		}
		...
		if (info->pli->do_top && tot_noverlap > 0) {
			cm_pli_AdjustNresForOverlaps(info->pli, tot_noverlap, FALSE);    /* 'FALSE': we're not on bottom strand */
		}
		if (info->pli->do_bot && tot_noverlap > 0) {
			cm_pli_AdjustNresForOverlaps(info->pli, tot_noverlap, TRUE);    /* 'TRUE':  we are on bottom strand */
		}

		/* Sort by sequence index/position and remove duplicates */
		cm_tophits_SortForOverlapRemoval(info->th);
		if ((status = cm_tophits_RemoveOrMarkOverlaps(info->th, FALSE, errbuf)) != eslOK) {
			mpi_failure(errbuf);
		}
		...
		if (info->pli->show_alignments) {
			if ((status = cm_tophits_HitAlignments(ofp, info->th, info->pli, textw)) != eslOK) {
				mpi_failure("Out of memory");
			}
			fprintf(ofp, "\n\n");
		...
		if ((info = create_info(go)) == NULL) {
			mpi_failure("Out of memory");    /* for the next model */
		}
	...
	case eslEOF:       /* do nothing. EOF is what we want. */
		break;
	default:
		mpi_failure("Unexpected error (%d) in reading CMs from %s\n%s", hstatus, cfg->cmfile, cmfp->errbuf);
	}
...
mpi_worker(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	while (hstatus == eslOK) {
		...
		if ((status = mpi_block_recv(0, INFERNAL_BLOCK_TAG, MPI_COMM_WORLD, &mpi_buf, &mpi_size, &block)) != eslOK) {
			mpi_failure("Failed to receive sequence block, error status code: %d\n", status);
		}
		...
		while (block->first_idx != -1) { /* receipt of a block with first_idx == -1 signals us that we're done with the database */
			...
			while (pkey_idx <= block->final_idx) {
				...
				if ((status = esl_ssi_FindNumber(dbfp->data.ascii.ssi, pkey_idx, NULL, NULL, NULL, &L, &pkey)) != eslOK) {
					if (status == eslENOTFOUND) {
						mpi_failure("(worker) unable to find sequence %d in SSI index file", pkey_idx);
					}
					if (status == eslEFORMAT) {
						mpi_failure("(worker) SSI index for database file is in incorrect format.");
					}
					if (status != eslOK) {
						mpi_failure("(worker) problem with SSI index for database file.");
					}
				}
				...
				if ((pkey_idx != block->first_idx) && (pkey_idx != block->final_idx)) {
					/* read full sequence */
					if ((status = esl_sqio_Fetch(dbfp, pkey, dbsq)) != eslOK) {
						mpi_failure("unable to fetch sequence %s\n", pkey);
					}
					seq_from = 1;
					seq_to   = L;
					...
					if ((status = esl_sqio_FetchSubseq(dbfp, pkey, seq_from, seq_to, dbsq)) != eslOK) {
						mpi_failure("unable to fetch subsequence %ld..%ld of %s\n", seq_from, seq_to, pkey);
					}
				...
				if (info->pli->do_top) {
					...
					if (seq_from != 1) {
						cm_tophits_UpdateHitPositions(info->th, prv_pli_ntophits, seq_from, FALSE);
					}
				}
			...
			if (block->blockL != readL) {
				mpi_failure("Block length mismatch - expected %ld found %ld for block idx:%ld from:%ld\n", block->blockL, readL, block->first_idx, block->first_from);
			}
		...
		if (info->pli->do_hmmonly_cur) {
			eZ = info->pli->Z / (float) info->om->max_length;
		} else {
			eZ = info->cm->expA[info->pli->final_cm_exp_mode]->cur_eff_dbsize;
		}
		...
		if ((info = create_info(go)) == NULL) {
			mpi_failure("Out of memory");    /* info is for the next model */
		}
...
process_commandline(int argc, char **argv, ESL_GETOPTS **ret_go, char **ret_cmfile, char **ret_seqfile)
{
	...
	if ((*ret_cmfile = esl_opt_GetArg(go, 1))  == NULL)  {
		puts("Failed to get <cmfile> argument on command line");
		goto ERROR;
	}
	if ((*ret_seqfile = esl_opt_GetArg(go, 2)) == NULL)  {
		puts("Failed to get <seqdb> argument on command line");
		goto ERROR;
	}

	if (strcmp(*ret_seqfile, "-") == 0) {
		puts("Can't read <seqdb> from stdout: don't use '-'");
		goto ERROR;
	}
...
output_header(FILE *ofp, const ESL_GETOPTS *go, char *cmfile, char *seqfile, int ncpus)
{
	...
	if (esl_opt_IsUsed(go, "-A")) {
		fprintf(ofp, "# MSA of significant hits saved to file: %s\n",             esl_opt_GetString(go, "-A"));
	}
	...
	if (esl_opt_IsUsed(go, "--5trunc")) {
		fprintf(ofp, "# allowing 5' truncated seqs only:       on\n");
	}
	if (esl_opt_IsUsed(go, "--3trunc")) {
		fprintf(ofp, "# allowing 3' truncated seqs only:       on\n");
	}
	if (esl_opt_IsUsed(go, "--nonull3")) {
	...
	if (esl_opt_IsUsed(go, "--tformat")) {
		fprintf(ofp, "# targ <seqdb> format asserted:          %s\n", esl_opt_GetString(go, "--tformat"));
	}
#ifdef HAVE_MPI
...
#ifdef HAVE_MPI
	if (esl_opt_IsUsed(go, "--sidx")) {
		fprintf(ofp, "# first target sequence to search:       %d\n", esl_opt_GetInteger(go, "--sidx"));
	}
	if (esl_opt_IsUsed(go, "--eidx")) {
		fprintf(ofp, "# final target sequence to search:       %d\n", esl_opt_GetInteger(go, "--eidx"));
	}
#endif
...
open_dbfile(ESL_GETOPTS *go, struct cfg_s *cfg, char *errbuf, ESL_SQFILE **ret_dbfp)
{
	...
	if (esl_opt_IsOn(go, "--tformat")) {
		...
		if (dbfmt == eslSQFILE_UNKNOWN) {
			ESL_FAIL(eslEINVAL, errbuf, "%s is not a recognized sequence database file format\n", esl_opt_GetString(go, "--tformat"));
		}
	}
	status = esl_sqfile_Open(cfg->dbfile, dbfmt, p7_SEQDBENV, ret_dbfp);
	if (status == eslENOTFOUND) {
		ESL_FAIL(status, errbuf, "Failed to open sequence file %s for reading\n",          cfg->dbfile);
	} else if (status == eslEFORMAT) {
		ESL_FAIL(status, errbuf, "Sequence file %s is empty or misformatted\n",            cfg->dbfile);
	} else if (status == eslEINVAL) {
		ESL_FAIL(status, errbuf, "Can't autodetect format of a stdin or .gz seqfile");
	} else if (status != eslOK) {
		ESL_FAIL(status, errbuf, "Unexpected error %d opening sequence file %s\n", status, cfg->dbfile);
	}
...
dbsize_and_seq_lengths(ESL_GETOPTS *go, struct cfg_s *cfg, ESL_SQFILE **dbfp_ptr, char *errbuf, int64_t **ret_srcL, int64_t *ret_nseqs)
{
	...
	while ((status = esl_sqio_ReadInfo(*dbfp_ptr, sq)) == eslOK) {
		...
		if (nseqs == nalloc_srcL) { /* reallocate */
			...
			for (i = nalloc_srcL - alloc_srcL; i < nalloc_srcL; i++) {
				srcL[i] = -1;    /* initialize */
			}
		}
		srcL[nseqs++] = sq->L;
	...
	if ((*dbfp_ptr)->data.ascii.do_gzip == TRUE) {
		...
		*/
		...
		if (status == eslENOTFOUND) {
			ESL_EXCEPTION(eslENOTFOUND, "failed to reopen alignment file");
		} else if (status != eslOK) {
			ESL_FAIL(status, errbuf, "unexpected error when reopening sequence file");
		}
	} else {
...
ERROR:
	if ((*dbfp_ptr)->data.ascii.do_gzip == TRUE) { /* same as above, close and reopen */
		...
		if (status == eslENOTFOUND) {
			...
			ESL_XFAIL(status, errbuf, "unexpected error when reopening sequence file");
		}
	} else {
		/* file is not gzipped, easier case */
		esl_sqfile_Position((*dbfp_ptr), 0);
...
clone_info(ESL_GETOPTS *go, WORKER_INFO *src_info, WORKER_INFO *dest_infoA, int dest_infocnt, char *errbuf)
{
	...
	for (i = 0; i < dest_infocnt; ++i) {
		if ((status = cm_Clone(src_info->cm, errbuf, &(dest_infoA[i].cm))) != eslOK) {
			return status;
		}
		if ((dest_infoA[i].gm  = p7_profile_Clone(src_info->gm)) == NULL) {
			goto ERROR;
		}
		dest_infoA[i].Rgm = dest_infoA[i].Lgm = dest_infoA[i].Tgm = NULL; /* changed below if nec */
		if (src_info->Rgm != NULL) {
			if ((dest_infoA[i].Rgm = p7_profile_Clone(src_info->Rgm)) == NULL) {
				goto ERROR;
			}
		}
		if (src_info->Lgm != NULL) {
			if ((dest_infoA[i].Lgm = p7_profile_Clone(src_info->Lgm)) == NULL) {
				goto ERROR;
			}
		}
		if (src_info->Tgm != NULL) {
			if ((dest_infoA[i].Tgm = p7_profile_Clone(src_info->Tgm)) == NULL) {
				goto ERROR;
			}
		}
		if ((dest_infoA[i].om  = p7_oprofile_Clone(src_info->om)) == NULL) {
			goto ERROR;
		}
		if ((dest_infoA[i].bg  = p7_bg_Create(src_info->bg->abc)) == NULL) {
			goto ERROR;
		}
		if (dest_infoA[i].p7_evparam == NULL) {
			ESL_ALLOC(dest_infoA[i].p7_evparam, sizeof(float) * CM_p7_NEVPARAM);
		}
...
configure_cm(WORKER_INFO *info)
{
	...
	if (info->pli->cm_config_opts & CM_CONFIG_SCANMX) {
		reqMb += cm_scan_mx_SizeNeeded(info->cm, TRUE, TRUE);
	}
	if (info->pli->cm_config_opts & CM_CONFIG_TRSCANMX) {
		reqMb += cm_tr_scan_mx_SizeNeeded(info->cm, TRUE, TRUE);
	}
	...
	if ((status = cm_Configure(info->cm, info->pli->errbuf, W_from_cmdline)) != eslOK) {
		return status;
	}
...
mpi_open_dbfile_ssi(ESL_GETOPTS *go, struct cfg_s *cfg, ESL_SQFILE *dbfp, char *errbuf)
{
	...
	if (dbfp->data.ascii.do_gzip) {
		ESL_FAIL(status, errbuf, "Reading gzipped sequence files is not supported with --mpi.");
	}
	if (dbfp->data.ascii.do_stdin) {
		ESL_FAIL(status, errbuf, "Reading sequence files from stdin is not supported with --mpi.");
	}
	status = esl_sqfile_OpenSSI(dbfp, NULL);
	if (status == eslEFORMAT) {
		ESL_FAIL(status, errbuf, "SSI index for database file is in incorrect format\n");
	} else if (status == eslERANGE) {
		ESL_FAIL(status, errbuf, "SSI index for database file is in 64-bit format and we can't read it\n");
	} else if (status != eslOK) {
...
mpi_dbsize_using_ssi(ESL_GETOPTS *go, struct cfg_s *cfg, ESL_SQFILE *dbfp, char *errbuf)
{
	...
	if (dbfp->data.ascii.ssi == NULL) {
		ESL_FAIL(status, errbuf, "SSI index failed to open");
	}
	...
	for (i = 0; i < dbfp->data.ascii.ssi->nprimary; i++) {
		...
		if (status == eslENOTFOUND) {
			ESL_FAIL(status, errbuf, "unable to find sequence %d in SSI index file, try re-indexing with esl-sfetch.", i);
		}
		if (status == eslEFORMAT) {
			ESL_FAIL(status, errbuf, "SSI index for database file is in incorrect format.");
		}
		if (status != eslOK) {
			ESL_FAIL(status, errbuf, "problem with SSI index for database file.");
		}
		cfg->Z += L;
	}
	if ((! esl_opt_GetBoolean(go, "--toponly")) &&
...
mpi_add_blocks(ESL_SQFILE *dbfp, ESL_SQ *sq, int64_t ncontext, char *errbuf, int64_t final_pkey_idx, MPI_BLOCK_LIST *block_list, int64_t *ret_pkey_idx, int64_t *ret_noverlap, int64_t *ret_nseq)
{
	...
	if (dbfp->data.ascii.ssi == NULL) {
		ESL_FAIL(status, errbuf, "No SSI index available (it should've been opened earlier)");
	}
	...
	while ((pkey_idx <= final_pkey_idx) && (! block_list->blocks[0]->complete)) {
		...
		nseq++;
		if (block_list == NULL) {
			ESL_FAIL(eslFAIL, errbuf, "error parsing database in add_blocks()");
		}
		pkey_idx++;
		tot_noverlap += noverlap;
	}
...
mpi_inspect_next_sequence_using_ssi(ESL_SQFILE *dbfp, ESL_SQ *sq, int64_t ncontext, int64_t pkey_idx, char *errbuf, MPI_BLOCK_LIST *block_list, int64_t *ret_noverlap)
{
	...
	/* Contract check */
	if (dbfp->data.ascii.ssi == NULL) {
		ESL_FAIL(eslFAIL, errbuf, "No SSI index available (it should've been opened earlier)");
	}
	...
	*/
	if (status == eslENOTFOUND) {
		ESL_FAIL(status, errbuf, "unable to find sequence %ld in SSI index file, try re-indexing with esl-sfetch.", pkey_idx);
	}
	if (status == eslEFORMAT) {
		ESL_FAIL(status, errbuf, "SSI index for database file is in incorrect format.");
	}
...
mpi_block_send(MPI_BLOCK *block, int dest, int tag, MPI_Comm comm, char **buf, int *nalloc)
{
	...
	if ((status = mpi_block_pack_size(block, comm, &n)) != eslOK) {
		goto ERROR;
	}
	...
	pos  = 0;
	if ((status = mpi_block_pack(block, *buf, n, &pos, comm)) != eslOK) {
		goto ERROR;
	}

	/* Send the packed MPI_BLOCK to the destination. */
	if (MPI_Send(*buf, n, MPI_PACKED, dest, tag, comm) != 0) {
		ESL_EXCEPTION(eslESYS, "mpi send failed");
	}
...
mpi_block_recv(int source, int tag, MPI_Comm comm, char **buf, int *nalloc, MPI_BLOCK **ret_block)
{
	...
	pos = 0;
	if ((status = mpi_block_unpack(*buf, n, &pos, comm, ret_block)) != eslOK) {
		goto ERROR;
	}
...
mpi_block_pack_size(MPI_BLOCK *block, MPI_Comm comm, int *ret_n)
{
	...
	if (MPI_Pack_size(1, MPI_LONG_LONG_INT, comm, &sz) != 0) {
	...
	n += sz;  /* blockL     */
	if (MPI_Pack_size(1, MPI_INT,           comm, &sz) != 0) {
		ESL_XEXCEPTION(eslESYS, "pack size failed");
	}
	n += sz;  /* complete  */
...
mpi_block_pack(MPI_BLOCK *block, char *buf, int n, int *pos, MPI_Comm comm)
{
	...
	if (MPI_Pack(&block->first_idx,  1, MPI_LONG_LONG_INT, buf, n, pos, comm) != 0) {
		ESL_XEXCEPTION(eslESYS, "pack failed");
	}
	if (MPI_Pack(&block->final_idx,  1, MPI_LONG_LONG_INT, buf, n, pos, comm) != 0) {
		ESL_XEXCEPTION(eslESYS, "pack failed");
	}
	if (MPI_Pack(&block->first_from, 1, MPI_LONG_LONG_INT, buf, n, pos, comm) != 0) {
		ESL_XEXCEPTION(eslESYS, "pack failed");
	}
	if (MPI_Pack(&block->final_to,   1, MPI_LONG_LONG_INT, buf, n, pos, comm) != 0) {
		ESL_XEXCEPTION(eslESYS, "pack failed");
	}
	if (MPI_Pack(&block->blockL,     1, MPI_LONG_LONG_INT, buf, n, pos, comm) != 0) {
		ESL_XEXCEPTION(eslESYS, "pack failed");
	}
	if (MPI_Pack(&block->complete,   1, MPI_INT,           buf, n, pos, comm) != 0) {
		ESL_XEXCEPTION(eslESYS, "pack failed");
	}

	if (*pos > n) {
		ESL_EXCEPTION(eslEMEM, "buffer overflow");
	}
	return eslOK;
...
mpi_block_unpack(char *buf, int n, int *pos, MPI_Comm comm, MPI_BLOCK **ret_block)
{
	...
	if (MPI_Unpack(buf, n, pos, &block->first_idx,  1, MPI_LONG_LONG_INT, comm) != 0) {
		ESL_XEXCEPTION(eslESYS, "mpi unpack failed");
	}
	if (MPI_Unpack(buf, n, pos, &block->final_idx,  1, MPI_LONG_LONG_INT, comm) != 0) {
		ESL_XEXCEPTION(eslESYS, "mpi unpack failed");
	}
	if (MPI_Unpack(buf, n, pos, &block->first_from, 1, MPI_LONG_LONG_INT, comm) != 0) {
		ESL_XEXCEPTION(eslESYS, "mpi unpack failed");
	}
	if (MPI_Unpack(buf, n, pos, &block->final_to,   1, MPI_LONG_LONG_INT, comm) != 0) {
		ESL_XEXCEPTION(eslESYS, "mpi unpack failed");
	}
	if (MPI_Unpack(buf, n, pos, &block->blockL,     1, MPI_LONG_LONG_INT, comm) != 0) {
		ESL_XEXCEPTION(eslESYS, "mpi unpack failed");
	}
	if (MPI_Unpack(buf, n, pos, &block->complete,   1, MPI_INT, comm)           != 0) {
		ESL_XEXCEPTION(eslESYS, "mpi unpack failed");
	}
