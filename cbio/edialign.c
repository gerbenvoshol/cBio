...
int main(int argc, char **argv)
{
	...
	*/
	...
	for (ii = 1; ii < max_dia + 1; ii++) {
		if (tp400_prot) {
			AJFREE(tp400_prot[ii]);
		}
		if (tp400_dna) {
			AJFREE(tp400_dna[ii]);
		}
		if (tp400_trans) {
			AJFREE(tp400_trans[ii]);
		}
		if (wgt_prot) {
			AJFREE(wgt_prot[ii]);
		}
                                  ...
                                  ajint right)
{
	...
	do {
		...
		if (l < r) {
			edialign_change_struct_el(array, l, r);
		}
		if (l <= r) {
			l++;
			r--;
                                      ...
                                      ajint right)
{
	...
	do {
		while (array[l]->weight > element->weight) {
			l++;
		}
		while (element->weight  > array[r]->weight) {
			r--;
		}
