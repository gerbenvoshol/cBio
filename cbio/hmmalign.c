...
main(int argc, char **argv)
{
	...
	if (esl_opt_IsOn(go, "--informat")) {
		...
		if (infmt == eslSQFILE_UNKNOWN) {
			cmdline_failure(argv[0], "%s is not a recognized input sequence file format\n", esl_opt_GetString(go, "--informat"));
		}
	}
	...
	if (outfmt == eslMSAFILE_UNKNOWN) {
		cmdline_failure(argv[0], "%s is not a recognized output MSA file format\n", esl_opt_GetString(go, "--outformat"));
	}
	...
	if (status == eslEFORMAT) {
		p7_Fail("Bad file format in HMM file %s:\n%s\n",          hfp->fname, hfp->errbuf);
	} else if (status == eslEINCOMPAT) {
		p7_Fail("HMM in %s is not in the expected %s alphabet\n", hfp->fname, esl_abc_DecodeType(abc->type));
	} else if (status == eslEOF) {
		p7_Fail("Empty HMM file %s? No HMM data found.\n",        hfp->fname);
	} else if (status != eslOK) {
		p7_Fail("Unexpected error in reading HMMs from %s\n",     hfp->fname);
	}

	status = p7_hmmfile_Read(hfp, &abc, NULL);
	if (status != eslEOF) {
		p7_Fail("HMM file %s does not contain just one HMM\n",    hfp->fname);
	}
	...
	if (status == eslEFORMAT)
		...
		esl_fatal("Unexpected error %d reading sequence file %s", status, sqfp->filename);
	}
	...
	for (idx = 0; idx <= totseq; idx++) {
		esl_sq_Destroy(sq[idx]);    /* including sq[nseq] because we overallocated */
	}
	for (idx = 0; idx <  totseq; idx++) {
...
map_alignment(const char *msafile, const P7_HMM *hmm, ESL_SQ ***ret_sq, P7_TRACE ***ret_tr, int *ret_ntot)
{
	...
	if (!(hmm->flags & p7H_CHKSUM)) {
		esl_fatal("HMM has no checksum. --mapali unreliable without it.");
	}
	if (!(hmm->flags & p7H_MAP)) {
		esl_fatal("HMM has no map. --mapali can't work without it.");
	}
	esl_msa_Checksum(msa, &chksum);
	if (hmm->checksum != chksum) {
		esl_fatal("--mapali MSA %s isn't same as the one HMM came from (checksum mismatch)", msafile);
	}
