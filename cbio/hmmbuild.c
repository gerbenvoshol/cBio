...
process_commandline(int argc, char **argv, ESL_GETOPTS **ret_go, char **ret_hmmfile, char **ret_alifile)
{
	...
	if (esl_opt_ProcessEnvironment(go)         != eslOK) {
		if (printf("Failed to process environment:\n%s\n", go->errbuf) < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
	if (esl_opt_ProcessCmdline(go, argc, argv) != eslOK) {
		if (printf("Failed to parse command line:\n%s\n",  go->errbuf) < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
	...
	if (esl_opt_GetBoolean(go, "-h") == TRUE) {
		...
		if (puts("\nOptions for selecting alphabet rather than guessing it:") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		esl_opt_DisplayHelp(stdout, go, 2, 2, 80);

		if (puts("\nAlternative model construction strategies:") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		esl_opt_DisplayHelp(stdout, go, 3, 2, 80);

		if (puts("\nAlternative relative sequence weighting strategies:") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		esl_opt_DisplayHelp(stdout, go, 4, 2, 80);

		if (puts("\nAlternative effective sequence weighting strategies:") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		esl_opt_DisplayHelp(stdout, go, 5, 2, 80);

		if (puts("\nAlternative prior strategies:") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		esl_opt_DisplayHelp(stdout, go, 9, 2, 80);

		if (puts("\nHandling single sequence inputs:") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		esl_opt_DisplayHelp(stdout, go, 10, 2, 80);


		if (puts("\nControl of E-value calibration:") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
	...
	if (esl_opt_ArgNumber(go)                  != 2)    {
		if (puts("Incorrect number of command line arguments.")          < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
	if ((*ret_hmmfile = esl_opt_GetArg(go, 1)) == NULL) {
		if (puts("Failed to get <hmmfile_out> argument on command line") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
	if ((*ret_alifile = esl_opt_GetArg(go, 2)) == NULL) {
		if (puts("Failed to get <msafile> argument on command line")     < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
	...
	if (strcmp(*ret_hmmfile, "-") == 0) {
		if (puts("Can't write <hmmfile_out> to stdout: don't use '-'")         < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
	if (strcmp(*ret_alifile, "-") == 0 && ! esl_opt_IsOn(go, "--informat")) {
		if (puts("Must specify --informat to read <alifile> from stdin ('-')") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
	...
	if (esl_opt_IsOn(go, "--mpi") && esl_opt_IsOn(go, "--cpu")) {
		...
		if (mpisetby == cpusetby) {
			if (puts("Options --cpu and --mpi are incompatible. The MPI implementation is not multithreaded.") < 0) {
				ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
			}
			goto FAILURE;
		}
...
usual_master(const ESL_GETOPTS *go, struct cfg_s *cfg)
{
	 ...
	 */
	if (esl_opt_GetBoolean(go, "--amino")) {
		cfg->abc = esl_alphabet_Create(eslAMINO);
	} else if (esl_opt_GetBoolean(go, "--dna")) {
		cfg->abc = esl_alphabet_Create(eslDNA);
	} else if (esl_opt_GetBoolean(go, "--rna")) {
	...
	if (cfg->hmmfp == NULL) {
		p7_Fail("Failed to open HMM file %s for writing", cfg->hmmfile);
	}

	if (esl_opt_IsUsed(go, "-o")) {
		cfg->ofp = fopen(esl_opt_GetString(go, "-o"), "w");
		if (cfg->ofp == NULL) {
			p7_Fail("Failed to open -o output file %s\n", esl_opt_GetString(go, "-o"));
		}
	} else {
		cfg->ofp = stdout;
	}
	...
	if (cfg->postmsafile) {
		...
		if (cfg->postmsafp == NULL) {
			p7_Fail("Failed to resave MSA file %s", cfg->postmsafile);
		}
	} else {
		cfg->postmsafp = NULL;
	}
	...
	if (cfg->seqweights_r_file) {
		...
		if (cfg->seqweights_r_fp == NULL) {
			p7_Fail("Failed to write sequence weight (W) file %s", cfg->seqweights_r_file);
		}
	} else {
		cfg->seqweights_r_fp = NULL;
	}
	...
	if (cfg->seqweights_e_file) {
		...
		if (cfg->seqweights_e_fp == NULL) {
			p7_Fail("Failed to write sequence weight (W) file %s", cfg->seqweights_e_file);
		}
	} else {
		cfg->seqweights_e_fp = NULL;
	}
	...
	for (i = 0; i < infocnt; ++i) {
		...
		if (info[i].bld == NULL) {
			p7_Fail("p7_builder_Create failed");
		}
		...
		if (!esl_opt_GetBoolean(go, "--pnone") && !esl_opt_GetBoolean(go, "--plaplace")) {
			if (esl_opt_IsUsed(go, "--tmm")) {
				info[i].bld->prior->tm->alpha[0][0] = esl_opt_GetReal(go, "--tmm");    // TMM
			}
			if (esl_opt_IsUsed(go, "--tmi")) {
				info[i].bld->prior->tm->alpha[0][1] = esl_opt_GetReal(go, "--tmi");    // TMM
			}
			if (esl_opt_IsUsed(go, "--tmd")) {
				info[i].bld->prior->tm->alpha[0][2] = esl_opt_GetReal(go, "--tmd");    // TMM
			}

			if (esl_opt_IsUsed(go, "--tim")) {
				info[i].bld->prior->ti->alpha[0][0] = esl_opt_GetReal(go, "--tim");    // TMM
			}
			if (esl_opt_IsUsed(go, "--tii")) {
				info[i].bld->prior->ti->alpha[0][1] = esl_opt_GetReal(go, "--tii");    // TMM
			}

			if (esl_opt_IsUsed(go, "--tdm")) {
				info[i].bld->prior->td->alpha[0][0] = esl_opt_GetReal(go, "--tdm");    // TMM
			}
			if (esl_opt_IsUsed(go, "--tdd")) {
				info[i].bld->prior->td->alpha[0][1] = esl_opt_GetReal(go, "--tdd");    // TMM
			}
		...
		if (esl_opt_IsUsed(go, "--singlemx")) {
			...
			if (esl_opt_IsOn(go, "--mxfile")) {
				status = p7_builder_SetScoreSystem(info[i].bld, esl_opt_GetString(go, "--mxfile"), NULL, popen, pextend, info[i].bg);
			} else {
				status = p7_builder_LoadScoreSystem(info[i].bld, mx,                                      popen, pextend, info[i].bg);
			}
			if (status != eslOK) {
			...
			if (esl_opt_IsUsed(go, "--popen")) {
				info[i].bld->popen   = popen;
			}
			if (esl_opt_IsUsed(go, "--pextend")) {
				info[i].bld->pextend = pextend;
			}
		}
		...
		if (info[i].bld->w_beta < 0 || info[i].bld->w_beta > 1) {
			esl_fatal("Invalid window-length beta value\n");
		}
...
mpi_master(const ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	status = esl_msafile_Open(&(cfg->abc), cfg->alifile, NULL, cfg->fmt, NULL, &(cfg->afp));
	if (status != eslOK) {
		mpi_init_open_failure(cfg->afp, status);
	}

	cfg->hmmfp = fopen(cfg->hmmfile, "w");
	if (cfg->hmmfp == NULL) {
		mpi_init_other_failure("Failed to open HMM file %s for writing", cfg->hmmfile);
	}

	if (esl_opt_IsUsed(go, "-o")) {
		cfg->ofp = fopen(esl_opt_GetString(go, "-o"), "w");
		if (cfg->ofp == NULL) {
			mpi_init_other_failure("Failed to open -o output file %s\n", esl_opt_GetString(go, "-o"));
		}
	} else {
		cfg->ofp = stdout;
	}
	...
	if (cfg->postmsafile) {
		cfg->postmsafp = fopen(cfg->postmsafile, "w");
		if (cfg->postmsafp == NULL) {
			mpi_init_other_failure("Failed to MSA resave file %s for writing", cfg->postmsafile);
		}
	} else {
		cfg->postmsafp = NULL;
	}
	 ...
	 */
	bn = 4096;
	if ((buf     = malloc(sizeof(char) * bn))              == NULL) {
		mpi_init_other_failure("allocation failed");
	}
	if ((msalist = malloc(sizeof(ESL_MSA *) * cfg->nproc)) == NULL) {
		mpi_init_other_failure("allocation failed");
	}
	if ((msaidx  = malloc(sizeof(int)       * cfg->nproc)) == NULL) {
		mpi_init_other_failure("allocation failed");
	}
	if ((bg      = p7_bg_Create(cfg->abc))                 == NULL) {
		mpi_init_other_failure("allocation failed");
	}
	...
	if (status != eslOK) {
		MPI_Finalize();
		p7_Fail("One or more MPI worker processes failed to initialize.");
	}
	...
	while (have_work || nproc_working) {
		if (have_work) {
			...
			if (rstatus == eslOK)  {
				...
				have_work  = FALSE;
				xstatus = rstatus;
				ESL_DPRINTF1(("MPI master msa read has failed... start to shut down\n"));
			}
		}

		if ((have_work && nproc_working == cfg->nproc - 1) || (!have_work && nproc_working > 0)) {
			if (MPI_Probe(MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &mpistatus) != 0) {
				MPI_Finalize();
				p7_Fail("mpi probe failed");
			}
			if (MPI_Get_count(&mpistatus, MPI_PACKED, &n)                != 0) {
				MPI_Finalize();
				p7_Fail("mpi get count failed");
			}
			...
			if (n > bn) {
				if ((buf = realloc(buf, sizeof(char) * n)) == NULL) {
					p7_Fail("reallocation failed");
				}
				bn = n;
			}
			if (MPI_Recv(buf, bn, MPI_PACKED, wi, 0, MPI_COMM_WORLD, &mpistatus) != 0) {
				MPI_Finalize();
				p7_Fail("mpi recv failed");
			}
			...
			if (xstatus == eslOK) {
				pos = 0;
				if (MPI_Unpack(buf, bn, &pos, &xstatus, 1, MPI_INT, MPI_COMM_WORLD)     != 0) {
					MPI_Finalize();
					p7_Fail("mpi unpack failed");
				}
				if (xstatus == eslOK) { /* worker reported success. Get the HMM. */
					ESL_DPRINTF1(("MPI master sees that the result buffer contains an HMM\n"));
					if (p7_hmm_MPIUnpack(buf, bn, &pos, MPI_COMM_WORLD, &(cfg->abc), &hmm) != eslOK) {
						MPI_Finalize();
						p7_Fail("HMM unpack failed");
					}
					ESL_DPRINTF1(("MPI master has unpacked the HMM\n"));

					if (cfg->postmsafile != NULL) {
						if (esl_msa_MPIUnpack(cfg->abc, buf, bn, &pos, MPI_COMM_WORLD, &postmsa) != eslOK) {
							MPI_Finalize();
							p7_Fail("postmsa unpack failed");
						}
					}

					entropy = p7_MeanMatchRelativeEntropy(hmm, bg);
					if ((status = output_result(cfg, errmsg, msaidx[wi], msalist[wi], hmm, postmsa, entropy)) != eslOK) {
						xstatus = status;
					}
					...
					if (MPI_Unpack(buf, bn, &pos, errmsg, eslERRBUFSIZE, MPI_CHAR, MPI_COMM_WORLD) != 0) {
						MPI_Finalize();
						p7_Fail("mpi unpack of errmsg failed");
					}
		...
		if (have_work) {
			...
			if (esl_msa_MPISend(msa, wi, 0, MPI_COMM_WORLD, &buf, &bn) != eslOK) {
				p7_Fail("MPI msa send failed");
			}
			msalist[wi] = msa;
	...
	for (wi = 1; wi < cfg->nproc; wi++)
		if (esl_msa_MPISend(NULL, wi, 0, MPI_COMM_WORLD, &buf, &bn) != eslOK) {
			p7_Fail("MPI msa send failed");
		}
	...
	if (rstatus != eslOK) {
		MPI_Finalize();
		esl_msafile_ReadFailure(cfg->afp, rstatus);
	} else if (xstatus != eslOK) {
		MPI_Finalize();
		p7_Fail(errmsg);
	} else {
		return;
...
mpi_init_other_failure(char *format, ...)
{
	...
	if (vfprintf(stderr, format, argp) < 0) {
		exit(eslEWRITE);
	}
	va_end(argp);
...
mpi_worker(const ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	MPI_Bcast(&xstatus, 1, MPI_INT, 0, MPI_COMM_WORLD);
	if (xstatus != eslOK) {
		return;    /* master saw an error code; workers do an immediate normal shutdown. */
	}
	...
	if (xstatus == eslOK) {
		if ((cfg->abc = esl_alphabet_Create(type))      == NULL) {
			xstatus = eslEMEM;
		}
	}
	if (xstatus == eslOK) {
		wn = 4096;
		if ((wbuf = malloc(wn * sizeof(char))) == NULL) {
			xstatus = eslEMEM;
		}
	}
	if (xstatus == eslOK) {
		if ((bld = p7_builder_Create(go, cfg->abc))     == NULL) {
			xstatus = eslEMEM;
		}
	}
	...
	while (esl_msa_MPIRecv(0, 0, MPI_COMM_WORLD, cfg->abc, &wbuf, &wn, &msa) == eslOK) {
		...
		if (msa->nseq == 1 && esl_opt_IsUsed(go, "--singlemx")) {
			...
			if ((status = esl_sq_FetchFromMSA(msa, 0, &sq)) != eslOK) {
				strcpy(errmsg, bld->errbuf);
				goto ERROR;
			}
			if ((status = p7_SingleBuilder(bld, sq, bg, &hmm, NULL, NULL, NULL)) != eslOK) {
				strcpy(errmsg, bld->errbuf);
				goto ERROR;
			}
			esl_sq_Destroy(sq);
			...
			if ((status = p7_Builder(bld, msa, bg, &hmm, NULL, NULL, NULL, postmsa_ptr, NULL, NULL)) != eslOK) {
				strcpy(errmsg, bld->errbuf);
				goto ERROR;
			}
		}
		...
		n = 0;
		if (MPI_Pack_size(1,    MPI_INT, MPI_COMM_WORLD, &sz) != 0) {
			goto ERROR;
		}
		n += sz;
		if (p7_hmm_MPIPackSize(hmm,     MPI_COMM_WORLD, &sz) != eslOK) {
			goto ERROR;
		}
		n += sz;
		if (esl_msa_MPIPackSize(postmsa, MPI_COMM_WORLD, &sz) != eslOK) {
			goto ERROR;
		}
		n += sz;
		if (n > wn) {
		...
		if (p7_hmm_MPIPack(hmm,                 wbuf, wn, &pos, MPI_COMM_WORLD) != eslOK) {
			goto ERROR;
		}
		if (esl_msa_MPIPack(postmsa,             wbuf, wn, &pos, MPI_COMM_WORLD) != eslOK) {
			goto ERROR;
		}
...
serial_loop(WORKER_INFO *info, struct cfg_s *cfg, const ESL_GETOPTS *go)
{
	...
	while ((status = esl_msafile_Read(cfg->afp, &msa)) != eslEOF) {
		...
		if ((status = set_msa_name(cfg, errmsg, msa)) != eslOK) {
		...
		if (msa->nseq == 1 && esl_opt_IsUsed(go, "--singlemx")) {
			...
			if ((status = p7_SingleBuilder(info->bld, sq, info->bg, &hmm, NULL, NULL, NULL)) != eslOK) {
				p7_Fail("build failed: %s", bld->errbuf);
			}
			esl_sq_Destroy(sq);
			sq = NULL;
			...
			if ((status = p7_Builder(info->bld, msa, info->bg, &hmm, NULL, NULL, NULL, postmsa_ptr, cfg->seqweights_r_fp,  cfg->seqweights_e_fp)) != eslOK) {
				p7_Fail("build failed: %s", bld->errbuf);
			}
		...
		if ((status = output_result(cfg, errmsg, cfg->nali, msa, hmm, postmsa, entropy))         != eslOK) {
			p7_Fail(errmsg);
		}
...
thread_loop(ESL_THREADS *obj, ESL_WORK_QUEUE *queue, struct cfg_s *cfg, const ESL_GETOPTS *go)
{
	...
	while (sstatus == eslOK) {
		...
		if (sstatus == eslOK) {
			item->nali = ++cfg->nali;
			if (set_msa_name(cfg, errmsg, item->msa) != eslOK) {
				p7_Fail("%s\n", errmsg);
			}
		} else if (sstatus == eslEOF && processed < cfg->nali) {
			sstatus = eslOK;
...
pipeline_thread(void *arg)
{
	...
	while (item->msa != NULL) {
		...
		if (item->msa->nseq == 1 && item->force_single) {
			...
			if (status != eslOK) {
				p7_Fail("build failed: %s", info->bld->errbuf);
			}
...
output_result(const struct cfg_s *cfg, char *errbuf, int msaidx, ESL_MSA *msa, P7_HMM *hmm, ESL_MSA *postmsa, double entropy)
{
	...
	if (msa == NULL) {
		if (cfg->abc->type == eslAMINO) {
			if (fprintf(cfg->ofp, "#%4s %-20s %5s %5s %5s %8s %6s %s\n", " idx", "name",                 "nseq",  "alen",  "mlen",  "eff_nseq",  "re/pos",  "description")     < 0) {
			...
			if (fprintf(cfg->ofp, "#%4s %-20s %5s %5s %5s %5s %8s %6s %s\n", " idx", "name",                 "nseq",  "alen",  "mlen",  "W", "eff_nseq",  "re/pos",  "description")     < 0) {
				ESL_EXCEPTION_SYS(eslEWRITE, "output_result: write failed");
			}
			if (fprintf(cfg->ofp, "#%4s %-20s %5s %5s %5s %5s %8s %6s %s\n", "----", "--------------------", "-----", "-----", "-----", "-----", "--------",  "------",  "-----------") < 0) {
				ESL_EXCEPTION_SYS(eslEWRITE, "output_result: write failed");
...
//  if ((status = p7_hmm_Validate(hmm, errbuf, 0.0001))       != eslOK) return status;
	if ((status = p7_hmmfile_WriteASCII(cfg->hmmfp, -1, hmm)) != eslOK) {
		ESL_FAIL(status, errbuf, "HMM save failed");
	}
...
set_msa_name(struct cfg_s *cfg, char *errbuf, ESL_MSA *msa)
{
	...
	if (cfg->do_mpi == FALSE && cfg->nali == 1) { /* first (only?) HMM in file: */
		if (cfg->hmmName != NULL) {
			if ((status = esl_msa_SetName(msa, cfg->hmmName, -1)) != eslOK) {
				return status;
			}
		...
		if (cfg->hmmName   != NULL) {
			...
			ESL_FAIL(eslEINVAL, errbuf, "Oops. Wait. I need name annotation on each alignment in a multi MSA file; failed on #%d", cfg->nali + 1);
		}
