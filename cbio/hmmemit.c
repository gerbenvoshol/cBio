...
main(int argc, char **argv)
{
	...
	if (esl_opt_VerifyConfig(go)               != eslOK) {
		cmdline_failure(argv[0], "Error in configuration: %s\n",       go->errbuf);
	}
	...
	if ((hmmfile = esl_opt_GetArg(go, 1)) == NULL) {
		cmdline_failure(argv[0], "Failed to get <hmmfile> on cmdline: %s\n", go->errbuf);
	}

	if (esl_opt_IsOn(go, "-o")) {
		if ((ofp = fopen(esl_opt_GetString(go, "-o"), "w")) == NULL) {
			esl_fatal("Failed to open output file %s", esl_opt_GetString(go, "-o"));
		}
	} else {
		ofp = stdout;
	}
	...
	while ((status = p7_hmmfile_Read(hfp, &abc, &hmm)) != eslEOF) {
		if (status == eslEFORMAT) {
			esl_fatal("Bad file format in HMM file %s:\n%s\n",          hfp->fname, hfp->errbuf);
		} else if (status == eslEINCOMPAT) {
			esl_fatal("HMM in %s is not in the expected %s alphabet\n", hfp->fname, esl_abc_DecodeType(abc->type));
		} else if (status != eslOK) {
			esl_fatal("Unexpected error in reading HMMs from %s\n",     hfp->fname);
		}
		nhmms++;

		if (esl_opt_GetBoolean(go, "-c")) {
			emit_consensus(go, ofp, outfmt,    hmm);
		} else if (esl_opt_GetBoolean(go, "-C")) {
			emit_fancycons(go, ofp, outfmt,    hmm);
		} else if (esl_opt_GetBoolean(go, "-a")) {
			emit_alignment(go, ofp, outfmt, r, hmm);
		} else {
			emit_sequences(go, ofp, outfmt, r, hmm);
		}
	...
	if (nhmms == 0) {
		esl_fatal("Empty HMM file %s? No HMM data found.\n");
	}
...
emit_consensus(ESL_GETOPTS *go, FILE *ofp, int outfmt, P7_HMM *hmm)
{
	...
	if ((sq = esl_sq_CreateDigital(hmm->abc))             == NULL) {
		esl_fatal("failed to allocate sequence");
	}

	if (p7_emit_SimpleConsensus(hmm, sq)                 != eslOK) {
		esl_fatal("failed to create simple consensus seq");
	}
	if (esl_sq_FormatName(sq, "%s-consensus", hmm->name) != eslOK) {
		esl_fatal("failed to set sequence name");
	}
	if (esl_sqio_Write(ofp, sq, outfmt, FALSE)           != eslOK) {
		esl_fatal("failed to write sequence");
	}
...
emit_fancycons(ESL_GETOPTS *go, FILE *ofp, int outfmt, P7_HMM *hmm)
{
	...
	if ((sq = esl_sq_Create()) == NULL) {
		esl_fatal("failed to allocate sequence");
	}

	if (p7_emit_FancyConsensus(hmm, minl, minu, sq)      != eslOK) {
		esl_fatal("failed to create consensus seq");
	}
	if (esl_sq_FormatName(sq, "%s-consensus", hmm->name) != eslOK) {
		esl_fatal("failed to set sequence name");
	}
...
emit_alignment(ESL_GETOPTS *go, FILE *ofp, int outfmt, ESL_RANDOMNESS *r, P7_HMM *hmm)
{
	...
	if ((tr = malloc(sizeof(P7_TRACE *) * N)) == NULL) {
		esl_fatal("failed to allocate trace array");
	}
	if ((sq = malloc(sizeof(ESL_SQ   *) * N)) == NULL) {
		esl_fatal("failed to allocate seq array");
	}
	for (i = 0; i < N; i++) {
		if ((sq[i] = esl_sq_CreateDigital(hmm->abc)) == NULL) {
			esl_fatal("failed to allocate seq");
		}
		if ((tr[i] = p7_trace_Create())              == NULL) {
			esl_fatal("failed to allocate trace");
		}
	}

	for (i = 0; i < N; i++) {
		if (p7_CoreEmit(r, hmm, sq[i], tr[i])                       != eslOK) {
			esl_fatal("Failed to emit sequence");
		}
		if (esl_sq_FormatName(sq[i], "%s-sample%d", hmm->name, i + 1) != eslOK) {
			esl_fatal("Failed to set sequence name\n");
		}
	}
...
emit_sequences(ESL_GETOPTS *go, FILE *ofp, int outfmt, ESL_RANDOMNESS *r, P7_HMM *hmm)
{
	...
	if (esl_opt_GetBoolean(go, "--local")) {
		mode = p7_LOCAL;
	} else if (esl_opt_GetBoolean(go, "--unilocal")) {
		mode = p7_UNILOCAL;
	} else if (esl_opt_GetBoolean(go, "--glocal")) {
		mode = p7_GLOCAL;
	} else if (esl_opt_GetBoolean(go, "--uniglocal")) {
		mode = p7_UNIGLOCAL;
	}
	...
	if ((tr = p7_trace_Create())                   == NULL) {
		esl_fatal("failed to allocate trace");
	}
	if ((bg = p7_bg_Create(hmm->abc))              == NULL) {
		esl_fatal("failed to create null model");
	}
	if ((gm = p7_profile_Create(hmm->M, hmm->abc)) == NULL) {
		esl_fatal("failed to create profile");
	}

	if (p7_ProfileConfig(hmm, bg, gm, L, mode)     != eslOK) {
		esl_fatal("failed to configure profile");
	}
	if (p7_bg_SetLength(bg, L)                     != eslOK) {
		esl_fatal("failed to reconfig null model length");
	}
	if (p7_hmm_Validate(hmm, NULL, 0.0001)     != eslOK) {
		esl_fatal("whoops, HMM is bad!");
	}
	if (p7_profile_Validate(gm,  NULL, 0.0001)     != eslOK) {
		esl_fatal("whoops, profile is bad!");
	}
