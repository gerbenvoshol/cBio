...
main(int argc, char **argv)
{
	...
	if (esl_opt_GetBoolean(go, "--index")) {
		...
		if (strcmp(hmmfile, "-") == 0) {
			cmdline_failure(argv[0], "Can't use - with --index, can't index <stdin>.\n");
		}
	}
	...
	if (esl_opt_GetBoolean(go, "-O")) {
		if (! keyname) {
			p7_Fail("No key name? Can't use -O\n");
		}
		if ((ofp = fopen(keyname, "w")) == NULL)	{
			p7_Fail("Failed to open output file %s\n", keyname);
		}
	...
	if (esl_opt_GetBoolean(go, "-O") || esl_opt_GetString(go, "-o") != NULL) {
		fclose(ofp);
	}
...
create_ssi_index(ESL_GETOPTS *go, P7_HMMFILE *hfp)
{
	...
	if (esl_sprintf(&ssifile, "%s.ssi", hfp->fname) != eslOK) {
		p7_Die("esl_sprintf() failed");
	}

	status = esl_newssi_Open(ssifile, FALSE, &ns);
	if (status == eslENOTFOUND) {
		esl_fatal("failed to open SSI index %s", ssifile);
	} else if (status == eslEOVERWRITE) {
		esl_fatal("SSI index %s already exists; delete or rename it", ssifile);
	} else if (status != eslOK) {
		esl_fatal("failed to create a new SSI index");
	}
	...
	while ((status = p7_hmmfile_Read(hfp, &abc, &hmm)) != eslEOF) {
		if (status == eslEOD) {
			p7_Fail("read failed, HMM file %s may be truncated?", hfp->fname);
		} else if (status == eslEFORMAT) {
			p7_Fail("bad file format in HMM file %s",             hfp->fname);
		} else if (status == eslEINCOMPAT) {
			p7_Fail("HMM file %s contains different alphabets",   hfp->fname);
		} else if (status != eslOK) {
			p7_Fail("Unexpected error in reading HMMs from %s",   hfp->fname);
		}
		nhmm++;
...
multifetch(ESL_GETOPTS *go, FILE *ofp, char *keyfile, P7_HMMFILE *hfp)
{
	...
	if (esl_fileparser_Open(keyfile, NULL, &efp) != eslOK) {
		p7_Fail("Failed to open key file %s\n", keyfile);
	}
	...
	while (esl_fileparser_NextLine(efp) == eslOK) {
		...
		if (status == eslEDUP) {
			p7_Fail("HMM key %s occurs more than once in file %s\n", key, keyfile);
		}

		if (hfp->ssi != NULL) {
			onefetch(go, ofp, key, hfp);
			nhmm++;
		}
	}
...
onefetch(ESL_GETOPTS *go, FILE *ofp, char *key, P7_HMMFILE *hfp)
{
	...
	if (hfp->ssi != NULL) {
		...
		if (status == eslENOTFOUND) {
			p7_Fail("HMM %s not found in SSI index for file %s\n", key, hfp->fname);
		} else if (status == eslEFORMAT) {
			p7_Fail("Failed to parse SSI index for %s\n", hfp->fname);
		} else if (status != eslOK) {
			p7_Fail("Failed to look up location of HMM %s in SSI index of file %s\n", key, hfp->fname);
		}
	}
	...
	while ((status = p7_hmmfile_Read(hfp, &abc, &hmm)) != eslEOF) {
		...
		if (strcmp(key, hmm->name) == 0 || (hmm->acc && strcmp(key, hmm->acc) == 0)) {
			break;
		}
		p7_hmm_Destroy(hmm);
		hmm = NULL;
	...
	if (status == eslOK) {
		...
		p7_Fail("HMM %s not found in file %s\n", key, hfp->fname);
	}
