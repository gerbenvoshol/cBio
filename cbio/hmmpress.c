...
main(int argc, char **argv)
{
	...
	if (strcmp(hmmfile, "-") == 0) {
		p7_Fail("Can't use - for <hmmfile> argument: can't index standard input\n");
	}
	...
	if (hfp->do_stdin || hfp->do_gzip) {
		p7_Fail("HMM file %s must be a normal file, not gzipped or a stdin pipe", hmmfile);
	}
	...
	while ((status = p7_hmmfile_Read(hfp, &abc, &hmm)) == eslOK) {
		if (hmm->name == NULL) {
			p7_Fail("Every HMM must have a name to be indexed. Failed to find name of HMM #%d\n", nmodel + 1);
		}
		...
		if ((om->offs[p7_MOFFSET] = ftello(mfp)) == -1) {
			p7_Fail("Failed to ftello() current disk position of HMM db file");
		}
		if ((om->offs[p7_FOFFSET] = ftello(ffp)) == -1) {
			p7_Fail("Failed to ftello() current disk position of MSV db file");
		}
		if ((om->offs[p7_POFFSET] = ftello(pfp)) == -1) {
			p7_Fail("Failed to ftello() current disk position of profile db file");
		}

#ifndef p7_IMPL_DUMMY
		if (esl_newssi_AddKey(nssi, hmm->name, fh, om->offs[p7_MOFFSET], 0, 0) != eslOK)	{
			p7_Fail("Failed to add key %s to SSI index", hmm->name);
		}
		if (hmm->acc) {
			if (esl_newssi_AddAlias(nssi, hmm->acc, hmm->name) != eslOK) {
	...
	if (esl_newssi_Write(nssi) != eslOK) {
		p7_Fail("Failed to write keys to ssi file\n");
	}
...
open_db_files(ESL_GETOPTS *go, char *basename, FILE **ret_mfp,  FILE **ret_ffp,  FILE **ret_pfp, ESL_NEWSSI **ret_nssi)
{
	...
	if (esl_sprintf(&ssifile, "%s.h3i", basename) != eslOK) {
		p7_Die("esl_sprintf() failed");
	}
	status = esl_newssi_Open(ssifile, allow_overwrite, &nssi);
	if (status == eslENOTFOUND) {
		p7_Fail("failed to open SSI index %s", ssifile);
	} else if (status == eslEOVERWRITE) {
		p7_Fail("Looks like %s is already pressed (.h3i file present, anyway):\nDelete old hmmpress indices first", basename);
	} else if (status != eslOK) {
		p7_Fail("failed to create a new SSI index");
	}

	if (esl_sprintf(&mfile, "%s.h3m", basename) != eslOK) {
		p7_Die("esl_sprintf() failed");
	}
	if (! allow_overwrite && esl_FileExists(mfile)) {
		p7_Fail("Binary HMM file %s already exists;\nDelete old hmmpress indices first", mfile);
	}
	if ((mfp = fopen(mfile, "wb"))              == NULL) {
		p7_Fail("Failed to open binary HMM file %s for writing", mfile);
	}

	if (esl_sprintf(&ffile, "%s.h3f", basename) != eslOK) {
		p7_Die("esl_sprintf() failed");
	}
	if (! allow_overwrite && esl_FileExists(ffile)) {
		p7_Fail("Binary MSV filter file %s already exists\nDelete old hmmpress indices first", ffile);
	}
	if ((ffp = fopen(ffile, "wb"))              == NULL) {
		p7_Fail("Failed to open binary MSV filter file %s for writing", ffile);
	}

	if (esl_sprintf(&pfile, "%s.h3p", basename) != eslOK) {
		p7_Die("esl_sprintf() failed");
	}
	if (! allow_overwrite && esl_FileExists(pfile)) {
		p7_Fail("Binary profile file %s already exists\nDelete old hmmpress indices first", pfile);
	}
	if ((pfp = fopen(pfile, "wb"))              == NULL) {
		p7_Fail("Failed to open binary profile file %s for writing", pfile);
	}
