...
process_commandline(int argc, char **argv, ESL_GETOPTS **ret_go, char **ret_hmmfile, char **ret_seqfile)
{
	...
	if (esl_opt_GetBoolean(go, "-h") == TRUE) {
		...
		if (puts("\nOptions controlling output:")                              < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		esl_opt_DisplayHelp(stdout, go, 2, 2, 80);

		if (puts("\nOptions controlling reporting thresholds:")                < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		...
		if (puts("\nOptions for model-specific thresholding:")                 < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		esl_opt_DisplayHelp(stdout, go, 6, 2, 80);

		if (puts("\nOptions controlling acceleration heuristics:")             < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		esl_opt_DisplayHelp(stdout, go, 7, 2, 80);

		if (puts("\nOther expert options:")                                    < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
	...
	if ((*ret_hmmfile = esl_opt_GetArg(go, 1)) == NULL)  {
		if (puts("Failed to get <hmmdb> argument on command line")   < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
	if ((*ret_seqfile = esl_opt_GetArg(go, 2)) == NULL)  {
		if (puts("Failed to get <seqfile> argument on command line") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
	...
	if (strcmp(*ret_hmmfile, "-") == 0) {
		if (puts("hmmscan cannot read <hmm database> from stdin stream, because it must have hmmpress'ed auxfiles") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
...
FAILURE:  /* all errors handled here are user errors, so be polite.  */
	esl_usage(stdout, argv[0], usage);
	if (puts("\nwhere most common options are:")                                 < 0) {
		ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
	}
	esl_opt_DisplayHelp(stdout, go, 1, 2, 80); /* 1= group; 2 = indentation; 80=textwidth*/
...
serial_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if (esl_opt_IsOn(go, "--daemon")) {
		...
		if (strcmp(cfg->seqfile, "-") != 0) {
			esl_fatal("Query sequence file must be '-'\n");
		}
	}
	...
	if (esl_opt_IsOn(go, "--domtblout")) {
		if ((domtblfp = fopen(esl_opt_GetString(go, "--domtblout"), "w")) == NULL) {
			esl_fatal("Failed to open tabular per-dom output file %s for writing\n", esl_opt_GetString(go, "--domtblout"));
		}
	}
	if (esl_opt_IsOn(go, "--pfamtblout")) {
		if ((pfamtblfp = fopen(esl_opt_GetString(go, "--pfamtblout"), "w")) == NULL) {
			esl_fatal("Failed to open pfam-style tabular output file %s for writing\n", esl_opt_GetString(go, "--pfamtblout"));
		}
	}
	...
	while ((sstatus = esl_sqio_Read(sqfp, qsq)) == eslOK) {
		...
		if (ncpus > 0) {
			...
			if (status != eslOK) {
				p7_Fail("Unexpected error %d creating lock\n", status);
			}
		}
#endif

		if (fprintf(ofp, "Query:       %s  [L=%ld]\n", qsq->name, (long) qsq->n) < 0) {
			ESL_EXCEPTION_SYS(eslEWRITE, "write failed");
		}
		...
		if (tblfp) {
			p7_tophits_TabularTargets(tblfp,    qsq->name, qsq->acc, info->th, info->pli, (nquery == 1));
		}
		if (domtblfp) {
			p7_tophits_TabularDomains(domtblfp, qsq->name, qsq->acc, info->th, info->pli, (nquery == 1));
		}
		if (pfamtblfp) {
			p7_tophits_TabularXfam(pfamtblfp, qsq->name, qsq->acc, info->th, info->pli);
		}
	 ...
	 */
	if (tblfp) {
		p7_tophits_TabularTail(tblfp,    "hmmscan", p7_SCAN_MODELS, cfg->seqfile, cfg->hmmfile, go);
	}
	if (domtblfp) {
		p7_tophits_TabularTail(domtblfp, "hmmscan", p7_SCAN_MODELS, cfg->seqfile, cfg->hmmfile, go);
	}
	if (pfamtblfp) {
		p7_tophits_TabularTail(pfamtblfp, "hmmscan", p7_SEARCH_SEQS, cfg->seqfile, cfg->hmmfile, go);
	}
	if (ofp)      {
		if (fprintf(ofp, "[ok]\n") < 0) {
			ESL_EXCEPTION_SYS(eslEWRITE, "write failed");
...
mpi_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	status = p7_hmmfile_OpenE(cfg->hmmfile, p7_HMMDBENV, &hfp, errbuf);
	if (status == eslENOTFOUND) {
		mpi_failure("File existence/permissions problem in trying to open HMM file %s.\n%s\n", cfg->hmmfile, errbuf);
	} else if (status == eslEFORMAT) {
		mpi_failure("File format problem, trying to open HMM file %s.\n%s\n",                  cfg->hmmfile, errbuf);
	} else if (status != eslOK) {
		mpi_failure("Unexpected error %d in opening HMM file %s.\n%s\n",               status, cfg->hmmfile, errbuf);
	}
	if (! hfp->is_pressed) {
		mpi_failure("Failed to open binary auxfiles for %s: use hmmpress first\n",             hfp->fname);
	}

	hstatus = p7_oprofile_ReadMSV(hfp, &abc, &om);
	if (hstatus == eslEFORMAT) {
		mpi_failure("bad format, binary auxfiles, %s",            cfg->hmmfile);
	} else if (hstatus == eslEINCOMPAT) {
		mpi_failure("HMM file %s contains different alphabets",   cfg->hmmfile);
	} else if (hstatus != eslOK) {
		mpi_failure("Unexpected error in reading HMMs from %s",   cfg->hmmfile);
	}
	...
	while ((sstatus = esl_sqio_Read(sqfp, qsq)) == eslOK) {
		...
		p7_tophits_Targets(ofp, th, pli, textw);
		fprintf(ofp, "\n\n");
		p7_tophits_Domains(ofp, th, pli, textw);
		fprintf(ofp, "\n\n");

		if (tblfp) {
			p7_tophits_TabularTargets(tblfp,    qsq->name, qsq->acc, th, pli, (nquery == 1));
		}
		if (domtblfp) {
			p7_tophits_TabularDomains(domtblfp, qsq->name, qsq->acc, th, pli, (nquery == 1));
		}
		if (pfamtblfp) {
			p7_tophits_TabularXfam(pfamtblfp,   qsq->name, qsq->acc, th, pli);
		}
...
mpi_worker(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if (status == eslENOTFOUND) {
		...
		mpi_failure("File format problem in trying to open HMM file %s.\n%s\n",                cfg->hmmfile, errbuf);
	} else if (status != eslOK) {
		mpi_failure("Unexpected error %d in opening HMM file %s.\n%s\n",               status, cfg->hmmfile, errbuf);
	}
	if (! hfp->is_pressed) {
		mpi_failure("Failed to open binary dbs for HMM file %s: use hmmpress first\n",         hfp->fname);
	}
