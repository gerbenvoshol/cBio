...
process_commandline(int argc, char **argv, ESL_GETOPTS **ret_go, char **ret_hmmfile, char **ret_seqfile)
{
	...
	if (esl_opt_GetBoolean(go, "-h") == TRUE) {
		...
		if (puts("\nOptions controlling model-specific thresholding:")         < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
	...
	if ((*ret_hmmfile = esl_opt_GetArg(go, 1)) == NULL)  {
		if (puts("Failed to get <hmmfile> argument on command line") < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
	if ((*ret_seqfile = esl_opt_GetArg(go, 2)) == NULL)  {
		if (puts("Failed to get <seqdb> argument on command line")   < 0) {
			ESL_XEXCEPTION_SYS(eslEWRITE, "write failed");
		}
		goto FAILURE;
	}
...
main(int argc, char **argv)
{
	...
	if (esl_opt_IsUsed(go, "--restrictdb_stkey"))
		if ((cfg.firstseq_key = esl_opt_GetString(go, "--restrictdb_stkey")) == NULL) {
			p7_Fail("Failure capturing --restrictdb_stkey\n");
		}
...
serial_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if (esl_opt_IsOn(go, "-A"))          {
		if ((afp      = fopen(esl_opt_GetString(go, "-A"), "w")) == NULL) {
			p7_Fail("Failed to open alignment file %s for writing\n", esl_opt_GetString(go, "-A"));
		}
	}
	...
	while (hstatus == eslOK) {
...
#ifdef HMMER_THREADS
		if (ncpus > 0) {
			sstatus = thread_loop(threadObj, queue, dbfp, cfg->n_targetseq);
		} else {
		...
		if (tblfp) {
			p7_tophits_TabularTargets(tblfp,    hmm->name, hmm->acc, info->th, info->pli, (nquery == 1));
		}
		if (domtblfp) {
			p7_tophits_TabularDomains(domtblfp, hmm->name, hmm->acc, info->th, info->pli, (nquery == 1));
		}
		if (pfamtblfp) {
			p7_tophits_TabularXfam(pfamtblfp, hmm->name, hmm->acc, info->th, info->pli);
		}
	 ...
	 */
	if (tblfp) {
		p7_tophits_TabularTail(tblfp,    "hmmsearch", p7_SEARCH_SEQS, cfg->hmmfile, cfg->dbfile, go);
	}
	if (domtblfp) {
		p7_tophits_TabularTail(domtblfp, "hmmsearch", p7_SEARCH_SEQS, cfg->hmmfile, cfg->dbfile, go);
	}
	if (pfamtblfp) {
		p7_tophits_TabularTail(pfamtblfp, "hmmsearch", p7_SEARCH_SEQS, cfg->hmmfile, cfg->dbfile, go);
	}
	if (ofp)      {
		if (fprintf(ofp, "[ok]\n") < 0) {
			ESL_EXCEPTION_SYS(eslEWRITE, "write failed");
...
int next_block(ESL_SQFILE *sqfp, ESL_SQ *sq, BLOCK_LIST *list, SEQ_BLOCK *block, int n_targetseqs)
{
	...
	esl_sq_Reuse(sq);
	if (n_targetseqs == 0) {
		status = eslEOF;    //this is to handle the end-case of a restrictdb scenario, where no more targets are required, and we want to mark the list as complete
	}
	while (block->length < MAX_BLOCK_SIZE && (n_targetseqs < 0 || block->count < n_targetseqs) && (status = esl_sqio_ReadInfo(sqfp, sq)) == eslOK) {
		if (block->count == 0) {
			block->offset = sq->roff;
		}
...
mpi_master(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	if (esl_opt_IsOn(go, "--tformat")) {
		dbfmt = esl_sqio_EncodeFormat(esl_opt_GetString(go, "--tformat"));
		if (dbfmt == eslSQFILE_UNKNOWN) {
			mpi_failure("%s is not a recognized sequence database file format\n", esl_opt_GetString(go, "--tformat"));
		}
	}
	...
	while (hstatus == eslOK) {
		...
		if (tblfp) {
			p7_tophits_TabularTargets(tblfp,    hmm->name, hmm->acc, th, pli, (nquery == 1));
		}
		if (domtblfp) {
			p7_tophits_TabularDomains(domtblfp, hmm->name, hmm->acc, th, pli, (nquery == 1));
		}
		if (pfamtblfp) {
			p7_tophits_TabularXfam(pfamtblfp, hmm->name, hmm->acc, th, pli);
		}
	...
	case eslEINCOMPAT:
		...
		break;
	case eslEOF:       /* EOF is good, that's what we expect here */
		break;
	default:
...
mpi_worker(ESL_GETOPTS *go, struct cfg_s *cfg)
{
	...
	while (hstatus == eslOK) {
		...
		while (block.count > 0) {
			...
			if (status != eslOK) {
				mpi_failure("Cannot position sequence database to %ld\n", block.offset);
			}
