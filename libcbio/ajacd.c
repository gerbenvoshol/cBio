		...
		while ((item = ajListIterGet(iter))) {
			...
			*/
			for (i = 0; acdinputs[i]; i++)
				if (ajStrMatchC(item->Type, acdinputs[i])) {
					break;
				}
	...
	if (ajListGetLength(outlist)) {
		...
		while ((item = ajListIterGet(iter))) {
			for (i = 0; acdoutputs[i]; i++)
				if (ajStrMatchC(item->Type, acdoutputs[i])) {
					break;
				}
...
void ajAcdPrintAppl(AjPFile outf, AjBool full)
{
	...
	if (maxtmp > 12)
		ajWarn("ajAcdPrintAppl max tmpstr len %d",
		       maxtmp);
	ajStrDel(&tmpstr);
...
void ajAcdPrintQual(AjPFile outf, AjBool full)
{
	...
	ajFmtPrintF(outf, "}\n\n");
	if (maxtmp > 12)
		ajWarn("ajAcdPrintQual max tmpstr len %d",
		       maxtmp);
	ajStrDel(&tmpstr);
